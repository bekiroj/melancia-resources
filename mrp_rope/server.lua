...
if not exports["mrp_items"]:hasItem(plr,46) then outputChatBox("[!]#ffffff Maalesef üzerinizde ip yok.",plr,255,100,100,true) return end
		local target, targetPlayerName = exports.mrp_global:findPlayerByPartialNick(plr, target)
	...
	if target then
		...
		exports.mrp_global:sendLocalMeAction(plr, "Şahsın sağ ve sol ellerini birleştirerek ip ile sıkıca bağlar.")
		...
		exports.mrp_handcuff:kelepcele(target, "ipver")
		exports["mrp_items"]:takeItem(plr, 46)
	end
end)
...
addEventHandler("ipcoz",root,function(plr,target)
	...
	if target then
		...
		exports.mrp_global:sendLocalMeAction(plr, "Şahsın bileklerindeki ipi keserek çözer.")
		...
		exports.mrp_handcuff:kelepcele(target, "ipal")
	end
end)