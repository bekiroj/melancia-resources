...
function resourceStart(resource)
	setWaveHeight ( 0 )
	setGameType("Roleplay Project")
	setMapName("İstanbul")
	setRuleValue("Mode Version", exports.mrp_global:getScriptVersion())
	setRuleValue("Açıklama", "Melancia")
...
function resetNick(oldNick, newNick)
	...
	exports.mrp_global:sendMessageToAdmins("Admin Warn: " .. tostring(oldNick) .. " kendi adını değiştirmek için çalıştı " .. tostring(newNick) .. ".")
end
...
function loginCallback(queryHandler,source,username,password)
	...
	if rows > 0 then
		...
		exports['mrp_reports']:reportLazyFix(source)
		...
		if exports.mrp_integration:isPlayerLeadAdmin(source) then
			setElementDataEx(source, "hiddenadmin", row["hiddenadmin"], true)
		else
			setElementDataEx(source, "hiddenadmin", 0, true)
		end

		local vehicleConsultationTeam = exports.mrp_integration:isPlayerVehicleConsultant(source)
		...
		dbQuery(
			function(qh, source)
				...
				triggerEvent( "social:mrp_account", source, tonumber( row["id"] ) )
			end,
...
function myCallback( responseData, errno, id )
    if errno == 0 then
        exports.mrp_cache:addImage(id, responseData)
	end
end
...
function loginCharacterAdminCallback(qh, thePlayer, name)
	...
	if rows > 0 then
        ...
        local theAdminPower = exports.mrp_global:getPlayerAdminLevel(thePlayer)
        if targetCharID and  targetUserID then
            local adminTitle = exports.mrp_global:getPlayerFullIdentity(thePlayer)
            if targetAdminLevel > theAdminPower then
                ...
                exports.mrp_global:sendMessageToAdmins("[GİRİŞ]: " .. tostring(adminTitle) .. " yüksek yetkiye sahip bir yöneticinin karakterine girmeye çalıştı ("..targetUsername..").")
                return false
            end
           
            spawnCharacter(targetCharID, targetUserID, thePlayer, targetUsername)  
            exports.mrp_global:sendMessageToAdmins("[GİRİŞ]: " .. tostring(adminTitle) .. " hesabına giriş yaptı, '"..targetUsername.."'.")
        end
	else
			...
			if rows > 0 then
				...
				if characterData then
					...
					if isElement(playerWithNick) and (playerWithNick~=client) then
						if theAdmin then
							local adminTitle = exports.mrp_global:getPlayerAdminTitle(theAdmin)
					...
					exports["mrp_items"]:loadItems(client)
					...
					if (tonumber(characterData["faction_id"])~=-1) then
						teamElement = exports.mrp_pool:getElement('team', tonumber(characterData["faction_id"]))
					...
					exports.mrp_global:updateNametagColor(client)
					-- ADMIN JAIL
					...
					if jailed then
						...
						if jailed_time ~= 999 then
							if not getElementData(client, "jailtimer") then
								...
								--exports['mrp_admins']:timerUnjailPlayer(client)
					...
					exports.mrp_global:setMoney(client, tonumber(characterData["money"]), true)
					exports.mrp_global:checkMoneyHacks(client)
					...
					exports["mrp_vipsystem"]:loadVIP(tonumber(characterData["id"] ))
					...
					--	dbExec(mysql:getConnection(), "UPDATE characters SET online='1', lastlogin=NOW() WHERE id='" .. characterID .. "'")
						...
						if monitored then
							if monitored ~= "New Player" then
								exports.mrp_global:sendMessageToAdmins("[MONITOR] ".. getPlayerName(client):gsub("_", " ") .." ("..pid.."): "..monitored)
								exports.mrp_global:sendMessageToSupporters("[MONITOR] ".. getPlayerName(client):gsub("_", " ") .." ("..pid.."): "..monitored)
							end
						end
					end
					...
					if theAdmin then
						...
						if hiddenAdmin == 0 then
							exports.mrp_global:sendMessageToAdmins("AdmKmt: " .. tostring(adminTitle) .. " "..adminUsername.." başka bir hesaba girdi, ("..targetAccountName..") "..tostring(characterData["charactername"]):gsub("_"," ")..".")
						end
					end
...
function Characters_onCharacterChange()
	...
	exports.mrp_logs:dbLog("ac"..tostring(clientAccountID), 27, { "ac"..tostring(clientAccountID), client } , "Went to character selection" )
	...
	if padId then
		...
		for key, thePad in pairs(getElementsByType("object", getResourceRootElement(getResourceFromName("mrp_item_world")))) do
...
function removeMasksAndBadges(client)
    for k, v in ipairs({exports['mrp_items']:getMasks(), exports['mrp_items']:getBadges()}) do
	...
	local characters = {}
		dbQuery(
			function(qh, source)
				...
				setElementData(source, "account:characters", characters)
				triggerClientEvent(source, "accounts:login:attempt", source, 0 )
				triggerEvent( "social:mrp_account", source, acID)
			end,
