...
function showSettingsWindow(tab)
	...
	if getResourceFromName("mrp_logs") then
	...
	if getResourceFromName("mrp_logs") and getResourceFromName("mrp_phone") then
	...
	if getResourceFromName("mrp_chat") then
	...
	if getResourceFromName("mrp_chat") and guiCheckBoxGetSelected(GUIEditor_Checkbox.graphic_chatbub) then
	...
	if getResourceFromName("mrp_hud") then
	...
	if getResourceFromName("mrp_shader_radar") then
	...
	if getResourceFromName("mrp_shader_water") then
	...
	if getResourceFromName("mrp_shader_car_paint") then
	...
	if getResourceFromName('mrp_vehicle') then
	...
	if getResourceFromName("mrp_realism") then
	...
	posY = posY + lineH
	local dyn_light = exports.mrp_global:isResourceRunning('mrp_dynamic_lighting') and true or false
	...
	local xmas = exports.mrp_global:isResourceRunning('mrp_xmas') and true or false
	if xmas then
		posY = posY + lineH
	...
	if exports.mrp_integration:isPlayerSupporter(getLocalPlayer()) or exports.mrp_integration:isPlayerTrialAdmin(getLocalPlayer()) then
		posY = posY + lineH
	...
	if exports.mrp_integration:isPlayerStaff(localPlayer) then
		posY = posY + lineH
	...
	if getResourceFromName("mrp_description") then
...
function applyGameSettings(name, value)
	if name and value then
		...
		elseif name == "hide_hud" then
			...
			elseif exports.mrp_global:hasItem(localPlayer, 111) then
				setPlayerHudComponentVisible("radar", true)
			end
		elseif name == "groundsnow" then
			local groundsnow = exports.mrp_global:isResourceRunning('mrp_shader_snow_ground') and true or false
			if groundsnow then
				if value == "0" then
