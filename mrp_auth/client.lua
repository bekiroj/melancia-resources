...
local BizNoteFont18 = dxCreateFont ( ":mrp_resources/BizNote.ttf" , 18 )
...
function Exclusive.loginStart()
	font = DxFont(":mrp_hud/Bebas.ttf", 18)
...
function Exclusive.drawnCharacters(table)
	...
	destroyTimer = setTimer(
		function()
			...
			showCursor(true);
			lastArrayClick = 0
			font2 = DxFont(":mrp_interface/fonts/FontAwesome.ttf")
			fadeCamera ( true, 1, 0,0,0 );
...
function characters_onSpawn(fixedName, adminLevel, gmLevel, location)
	...
	setTimer(
		...
		local syntax = exports.mrp_pool:getServerSyntax(false, "e")
...
function Exclusive.drawnNewCharacter()
	...
	if creationSteps.currentStep == 5 then
		creationSteps.stepTexts['top'][creationSteps.currentStep] = pname.." olarak İstanbul'a iniyorsun.."
	end
...
function Exclusive.createGUI()
	...
	for _, value in ipairs(exports['mrp_languages']:getLanguageList()) do 
	...
	guiSetEnabled(gui["chkAsian"], false)
	...
	if (button == "left") then 
		...
		creationSteps['language'] = exports['mrp_languages']:getLanguageName(row + 1)
	...
	addEventHandler("onClientGUIClick", gui['langWindowCloseBtn'], function(button)
		if (source == gui['langWindowCloseBtn'] and button == "left") then
			...
			triggerServerEvent("accounts:characters:new", localPlayer, pname, 'Melancia', race, gender, skin, pheight, pweight, page, languageselected, 1, 1, { 1922.9072265625, -1760.6982421875, 13.546875, 0, 0, 0, ""})
