...
function getPlateList()
	...
	for _,vehicleElement in ipairs( exports.mrp_pool:getPoolElementsByType("vehicle") ) do
		if (getElementData(vehicleElement, "owner")) and (tonumber(getElementData(vehicleElement, "owner")) == tonumber(playerDBID)) and exports['mrp_vehicle']:hasVehiclePlates(vehicleElement) then
...
function pedTalk(state)
	...
	elseif (state == 3) then
	elseif (state == 4) then
		exports.mrp_global:sendLocalText(source, "[English] Gabrielle McCoy says: No? Well I hope you change your mind later. Have a nice day!", 255, 255, 255, 10)
	elseif (state == 5) then
		exports.mrp_global:sendLocalText(source, " *Gabrielle McCoy begins inputting the information into her computer.", 255, 51, 102)
		exports.mrp_global:sendLocalText(source, "[English] Gabrielle McCoy says: Alright, you should be good to go. Have a nice day!", 255, 255, 255, 10)
	elseif (state == 6) then
		exports.mrp_global:sendLocalText(source, "[English] Gabrielle McCoy says: Hmmm. According to our records, that is already a registered license plate.", 255, 255, 255, 10)
	elseif (state == 7) then
		exports.mrp_global:sendLocalText(source, "[English] Gabrielle McCoy says: Well, I'm sorry but your vehicle doesn't require a registered plate or papers.", 255, 255, 255, 10)
	elseif (state == 8) then
		exports.mrp_global:sendLocalText(source, "[English] Gabrielle McCoy says: I'm sorry but are you the owner of this vehicle on papers?", 255, 255, 255, 10)
	end
end
...
function setNewInfo(data, car)
	...
	local tvehicle = exports.mrp_pool:getElement("vehicle", car)
	if not exports['mrp_vehicle']:hasVehiclePlates(tvehicle) then
	...
	exports.mrp_anticheat:changeProtectedElementDataEx(tvehicle, "plate", data, true)
...
function setNewReg(car)
	...
	if not exports.mrp_global:takeMoney(source, data == 1 and 175 or 50) then
		exports.mrp_global:sendLocalText(source, "[English] Gabrielle McCoy says: Could I have $"..(data == 1 and 175 or 50).." please?", 255, 255, 255, 10)
	end

	exports.mrp_anticheat:changeProtectedElementDataEx(tvehicle, "registered", data, true)
...
function givePaperToSellVehicle(thePlayer)
	source = thePlayer
	exports.mrp_global:takeMoney(thePlayer, 100)
	exports.mrp_global:giveItem(thePlayer, 173, 1)
end
