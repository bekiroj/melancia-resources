...
local gabby = createPed(150,  1111.36328125, -776.4462890625, 976.80633544922)
--setPedRotation(gabby, 270.00) -- Old Rotation
setPedRotation(gabby, 97.98)
setElementDimension(gabby, 84)
...
setElementDimension(tonny, 1084)
...
function getPaperFromTony(button, state, absX, absY, wx, wy, wz, element)
    if (element) and (getElementType(element)=="ped") and (button=="right") and (state=="down") then --if it's a right-click on a object
        ...
        if(pedName == "Tony Johnston") then
            rcMenu = exports.mrp_rightclick:create(pedName)
            showCursor(true)
            local row = exports.mrp_rightclick:addRow("Buy DMV transaction paper - 100$")
            addEventHandler("onClientGUIClick", row,  function (button, state)
            	if exports.mrp_global:hasMoney(localPlayer, 100) and getElementData(localPlayer, "loggedin") == 1 then
            ...
            local row2 = exports.mrp_rightclick:addRow("Close")
            addEventHandler("onClientGUIClick", row2,  function (button, state)
                exports.mrp_rightclick:destroy(rcMenu)
                showCursor(false)
            end, false)
        end
    end
...
function cBeginGUI()
	...
	greetingWindow = guiCreateStaticImage(x, y, width, height, ":mrp_resources/window_body.png", false)
...
function PlateWindow(vehicleList)
	...
	for i,r in ipairs(vehicleList) do
		...
		guiGridListSetItemText(vehlist, row, ov, exports.mrp_global:getVehicleName(v) , false, false) --tostring(getVehicleNameFromModel(vm))
...
function editPlateWindow()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		efinalWindow = guiCreateStaticImage(x, y, width, height, ":mrp_resources/window_body.png", false)
		...
		submitNP = guiCreateButton(0.1, 0.60, 0.8, 0.15, "Purchase (Cost: "..cost.." $)", true, efinalWindow)
...
function updateRegistration()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		if exports.mrp_global:hasMoney(localPlayer,  state == "Unregistered" and 175 or 50 ) then
...
function insuranceDetails()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		local insuranceFactionName = exports.mrp_factions:getFactionName(tonumber(data.insurancefaction))
		...
		string = string.."Premium: $"..exports.mrp_global:formatMoney(tonumber(data.premium)).."\n"
