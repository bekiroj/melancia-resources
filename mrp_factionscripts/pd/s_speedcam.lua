--[[
	...
	* Created for United Gaming (owlgaming.net)
...
addCommandHandler({"togspeedcamera", "togglespeedcamera", "togspeedcam", "togglespeedcam", "togspeed", "togglespeed"},
	function(player, cmd, speed)
		...
		if vehicle then
			if tonumber(getElementData(vehicle, "faction")) == 1 or tonumber(getElementData(vehicle, "faction")) == 6 and (getVehicleController(vehicle) == player) then
							...
							setTimer(function(hitElement, x, y, z, kmh)
								...
								if isVerifiedElement(radar, "vehicle") and getElementData(radar, "speedcamera:state") then
									...
									for seat,player in pairs(getVehicleOccupants(radar)) do
										outputChatBox("[RADAR] A " .. getColorName(c1, c2) .. " " .. exports.mrp_global:getVehicleName(hitElement) .. " was clocked travelling at " .. tonumber(kmh) .. " km/h and is heading " .. direction .. ".", player, 255, 180, 20, false)
