function beanbagFired(x, y, z, target)
	...
	if (distance<35) then
		if (isElement(target) and getElementType(target)=="player") then
			...
			exports.mrp_global:applyAnimation(target, "ped", "FLOOR_hit_f", -1, false, false, true, true)
...
function updateShotgunMode(mode)
	if ( tonumber(mode) and (tonumber(mode) >= 0 and tonumber(mode) <= 1) ) then
		exports.mrp_anticheat:changeProtectedElementDataEx(client, "shotgunmode", mode, true)
	end
end
