...
function yakaTelsiz(thePlayer, commandName, ...)
		...
		if not (...) then
			...
			exports.mrp_global:sendLocalMeAction(thePlayer, "Telsizinin tuşuna basarak konuşmaya başlar.")
...
function panic(thePlayer, commandName)
        ...
        if (logged==1) and getElementData(thePlayer, "faction")==1 then -- Kodun Uygulanacağı Fact ID'ı
			...
			local factionRankTitle = factionRanks[factionRank]
			local username = getPlayerName(thePlayer)
			exports.mrp_global:sendLocalMeAction(thePlayer, "Panik butonuna basar.")
            for k, arrayPlayer in ipairs(getPlayersInTeam(getTeamFromName ("İstanbul Emniyet Müdürlüğü"))) do  -- Kodun Uygulanacağı Fact Adı
				if getElementData(thePlayer, "faction")==1 then -- Kodun Uygulanacağı Fact ID'ı
					outputChatBox("#6464FF[!]#8B8B8E (CH: 155) " .. username:gsub("_", " ") .. " panik butonuna bastı, yönelmeniz bekleniyor.", arrayPlayer, 0,0,0,true)
...
function operator(thePlayer, cmd, ...)
	...
	if getElementData(thePlayer, "faction") == 1 or getElementData(thePlayer, "faction") == 2 or getElementData(thePlayer, "faction") == 78 then
		...
		local username = getPlayerName(thePlayer)
		exports.mrp_global:sendLocalMeAction(thePlayer, "Telsizinin tuşuna basarak konuşmaya başlar.")
		for k, pdplayer in ipairs(getPlayersInTeam(getTeamFromName ("İstanbul Emniyet Müdürlüğü"))) do  -- Kodun Uygulanacağı Fact Adı
			outputChatBox("[operator] ".. factionRankTitle .. " " .. username:gsub("_", " ") .. " : " .. message.."",pdplayer,100,100,255,true)
		end

		for k, mdplayer in ipairs(getPlayersInTeam(getTeamFromName ("İstanbul Devlet Hastanesi"))) do  -- Kodun Uygulanacağı Fact Adı
			outputChatBox("[operator] ".. factionRankTitle .. " " .. username:gsub("_", " ") .. " : " .. message.."",mdplayer,100,100,255,true)
		end

		for k, lscsdplayer in ipairs(getPlayersInTeam(getTeamFromName ("İstanbul İl Jandarma Komutanlığı"))) do  -- Kodun Uygulanacağı Fact Adı
