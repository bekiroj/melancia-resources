function unsitInHelicopter(vehicle)
	...
	if (isElement(vehicle)) then
		if not (getElementType(vehicle)=="vehicle") then
			...
			for key, value in ipairs(helicopters) do
				...
				if (players) then
					...
					if (removed) then
						exports.mrp_anticheat:changeProtectedElementDataEx(value, "players", players, true)
					end
				end
			end
			
			exports.mrp_anticheat:changeProtectedElementDataEx(source, "seat")
			detachElements(source, vehicle)
			exports.mrp_global:removeAnimation(source)
		elseif (seat) and (seat>0) then
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(vehicle, "players", players, true)
			exports.mrp_anticheat:changeProtectedElementDataEx(source, "seat")
			detachElements(source, vehicle)
			exports.mrp_global:removeAnimation(source)
		end
...
function sitInHelicopter(vehicle)
	...
	if (not players) or (#players<3) then
		...
		if not (s1) then
			...
			exports.mrp_global:applyAnimation(source, "FOOD", "FF_Sit_Look", 999999, true, true, false)
		...
		players[seat] = source
		exports.mrp_anticheat:changeProtectedElementDataEx(source, "seat", seat)
