addCommandHandler("maske",function(plr)
	if plr:getData("vip") > 1 then		
		if not plr:getData("maske:tak") then
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(plr, "fakename", random, true)
		else
			plr:outputChat("[!]#ffffff Maskenizi çıkardınız.",0,255,0,true)
			exports.mrp_anticheat:changeProtectedElementDataEx(plr, "fakename", false, true)
...
addCommandHandler("maskeliler",function(plr)
	if exports.mrp_integration:isPlayerTrialAdmin(plr) then
