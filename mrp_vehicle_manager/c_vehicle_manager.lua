...
function createVehManagerWindow(data)
    if not exports.mrp_integration:isPlayerTrialAdmin(localPlayer) and not exports.mrp_integration:isPlayerVCTMember(localPlayer) and not exports.mrp_integration:isPlayerScripter(localPlayer) then
        return outputChatBox("Access denied.", 255,0,0)
    end
    ...
    -- vehList1 = vehList
    gWin = guiCreateWindow(sx / 2 - 400, sy / 2 - 300, 800, 600, "Araç Yönetimi @ 2020 - Melancia", false)
    ...
    for i = 1, 2 do
        ...
        colName[i] = guiGridListAddColumn(targetList[i], "Name", 0.3)
        ...
        colOwner[i] = guiGridListAddColumn(targetList[i], "Owner", 0.15)
    end
    ...
    local lTotal = guiCreateLabel(460, 25, 164, 17, "Standard Vehicles: " .. exports.mrp_global:formatMoney(#standardVehs), false, gWin)
    local lActive = guiCreateLabel(460, 42, 164, 17, "Unique Vehicles: " .. exports.mrp_global:formatMoney(#uniqueVehs), false, gWin)
    ...
    function fetchSearchResults(interiorsResultList)
        if interiorsResultList then
            ...
            for _, record in ipairs(interiorsResultList) do
                ...
                guiGridListSetItemText(targetList[i], row, colName[i], formatVehName(), false, false)
                ...
                --guiGridListSetItemText(targetList[i], row, colOwner[i], exports.mrp_cache:getCharacterNameFromID(getElementData(veh, "owner")) or "Unknown", false, false)
...
function loadTab(vehs, i)
    for index, veh in ipairs(vehs) do
        ...
        guiGridListSetItemText(targetList[i], row, colName[i], exports.mrp_global:getVehicleName(veh), false, false)
        ...
        --guiGridListSetItemText(targetList[i], row, colOwner[i], exports.mrp_cache:getCharacterNameFromID(getElementData(veh, "owner")) or "Unknown", false, false)
        guiGridListSetItemText(targetList[i], row, colLastUsed[i], formatLastUsed(getElementData(veh, "lastused_sec")) , false, false)
...
function formatLastUsed(sec)
    if sec or not tonumber(sec) then
        ...
        return exports.mrp_datetime:formatTimeInterval(sec).." ago"
    end
end
...
function createCheckVehWindow(result, adminTitle, history, notes1)
    ...
    guiSetText(lMileageAndHP, "Kilometre: " ..(math.floor(tonumber(result[1][17]) / 1000) or "0") .. " KM")
    ...
    if result[1][21] then
        ...
        false)
        guiSetEnabled(bRemoveVeh1, exports.mrp_integration:isPlayerSeniorAdmin(localPlayer))
