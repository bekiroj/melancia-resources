...
function refreshCarShop()
	local theResource = getResourceFromName("mrp_vehicle_shop")
	if theResource then
		...
		if getResourceState(theResource) == "running" then
			...
			if hiddenAdmin == 0 then
				exports.mrp_global:sendMessageToAdmins("[VEHICLE MANAGER]" .. getPlayerName(client) .. " restarted the carshops.")
			else
				exports.mrp_global:sendMessageToAdmins("[VEHICLE MANAGER] A hidden admin restarted the carshops.")
			end
			--exports.mrp_logs:logMessage("[STEVIE] " .. getElementData(client, "account:username") .. "/".. getPlayerName(client) .." restarted the gatekeepers." , 25)
			exports.mrp_logs:dbLog(client, 4, client, "RESETCARSHOP")
		elseif getResourceState(theResource) == "loaded" then
			...
			if hiddenAdmin == 0 then
				exports.mrp_global:sendMessageToAdmins("[VEHICLE MANAGER] " .. getPlayerName(client) .. " started the carshops.")
			else
				exports.mrp_global:sendMessageToAdmins("[VEHICLE MANAGER] A hidden admin started the carshops.")
			end
			exports.mrp_logs:dbLog(client, 4, client, "RESETCARSHOP")
...
--		outputDebugString("VEHICLE MANAGER / VEHICLE LIB / createVehicleRecord / NO DATA RECIEVED FROM CLIENT")
	...
	if not data.update then
		...
		exports.mrp_logs:dbLog(client, 6, { client }, " Created new vehicle in library.")
		exports.mrp_global:sendMessageToAdmins("[vehicle_manager]: "..getElementData(client, "account:username").." has created new vehicle in library.")
		return true
	else
		if data.copy then
			dbExec(mysql:getConnection(), "INSERT INTO vehicles_shop SET vehmtamodel='"..toSQL(data["mtaModel"]).."', vehbrand='"..toSQL(data["brand"]).."', vehmodel='"..toSQL(data["model"]).."', vehyear='"..toSQL(data["year"]).."', vehprice='"..toSQL(data["price"]).."', vehtax='"..toSQL(data["tax"]).."', createdby='"..toSQL(getElementData(client, "account:id")).."', notes='"..toSQL(data["note"]).."' , enabled='"..toSQL(enabled).."', `spawnto`='"..toSQL(data["spawnto"]).."', `doortype` = " .. data.doortype)
			...
			exports.mrp_logs:dbLog(client, 6, { client }, " Updated vehicle #"..data.id.." from library.")
			exports.mrp_global:sendMessageToAdmins("[vehicle_manager]: "..getElementData(client, "account:username").." has updated vehicle #"..data.id.." in library.")
			return true
		end
	end
end
...
function giveTempVctAccess(thePlayer, commandName, targetPlayer)
	if exports.mrp_integration:isPlayerSeniorAdmin(thePlayer) or exports.mrp_integration:isPlayerVehicleConsultant(thePlayer) then
		if not targetPlayer then
			...
			if not exports.mrp_integration:isPlayerVCTMember(targetPlayer) and not exports.mrp_integration:isPlayerSeniorAdmin(thePlayer) then
			...
			local thePlayerIdentity = exports.mrp_global:getPlayerFullIdentity(thePlayer)
			local targetPlayerIdentity = exports.mrp_global:getPlayerFullIdentity(targetPlayer)
			
			if not hasVctAdmin then
				if not (exports.mrp_integration:isPlayerSeniorAdmin(thePlayer) or exports.mrp_integration:isPlayerVehicleConsultant(thePlayer)) then
				...
				if setElementData(targetPlayer, "hasVctAdmin", true) then
					...
					exports.mrp_global:sendMessageToAdmins("[VCT] "..thePlayerIdentity.." has given " ..targetPlayerIdentity.. " temporary VCT admin.")
					exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, commandName)
				end
			else
				if setElementData(targetPlayer, "hasVctAdmin", false) then
					...
					exports.mrp_global:sendMessageToAdmins("[VCT] "..thePlayerIdentity.." has revoked from " .. targetPlayerIdentity .. " temporary VCT admin.")
				end
			end
		end
	end
