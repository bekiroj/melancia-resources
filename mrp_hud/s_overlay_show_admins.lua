...
local function getPlayerScripterRank( player )
	if exports.mrp_integration:isPlayerLeadScripter( player ) then
		return "Scripter"
	elseif exports.mrp_integration:isPlayerScripter( player ) then
		return "Deneme Scripter"
	elseif exports.mrp_integration:isPlayerTester( player ) then
		return "Deneme"
	else
		return ""
	end
...
local function getPlayerSupportRank( player )
	if exports.mrp_integration:isPlayerSupporter( player ) then
		return "Rehber"
	else
		return ""
	end
...
function showStaff( thePlayer, commandName )
	...
	-- ADMINS --
	if(logged==1) then
		local players = exports.mrp_global:getAdmins()
		local counter = 0
		...
		for k, arrayPlayer in ipairs(players) do
			...
			if logged == 1 then
				if tonumber(getElementData( arrayPlayer, "admin_level" )) < 10 then
					if exports.mrp_integration:isPlayerTrialAdmin(arrayPlayer) and ( hiddenAdmin == 0 or ( exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerScripter(thePlayer) ) ) and not exports.mrp_integration:isPlayerIA( arrayPlayer ) then
						admins[ #admins + 1 ] = { arrayPlayer, getElementData( arrayPlayer, "admin_level" ), getElementData( arrayPlayer, "duty_admin" ), exports.mrp_global:getPlayerName( arrayPlayer ) }
					end
				end
			end
		end
		...
		for k, v in ipairs(admins) do
			arrayPlayer = v[1]
			local adminTitle = exports.mrp_global:getPlayerAdminTitle(arrayPlayer)
			local hiddenAdmin = getElementData(arrayPlayer, "hiddenadmin")
			if hiddenAdmin == 0 or exports.mrp_integration:isPlayerTrialAdmin(thePlayer) then
	...
	--GMS--
	if(logged==1) then
		local players = exports.mrp_global:getGameMasters()
		local counter = 0
		...
		for k, arrayPlayer in ipairs(players) do
			local logged = getElementData(arrayPlayer, "loggedin")
			if logged == 1 then
				if exports.mrp_integration:isPlayerSupporter(arrayPlayer) then
					admins[ #admins + 1 ] = { arrayPlayer, getElementData( arrayPlayer, "account:gmlevel" ), getElementData( arrayPlayer, "duty_supporter" ), exports.mrp_global:getPlayerName( arrayPlayer ) }
				end
			end
		end
	...
	if logged == 1 then
		if not isOverlayDisabled then
			exports.mrp_hud:sendTopRightNotification(thePlayer, info, 350)
		end
	end
end
