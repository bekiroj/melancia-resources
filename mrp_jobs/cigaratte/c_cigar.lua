...
function cigarAcceptGUI(thePlayer)
	...
	addEventHandler("onClientGUIClick", acceptBtn, 
		function()
			...
			setTimer(function() exports.mrp_hud:sendBottomNotification(getLocalPlayer(), "Sigara Kaçakçılığı", "Yandaki beyaz kamyonlardan birini alıp, /sigara basla yazarak işe başlayabilirsiniz!") end, 500, 1)
			return	
		end
	)
...
function cstartJob(cmd, arg)
	if arg == "basla" then
		if not cigarBlip then --not routeMarker then
			...
			exports.mrp_hud:sendBottomNotification(localPlayer, "Sigara Kaçakçılığı", "Zaten bir sefere başladınız.")
		end
	end
end
				...
				for _, marker in ipairs(cigarOldMarker) do
					if source == marker[1] and matchingDimension then
						...
						elseif marker[2] == true and marker[3] == true then
							...
							exports.mrp_hud:sendBottomNotification(hitPlayer, "Sigara Kaçakçılığı", "Aracınıza yeni mallar yükleniyor, lütfen bekleyiniz.")
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									destroyElement(hitMarker)
									exports.mrp_hud:sendBottomNotification(thePlayer, "Sigara Kaçakçılığı", "Aracınıza yeni mallar yüklenmiştir. Gidebilirsiniz.")
						...
						elseif marker[2] == true and marker[3] == false then
							...
							setElementFrozen(hitPlayer, true)
							setElementFrozen(hitVehicle, true)
							exports.mrp_hud:sendBottomNotification(hitPlayer, "Sigara Kaçakçılığı", "Aracınızdaki mallar indiriliyor, lütfen bekleyiniz.")
							setTimer(
								function(thePlayer, hitVehicle, hitMarker)
									destroyElement(hitMarker)
									exports.mrp_hud:sendBottomNotification(hitPlayer, "Sigara Kaçakçılığı", "Aracınızdaki mallar indirilmiştir, geri dönebilirsiniz.")
									setElementFrozen(hitVehicle, false)
									setElementFrozen(thePlayer, false)
									cupdateRoutes()
								end, 1000, 1, hitPlayer, hitVehicle, source
...
function ccancelJob()
	...
	if pedVeh then
		if pedVehModel == 459 then
			...
			setTimer(function() exports.mrp_global:fadeFromBlack() end, 2000, 1)
		end
	end
end
