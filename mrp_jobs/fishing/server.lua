		...
		if isElementWithinColShape(thePlayer, balikCol) then
			if (not getElementData(thePlayer, "balikTutuyor")) then
				...
				if toplamyem > 0 then
					...
					exports.mrp_global:applyAnimation(thePlayer, "SWORD", "sword_IDLE", -1, false, true, true, false)
					setElementData(thePlayer, "balikTutuyor", true)
					exports.mrp_global:sendLocalMeAction(thePlayer, "oltasını denize doğru sallar.", false, true)
					...
					setTimer(function(thePlayer) 
						...
						if rastgeleSayi == 2 then
							...
							exports["mrp_items"]:giveItem(thePlayer, 273, givenFishTable.name)
...
addCommandHandler("balikdurum", 
	function(thePlayer, cmd)
		...
		local toplamBalik = exports["mrp_items"]:countItems(thePlayer, 273)
	...
	if para >= 1 then
		if isElementWithinColShape(thePlayer, yemCol) then
			...
			elseif toplamyem <= 20 then
				exports.mrp_global:takeMoney(thePlayer, 1)
...
function balikSat(thePlayer, cmd)
	local countOfPlayerFish = exports["mrp_items"]:countItems(thePlayer, 273)
	local countPrice = 0
	local playerFishs = exports["mrp_items"]:getItems(thePlayer)
	...
	if isElementWithinColShape(thePlayer, yemCol) then
		...
		if toplambalik <= 0 then
			...
			exports.mrp_global:giveMoney(thePlayer, countPrice)
			for i = 0, countOfPlayerFish do
				exports["mrp_items"]:takeItem(thePlayer, 273)
			end
