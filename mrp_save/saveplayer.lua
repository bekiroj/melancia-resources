...
function syncTIS()
	for key, value in ipairs(getElementsByType("player")) do
		...
		if (tis) and (getPlayerIdleTime(value) < 600000)  then
			exports.mrp_anticheat:changeProtectedElementDataEx(value, "timeinserver", tonumber(tis)+1, false)
		end
	end
end
...
addEventHandler("onResourceStart", root,
    function(startedRes)
        if getResourceName(startedRes) == "mrp_mysql" then
...
function savePlayer(reason, player)
	...
	if isElement(player) then
		...
		if (logged==1 or reason=="Change Character") then
			...
			if exports['mrp_freecam']:isPlayerFreecamEnabled(player) then 
			...
			local zone = exports.mrp_global:getElementZoneName(player)
...
addCommandHandler("saveall", function(p) if exports.mrp_integration:isPlayerSeniorAdmin(p) then beginSave() outputChatBox("Done.", p) end end)
