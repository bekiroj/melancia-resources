...
function getItemPropertiesData(object)
	...
	if creator > 0 then
		creatorName = exports.mrp_global:getCharacterNameFromID(creator)
	end
...
function saveItemProperties(object, use, useData, move, moveData, pickup, pickupData)
	...
	if getElementParent(getElementParent(object)) == getResourceRootElement(getResourceFromName("mrp_item_world")) then
		...
		if id then
			...
			local itemName = exports['mrp_items']:getItemName(tonumber(getElementData(object, "itemID")), getElementData(object, "itemValue"))
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(object, "worlditem.permissions", permissions)
