...
anticheat = exports.mrp_anticheat
items = exports['mrp_items']
itemtexture = exports['mrp_item_texture']
global = exports.mrp_global
integration = exports.mrp_integration

function canEditItemProperties(thePlayer, object)
	if not object then return false end
	local interiorID = getElementDimension(object)
	if exports.mrp_global:hasItem(localPlayer, 4, interiorID) or exports.mrp_global:hasItem(localPlayer, 5, interiorID) or (exports.mrp_integration:isPlayerTrialAdmin(thePlayer) and exports.mrp_global:isAdminOnDuty(thePlayer)) or exports.mrp_integration:isPlayerScripter(thePlayer) then
		return true
	end
	return false
end
...
function can(player, action, element)
	...
	elseif usePerm == 8 then --query string
		...
		local badges = exports["mrp_items"]:getBadges()
			...
			if(#orString > 1) then
				...
				for k, v in ipairs(orString) do
					...
					if isNumeric(theItem[1]) then --if number, then treat it as a itemID
						local hasItem, key, value2, value3 = exports.mrp_global:hasItem(thePlayer, tonumber(theItem[1]))
						if hasItem then
							...
							if theItem[2] then
								...
								if(tonumber(exports['mrp_items']:countItems(thePlayer, tonumber(theItem[1]), theItem[2])) > 0) then
									countOr = countOr + 1
						...
						if textFunction == "PILOT" then --check for pilot licenses
							local pilotlicenses = exports['mrp_mdc']:getPlayerPilotLicenses(thePlayer) or {}
							...
							if theItem[2] then --if a value is specified
								...
								if isNumeric(checkFaction[1]) then --if it is a number, we're checking for faction ID
									if isNumeric(checkFaction[2]) then --if faction rank is specified and is a number (valid)
										if exports.mrp_factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]), tonumber(checkFaction[2])) then
											countOr = countOr + 1
										end
									else
										if exports.mrp_factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1])) then
											countOr = countOr + 1
										end
									end
									...
									local factionID = exports.mrp_factions:getFactionIDFromName(checkFactionName)
									if factionID then
										if exports.mrp_factions:isPlayerInFaction(thePlayer, factionID) then
											countOr = countOr + 1
										end
									end
								end
						...
						elseif textFunction == "FL" or textFunction == "FACTIONLEADER" then --check for faction leadership
							if theItem[2] then --if a value is specified
								...
								if isNumeric(checkFaction[1]) then --if it is a number, we're checking for faction ID
									if isNumeric(checkFaction[2]) then --if faction rank is specified and is a number (valid)
										local isMember, rank, isLeader = exports.mrp_factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]), tonumber(checkFaction[2]))
										if isMember and isLeader then
											countOr = countOr + 1
										end
									else
										local isMember, rank, isLeader = exports.mrp_factions:isPlayerInFaction(thePlayer, tonumber(checkFaction[1]))
										if isMember and isLeader then
											countOr = countOr + 1
										end
									end
									...
									local factionID = exports.mrp_factions:getFactionIDFromName(checkFactionName)
									if factionID then
										local isMember, rank, isLeader = exports.mrp_factions:isPlayerInFaction(thePlayer, factionID)
										if isMember and isLeader then
											countOr = countOr + 1
										end
									end
