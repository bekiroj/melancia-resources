...
function triggerGate(password)
	...
	if reachedit then
		...
		if not (isGateBusy) then
			if (canPlayerControlGate(source, client, password)) or exports.mrp_global:isStaffOnDuty(client) then
				moveGate(source)
			end
		end
	end
...
function moveGate(theGate, secondtime)
	...
	if not (isGateBusy) or (secondtime) then
		exports.mrp_anticheat:changeProtectedElementDataEx(theGate, "gate:busy", true, false)
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(theGate, "gate:parameters", gateParameters, false)
	end
end
...
function resetBusyState(theGate)
	local isGateBusy = getElementData(theGate, "gate:busy")
	if (isGateBusy) then
		exports.mrp_anticheat:changeProtectedElementDataEx(theGate, "gate:busy", false, false)
	end
...
function canPlayerControlGate(theGate, thePlayer, password)
	...
	elseif gateProtection == 3 then
		...
		for _, itemID in ipairs(tempAccess) do
			if (exports.mrp_global:hasItem(thePlayer, tonumber(itemID))) then
				return true
			end
		end
	...
	elseif gateProtection == 4 then
		local tempAccess = split(gateParameters["gateSecurityParameters"], " ")
		local hasItem, slotID, itemValue, databaseID = exports.mrp_global:hasItem(thePlayer, tonumber(tempAccess[1]))
		if (hasItem) then
	...
	elseif gateProtection == 9 then --If player has access to the given vehicle
		...
		for _, vehID in ipairs(tempAccess) do
			...
			if veh then
				if(exports.mrp_global:isAdminOnDuty(thePlayer) or exports.mrp_global:hasItem(thePlayer, 3, tonumber(vehID)) or (getElementData(veh, "faction") > 0 and isPlayerInFaction(thePlayer, getElementData(veh, "faction"))) ) then
					return true
				end
			end
		end		
	...
	elseif gateProtection == 10 then --keycard
		...
		if(exports.mrp_global:hasItem(thePlayer, keycardItemID, gateParameters["gateSecurityParameters"])) then
			return true
		end
	else
