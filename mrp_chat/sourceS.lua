		...
		if pa2 == nil and p1 ~= nil then
			if p1 <= 100 and p1 >=0 then
				if math.random(100) >= p1 then
					exports.mrp_global:sendLocalText(thePlayer, "((OOC Şans - %"..p1..")) "..getPlayerName(thePlayer):gsub("_", " ").." isimli kişinin denemesi başarısız oldu.", 255, 51, 102, 30, {}, true)
				else
					exports.mrp_global:sendLocalText(thePlayer, "((OOC Şans - %"..p1..")) "..getPlayerName(thePlayer):gsub("_", " ").." isimli kişinin denemesi başarılı oldu.", 255, 51, 102, 30, {}, true)
				end
			else
...
function oocCoin(thePlayer)
	if  math.random( 1, 2 ) == 2 then
		exports.mrp_global:sendLocalText(thePlayer, " ((OOC Yazı Tura)) " .. getPlayerName(thePlayer):gsub("_", " ") .. " bir madeni para fırlatır, para yazıdır.", 255, 51, 102)
	else
		exports.mrp_global:sendLocalText(thePlayer, " ((OOC Yazı Tura)) " .. getPlayerName(thePlayer):gsub("_", " ") .. " bir madeni para fırlatır, para turadır.", 255, 51, 102)
	end
end
...
function sendActionNearClients(root, message, type)
	...
	if getElementType(root) == "player" and exports['mrp_freecam']:isPlayerFreecamEnabled(root) then return end
...
function getPlayerMaskState(player)
	local masks = exports["mrp_items"]:getMasks()
...
function localIC(source, message)
	if exports['mrp_freecam']:isPlayerFreecamEnabled(source) then return end
	...
	if message == ":)" then
		exports.mrp_global:sendLocalMeAction(source, "gülümser.")
		return
	elseif message == ":D" then
		exports.mrp_global:sendLocalMeAction(source, "kahkaha atar.")
		return
	elseif message == ";)" then
		exports.mrp_global:sendLocalMeAction(source, "göz kırpar.")
		return
	elseif message == "O.o" then
		exports.mrp_global:sendLocalMeAction(source, "sol kaşını havaya kaldırır.")
		return
	elseif message == "O.O" then
		exports.mrp_global:sendLocalMeAction(source, "sağ kaşını havaya kaldırır.")
		return
	elseif message == "X.x" then
		exports.mrp_global:sendLocalMeAction(source, "gözlerini kapatır.")
		return
	elseif message == ":(" then
		exports.mrp_global:sendLocalDoAction(source, "Yüzünde üzgün bir ifade oluştuğu görülebilir.")
		return	
	end

	local playerVehicle = getPedOccupiedVehicle(source)
	if playerVehicle then
		if (exports['mrp_vehicle']:isVehicleWindowUp(playerVehicle)) then
		...
		if dist < 20 then
			...
			if (nearbyPlayerDimension==dimension) and (nearbyPlayerInterior==interior) then
				...
				message2 = message
					...
					if pveh then
						if (exports['mrp_vehicle']:isVehicleWindowUp(pveh)) then
							...
							exports['mrp_freecam']:add(affectedElements)
							return
						end
					end

					if nbpveh and exports['mrp_vehicle']:isVehicleWindowUp(nbpveh) == true then
...
function meEmote(source, cmd, ...)
	...
	if logged == 1 then
		...
		if not (...) then
			...
			local result, affectedPlayers = exports.mrp_global:sendLocalMeAction(source, message, true, true)
...
function outputChatBoxCar( vehicle, target, text1, text2, color )
	if vehicle and exports['mrp_vehicle']:isVehicleWindowUp( vehicle ) then
...
function radio(source, radioID, message)
	...
	local hasRadio, itemKey, itemValue, itemID = exports.mrp_global:hasItem(source, 6)
	if hasRadio or getElementType(source) == "ped" or radioID == -2 then
		local theChannel = itemValue
		if radioID < 0 then
			theChannel = radioID
		elseif radioID == 1 and exports.mrp_integration:isPlayerTrialAdmin(source) and tonumber(message) and tonumber(message) >= 1 and tonumber(message) <= 10 then
			return
		elseif radioID ~= 1 then
			local count = 0
			local items = exports['mrp_items']:getItems(source)
		...
		elseif theChannel > 1 or radioID < 0 then
			...
			elseif radioID == -2 then
				local a = {}
				for key, value in ipairs(exports.mrp_sfia:getPlayersInAircraft( )) do
					table.insert(affectedElements, value)
					a[value] = true
				end
			...
			elseif radioID == -3 then --PA (speakers) in vehicles and interiors // Exciter
				...
				if(outputDim > 0) then
					...
					if(outputDim > 20000) then --vehicle interior
						...
						if not vehicle then
							for k,v in ipairs(exports.mrp_pool:getPoolElementsByType("vehicle")) do
						...
						if vehicle then
							canUsePA = getElementData(source, "adminduty") == 1 or exports.mrp_global:hasItem(source, 3, tonumber(dbid)) or (getElementData(source, "faction") > 0 and getElementData(source, "faction") == getElementData(vehicle, "faction"))
						end
					else
						canUsePA = getElementData(source, "adminduty") == 1 or exports.mrp_global:hasItem(source, 4, outputDim) or exports.mrp_global:hasItem(source, 5,outputDim)
					end
					...
					for key, value in ipairs(exports.mrp_pool:getPoolElementsByType("player")) do
			...
			elseif radioID == -4 then --PA (speakers) at airports // Exciter
				...
				for k,v in ipairs(allowedFactions) do
					if exports.mrp_factions:isPlayerInFaction(source, v) then
				...
				for key, value in ipairs(getElementsByType( "player" )) do
					if exports.mrp_global:hasItem(value, 6, theChannel) then
						local isRestricted, factionID = isThisFreqRestricted(theChannel)
			...
			for key, value in ipairs(affectedElements) do
				...
				if value ~= source then
					...
					--if not exports.mrp_global:hasItem(value, 88) == false then  ***Earpiece Fix***
					if exports.mrp_global:hasItem(value, 88) == false then
						-- Show it to people near who can hear his radio
						for k, v in ipairs(exports.mrp_global:getNearbyElements(value, "player",7)) do
			...
			exports.mrp_logs:dbLog(source, radioID < 0 and 10 or 9, affectedElements, ( radioID < 0 and "" or ( theChannel .. " " ) ) .." "..message)
		else
...
function chatMain(message, messageType)
	if exports['mrp_freecam']:isPlayerFreecamEnabled(source) then cancelEvent() return end
...
for i = 1, 20 do
	addCommandHandler( "r" .. tostring( i ),
		function( thePlayer, commandName, ... )
			if i <= exports['mrp_items']:countItems(thePlayer, 6) then
				if (...) then
	...
	if (theTeam) then
		...
		if (teamID==1 or teamID==2 or teamID==3 or teamID==47 or teamID==59) then
			...
			if factionLeader>0 then
				...
				exports.mrp_logs:dbLog(source, 16, source, message)
				for key, value in ipairs(exports.mrp_pool:getPoolElementsByType("player")) do
...
function airradio(thePlayer, commandName, ...)
	local playersInAir = exports.mrp_sfia:getPlayersInAircraft( )
	if playersInAir then
		local found = false
		if getPlayerTeam( thePlayer ) == getTeamFromName( "Federal Aviation Administration" ) then
			for _, itemRow in ipairs(exports['mrp_items']:getItems(thePlayer)) do
				if (itemRow[1] == 6 and itemRow[2] > 0) then
					found = true
					break
				end
...
function globalOOC(thePlayer, commandName, ...)
	...
	if (logged==1) then
		if not (exports.mrp_integration:isPlayerAdminI(thePlayer)) then
		return end
		if not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Message]", thePlayer, 255, 194, 14)
		else
			local oocEnabled = exports.mrp_global:getOOCState()
			message = table.concat({...}, " ")
			local muted = getElementData(thePlayer, "muted")
			if (oocEnabled==0) and not exports.mrp_integration:isPlayerAdminII(thePlayer) and not exports.mrp_integration:isPlayerScripter(thePlayer) then
			...
			elseif (muted==1) then
				...
				--exports.mrp_logs:logMessage("[OOC: Global Chat] " .. playerName .. ": " .. message, 1)
					...
					if (logged==1) and (targetOOCEnabled==1) then
						...
						if exports.mrp_integration:isPlayerAdminI(thePlayer) then
							...
							if getElementData(thePlayer, "hiddenadmin") then
								outputChatBox("[OOC] #FF0000"..exports.mrp_global:getPlayerFullIdentity(thePlayer).."#CCFFFF: " .. message .. " ))", arrayPlayer, 196, 255, 255, true)
							else
								outputChatBox("[OOC] #FF0000"..exports.mrp_global:getPlayerFullIdentity(thePlayer).."#CCFFFF: " .. message .. " ))", arrayPlayer, 196, 255, 255, true)
							end
                        else
				...
				exports.mrp_logs:dbLog(thePlayer, 18, affectedElements, message)
			end
		end
	end
end
...
function playerToggleOOC(thePlayer, commandName)
	...
	if (logged==1) then
		...
		if (playerOOCEnabled==1) then
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "globalooc", 0, false)
		else
			outputChatBox("Global OOC Chat'i şimdi etkinleştirdiniz.", thePlayer, 255, 194, 14)
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "globalooc", 1, false)
		end
...
function isFriendOf(thePlayer, targetPlayer)
	return exports['mrp_social']:isFriendOf( getElementData( thePlayer, "account:id"), getElementData( targetPlayer, "account:id" ))
end
		...
		if getElementData(player, "loggedin") == 1 then
			...
			if exports.mrp_integration:isPlayerTrialAdmin(player) or getElementData(player, 'vip') >= 1 or yt == 1 then
				if not getElementData(player, "pm:off") then
					...
					outputChatBox(exports.mrp_pool:getServerSyntax(false, "e").."Özel mesajlar zaten kapalı, aktifleştirmek için /pmaç komutunu kullanın.", player, 255, 255, 255, true)
				end
			end
		end
	end
		...
		if getElementData(player, "loggedin") == 1 then
			...
			if exports.mrp_integration:isPlayerTrialAdmin(player) or getElementData(player, 'vip') >= 1 or yt == 1 then
				if getElementData(player, "pm:off") then
					...
					outputChatBox(exports.mrp_pool:getServerSyntax(false, "e").."Özel mesajlar zaten aktif, kapatmak için /pmkapat komutunu kullanın.", player, 255, 255, 255, true)
				end
			end
		end
	end
...
function ignoreOnePlayer(thePlayer, commandName, targetPlayerNick)
	...
	if (logged==1) then
		if not (targetPlayerNick) then
			...
			if exports.mrp_integration:isPlayerTrialAdmin(targetPlayer) then
...
function pmPlayer(thePlayer, commandName, who, ...)
	...
	if who and message and getElementData(thePlayer, "loggedin") == 1 then
		...
		if (isElement(who)) then
			...
			targetPlayer, targetPlayerName = exports.mrp_global:findPlayerByPartialNick(thePlayer, who)
		end

		if (targetPlayer) then

			if getElementData(targetPlayer, "pm:off") and not exports.mrp_integration:isPlayerAdminI(thePlayer) then
			...
			if not exports.mrp_integration:isPlayerTrialAdmin(targetPlayer) and not exports.mrp_integration:isPlayerScripter(targetPlayer) then
				username = ""
			end

			if not exports.mrp_integration:isPlayerTrialAdmin(thePlayer) and not exports.mrp_integration:isPlayerScripter(thePlayer) then
				targetUsername = ""
			end
		
			if not exports.mrp_integration:isPlayerSeniorAdmin(thePlayer) and not exports.mrp_integration:isPlayerSeniorAdmin(targetPlayer) then
				...
				for k,v in ipairs(advertisementMessages) do
					...
					if (found) or (found2) or (string.lower(message)==tostring(v)) then
						exports.mrp_global:sendMessageToAdmins("AdmWrn: " .. tostring(playerName) .. " sent a possible advertisement PM to " .. tostring(targetPlayerName) .. ".")
						exports.mrp_global:sendMessageToAdmins("AdmWrn: Message: " .. tostring(message))
						break
					end
				end
			end
			...
			local url = exports.mrp_global:getUrlFromString(message)
			local received = {}
...
function localOOC(thePlayer, commandName, ...)
	if exports['mrp_freecam']:isPlayerFreecamEnabled(thePlayer) then return end
	...
	if (logged==1) and not (isPedDead(thePlayer)) then
		...
		elseif (muted==1) then
			...
			if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) and getElementData(thePlayer, "duty_admin") == 1 and getElementData(thePlayer, "hiddenadmin") == 0 and not getElementData(thePlayer, "supervising") then
				r,b,g = 96, 255, 255
				setElementData(thePlayer, "supervisorBchat", false)
			elseif exports.mrp_integration:isPlayerTrialAdmin(thePlayer) and getElementData(thePlayer, "duty_admin") == 1 and getElementData(thePlayer, "hiddenadmin") == 0 and getElementData(thePlayer, "supervising") then
				r,b,g = 96, 255, 255
				setElementData(thePlayer, "supervisorBchat", true)
			elseif exports.mrp_integration:isPlayerSupporter(thePlayer) and getElementData(thePlayer, "supervising") then
				r,b,g = 96, 255, 255
				setElementData(thePlayer, "supervisorBchat", true)
			elseif exports.mrp_integration:isPlayerSupporter(thePlayer) and not getElementData(thePlayer, "supervising") then
				r,b,g = 96, 255, 255
				setElementData(thePlayer, "supervisorBchat", false)
			end
			local message = table.concat({...}, " ")
			...
			if getElementData(thePlayer, "supervisorBchat") == false or nil then -- The below locals were contained in the if, else statements. Therefore returned nil to the export db //Chaos
				...
				if (getElementData(thePlayer, "duty_admin") == 1) then
					result, affectedElements = exports.mrp_global:sendLocalText(thePlayer, "#ccffff[OOC]#EE0C0C " .. ismi .. ":#ccffff (( " .. message .. " ))", r,b,g)
				else
					result, affectedElements = exports.mrp_global:sendLocalText(thePlayer, "#ccffff[OOC]#ccffff " .. ismi .. ": (( " .. message .. " ))", r,b,g)
				end
				
			else
				result, affectedElements = exports.mrp_global:sendLocalText(thePlayer, "#ccffff[OOC]#ccffff " .. exports.mrp_global:getPlayerFullIdentity(thePlayer) .. ": (( " .. message .. " ))", r,b,g,20,nil,true)
			end
			exports.mrp_logs:dbLog(thePlayer, 8, affectedElements, message)
		end
...
function districtIC(thePlayer, commandName, ...)
	...
	if (logged==1) and not (isPedDead(thePlayer)) then
		if not (...) then
			...
			local playerName = getPlayerName(thePlayer)
			local message = table.concat({...}, " ")
			local zonename = exports.mrp_global:getElementZoneName(thePlayer)
			local x, y = getElementPosition(thePlayer)

			for key, value in ipairs(exports.mrp_pool:getPoolElementsByType("player")) do
				local playerzone = exports.mrp_global:getElementZoneName(value)
			...
			exports.mrp_logs:dbLog(thePlayer, 13, affectedElements, message)
		end
	end
end
...
function localDo(thePlayer, commandName, ...)
	...
	if logged==1 then
		if not (...) then
			...
			local message = table.concat({...}, " ")
			--exports.mrp_logs:logMessage("[IC: Local Do] * " .. message .. " *      ((" .. getPlayerName(thePlayer) .. "))", 19)
			local result, affectedElements = exports.mrp_global:sendLocalDoAction(thePlayer, message, true)
			exports.mrp_logs:dbLog(thePlayer, 14, affectedElements, message)
...
function megaphoneShout(thePlayer, commandName, ...)
	...
	if not (isPedDead(thePlayer)) and (logged==1) then
		...
		if (factiontype==2) or (factiontype==3) or (factiontype==4) or (exports.mrp_global:hasItem(thePlayer, 141)) or ( isElement(vehicle) and exports.mrp_global:hasItem(vehicle, 141) and (seat==1 or seat==0)) then
			local affectedElements = { }
...
function toggleFactionSelf(thePlayer, commandName)
	...
	if(logged==1) then
		...
		if (factionBlocked==1) then
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "chat:blockF", 0, false)
			outputChatBox("Grup sohbeti artık kendiniz için etkin.", thePlayer, 0, 255, 0)
		else
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "chat:blockF", 1, false)
	...
	if (logged==1) then
		if not (...) then
			...
			if not(theTeam) or (theTeamName=="Citizen") then
				...
				--exports.mrp_logs:logMessage("[OOC: " .. theTeamName .. "] " .. playerName .. ": " .. message, 6)
				...
				exports.mrp_logs:dbLog(thePlayer, 11, affectedElements, message)
			end
		end
	end
end
...
function sfpdHq(thePlayer, commandName, ...)
	...
	if (factionType == 2) then
		...
		if not exports.mrp_factions:isPlayerFactionLeader(thePlayer, factionID) then
			outputChatBox("Bu komutu kullanma izniniz yok.", thePlayer, 255, 0, 0)
	...
	if (logged==1) then
		if not (...) then
			...
			elseif tonumber(playerLeader) ~= 1 then
				...
				exports.mrp_logs:dbLog(thePlayer, 11, affectedElements, "Leader: " .. message)
			end
		end
	end
end
...
function togGovOOCSelf(thePlayer, theCommand)
	...
	if (getTeamName(team) == "Los Santos Fire Department") or (getTeamName(team) == "İstanbul Emniyet Müdürlüğü") or (getTeamName(team) == "İstanbul Büyükşehir Belediyesi") or (getTeamName(team) == "San Andreas Highway Patrol") or (getTeamName(team) == "Superior Court of San Andreas") or (getTeamName(team) == "Federal Aviation Administration") and (logged==1) then
...
function govooc(thePlayer, commandName, ...)
	...
	if (getTeamName(team) == "Los Santos Fire Department") or (getTeamName(team) == "İstanbul Emniyet Müdürlüğü") or (getTeamName(team) == "İstanbul Büyükşehir Belediyesi") or (getTeamName(team) == "San Andreas Highway Patrol") or (getTeamName(team) == "Superior Court of San Andreas") or (getTeamName(team) == "Federal Aviation Administration") and (logged==1) then
		...
		if not (...) then
			...
			exports.mrp_logs:dbLog(thePlayer, 11, affectedElements, "GOV OOC: " .. message)
		end
	end
end
...
function setRadioChannel(thePlayer, commandName, slot, channel)
	...
	if not (channel) then
		...
		if (exports.mrp_global:hasItem(thePlayer, 6)) then
			local count = 0
			local items = exports['mrp_items']:getItems(thePlayer)
			for k, v in ipairs( items ) do
				if v[1] == 6 then
					count = count + 1
					if count == slot then
						if v[2] > 0 then
							...
							if channel > 1 and channel < 1000000000 and (not isRestricted or (tonumber(playerFaction) == tonumber(factionID) ) )then
								if exports['mrp_items']:updateItemValue(thePlayer, k, channel) then
		...
		for k, v in ipairs( items ) do
			if v[1] == 6 then
				if slot then
					count = count + 1
					if count == slot then
						exports['mrp_items']:updateItemValue(thePlayer, k, ( titemValue < 0 and 1 or -1 ) * math.abs( v[2] or 1))
						break
					end
				else
					exports['mrp_items']:updateItemValue(thePlayer, k, ( titemValue < 0 and 1 or -1 ) * math.abs( v[2] or 1))
...
function leadAdminChat(thePlayer, commandName, ...)
	...
	if(logged==1) and (exports.mrp_integration:isPlayerSeniorAdmin(thePlayer)) then
		if not (...) then
			...
			for k, arrayPlayer in ipairs(players) do
				local logged = getElementData(arrayPlayer, "loggedin")
				if (exports.mrp_integration:isPlayerSeniorAdmin(arrayPlayer)) and (logged==1) then
					table.insert(affectedElements, arrayPlayer)
					outputChatBox("[UAT] ("..playerid..") " ..adminTitle .. " "..accountName.. ": " .. message, arrayPlayer, 204, 102, 255)
				end
			end
			exports.mrp_logs:dbLog(thePlayer, 2, affectedElements, message)
		end
	end
end
...
function gmChat(thePlayer, commandName, ...)
	...
	if(logged==1) and (exports.mrp_integration:isPlayerTrialAdmin(thePlayer)  or exports.mrp_integration:isPlayerSupporter(thePlayer)) then
		if not (...) then
			outputChatBox("SYNTAX: /".. commandName .. " [Message]", thePlayer, 255, 194, 14)
		else
			...
			for k, arrayPlayer in ipairs(players) do
				local logged = getElementData(arrayPlayer, "loggedin")
				if logged==1 and (exports.mrp_integration:isPlayerTrialAdmin(arrayPlayer) or exports.mrp_integration:isPlayerSupporter(arrayPlayer)) then
			...
			exports.mrp_logs:dbLog(thePlayer, 24, affectedElements, message)
		end
	end
end
...
function toggleOOC(thePlayer, commandName)
	...
	if(logged==1) and (exports.mrp_integration:isPlayerTrialAdmin(thePlayer)) then
		local players = exports.mrp_pool:getPoolElementsByType("player")
		local oocEnabled = exports.mrp_global:getOOCState()
		if (commandName == "togooc") then
			if (oocEnabled==0) then
				exports.mrp_global:setOOCState(1)
			...
			elseif (oocEnabled==1) then
				exports.mrp_global:setOOCState(0)
		...
		if not (targetPlayerNick) or not (amount) or not tonumber(amount) then
			...
			if targetPlayer then
				...
				if (distance<=10) then
					...
					elseif (hoursplayed<5) and (amount>50) and not exports.mrp_integration:isPlayerTrialAdmin(thePlayer) and not exports.mrp_integration:isPlayerTrialAdmin(targetPlayer) and not exports.mrp_integration:isPlayerSupporter(thePlayer) and not exports.mrp_integration:isPlayerSupporter(targetPlayer) then
						outputChatBox("50 $ 'dan fazla transfer yapmadan 5 saat önce en az 5 saat oynamalısınız.", thePlayer, 255, 0, 0)
					elseif exports.mrp_global:hasMoney(thePlayer, amount) then
						if hoursplayed < 5 and not exports.mrp_integration:isPlayerTrialAdmin(targetPlayer) and not exports.mrp_integration:isPlayerTrialAdmin(thePlayer) and not exports.mrp_integration:isPlayerSupporter(targetPlayer) and not exports.mrp_integration:isPlayerSupporter(thePlayer) then
							...
							exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "payAmount", totalAmount, false)
							setTimer(
								function(thePlayer, amount)
									if isElement(thePlayer) then
										local totalAmount = ( getElementData(thePlayer, "payAmount") or 0 ) - amount
										exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "payAmount", totalAmount <= 0 and false or totalAmount, false)
									end
								end,
								300000, 1, thePlayer, amount
							)
						end
						--exports.mrp_logs:logMessage("[Money Transfer From " .. getPlayerName(thePlayer) .. " To: " .. targetPlayerName .. "] Value: " .. amount .. "$", 5)
						exports.mrp_logs:dbLog(thePlayer, 25, targetPlayer, "PAY " .. amount)

						if (hoursplayed<5) then
							exports.mrp_global:sendMessageToAdmins("AdmWarn: New Player '" .. getPlayerName(thePlayer) .. "' transferred $" .. exports.mrp_global:formatMoney(amount) .. " to '" .. targetPlayerName .. "'.")
						end

						-- DEAL!
						exports.mrp_global:takeMoney(thePlayer, amount)
						exports.mrp_global:giveMoney(targetPlayer, amount)
						...
						outputChatBox("" .. exports.mrp_global:formatMoney(amount) .. " para verdin. " .. targetPlayerName .. " kişisine.", thePlayer)
						outputChatBox(getPlayerName(thePlayer) .. " , " .. exports.mrp_global:formatMoney(amount) .. " para verdi.", targetPlayer)
...
function removeAnimation(thePlayer)
	exports.mrp_global:removeAnimation(thePlayer)
end
		...
		if not (targetPlayerNick) or not (...) then
			...
			if targetPlayer then
				...
				if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)<3) then
					...
					local message = table.concat({...}, " ")
					--exports.mrp_logs:logMessage("[IC: Whisper] " .. name .. " to " .. targetPlayerName .. ": " .. message, 1)
					exports.mrp_logs:dbLog(thePlayer, 21, targetPlayer, message)
...
function localClose(thePlayer, commandName, ...)
	...
	if (logged==1) then
		if not (...) then
			...
			local name = getPlayerName(thePlayer)
			local message = table.concat({...}, " ")
			--exports.mrp_logs:logMessage("[IC: Whisper] " .. name .. ": " .. message, 1)
			message = trunklateText( thePlayer, message )
			...
			for index, targetPlayers in ipairs( getElementsByType( "player" ) ) do
				if getElementDistance( thePlayer, targetPlayers ) < 3 then
					...
					if playerCar then
						if not exports['mrp_vehicle']:isVehicleWindowUp(playerCar) then
							if pveh then
								...
								elseif not (exports['mrp_vehicle']:isVehicleWindowUp(pveh)) then
									table.insert(affectedElements, targetPlayers)
									outputChatBox(name .. " fısıldıyor: " .. message2, targetPlayers, r, g, b)
									--icChatsToVoice(targetPlayers, message2, thePlayer)
								end
	...
	if (logged==1) then
		...
		if(factionType==6)then -- news faction
			if not (targetPartialPlayer) then
				...
				local targetPlayer, targetPlayerName = exports.mrp_global:findPlayerByPartialNick(thePlayer, targetPartialPlayer)
				if targetPlayer then
					...
					if (targetLogged==1) then
						if(getElementData(targetPlayer,"interview"))then
							...
							exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, "interview", true, false)
							local playerName = getPlayerName(thePlayer)
				...
				if targetPlayer then
					...
					if (targetLogged==1) then
						if not(getElementData(targetPlayer,"interview"))then
							...
							exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, "interview", false, false)
							local playerName = getPlayerName(thePlayer)
	...
	if (logged==1) then
		if(getElementData(thePlayer, "interview"))then
			if not(...)then
				...
				local finalmessage = "[TRT] Röportaj Misafir " .. name .." : ".. message
				local theTeam = getPlayerTeam(thePlayer)
				local factionType = getElementData(theTeam, "type")
				if(factionType==6)then -- news faction
					finalmessage = "[TRT] " .. name .." : ".. message
				end
				...
				exports.mrp_logs:dbLog(thePlayer, 23, thePlayer, "NEWS " .. message)
				exports.mrp_global:giveMoney(getTeamFromName("TRT Haber"), 200)
			end
		end
	end
end
...
function charityCash(thePlayer, commandName, amount)
	if not (amount) then
		...
		if (donation<=0) then
			...
			if not exports.mrp_global:takeMoney(thePlayer, donation) then
				outputChatBox("Çıkarılacak o kadar paran yok.", thePlayer, 255, 0, 0)
			else
				outputChatBox("$".. exports.mrp_global:formatMoney(donation) .." değerinde bağış yaptın.", thePlayer, 0, 255, 0)
				exports.mrp_global:sendMessageToAdmins("AdmWrn: " ..getPlayerName(thePlayer).. " bağış yaptı $" ..exports.global:formatMoney(donation))
				exports.mrp_logs:dbLog(thePlayer, 25, thePlayer, "CHARITY $" .. amount)
			end
		end
	end
end
...
function bigEars(thePlayer, commandName, targetPlayerNick)
	if exports.mrp_integration:isPlayerAdmin(thePlayer) then
		...
		elseif current and not targetPlayerNick then
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "bigears", false, false)
			...
			if targetPlayer then
				...
				exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "BIGEARS "..targetPlayerName)
				exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "bigears", targetPlayer, false)
			end
		end
	end
end
...
function removeBigEars()
	for key, value in pairs( getElementsByType( "player" ) ) do
		if isElement( value ) and getElementData( value, "bigears" ) == source then
			exports.mrp_anticheat:changeProtectedElementDataEx( value, "bigears", false, false )
...
function bigEarsFaction(thePlayer, commandName, factionID)
	if exports.mrp_integration:isPlayerAdmin(thePlayer) then
		...
		elseif current and not factionID then
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "bigearsfaction", false, false)
			outputChatBox("Büyük Kulaklar kapalı.", thePlayer, 255, 0, 0)
		else
			local team = exports.mrp_pool:getElement("team", factionID)
			if not team then
				outputChatBox("Böyle bir birlik bulunamadı.", thePlayer, 255, 0, 0)
			else
				outputChatBox("" .. getTeamName(team) .. " şimdi dinle.", thePlayer, 0, 255, 0)
				exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "bigearsfaction", team, false)
			end
		end
	end
end
...
function focus(thePlayer, commandName, targetPlayer, r, g, b)
	...
	if targetPlayer then
		...
		if targetPlayer then
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "focus", focus, false)
		end
	else
		if type(focus) == "table" then
		...
		for k, v in ipairs( getElementsByType( "player" ) ) do
			if v ~= source then
				...
				if focus and focus[source] then
					focus[source] = nil
					exports.mrp_anticheat:changeProtectedElementDataEx(v, "focus", focus, false)
				end
			end
		end
	end
...
function isPlayerStaff(thePlayer)
	--if exports.mrp_integration:isPlayerSupporter(thePlayer) then return true end
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) then return true end
	if exports.mrp_integration:isPlayerScripter(thePlayer) then return true end
			...
			for k, arrayPlayer in ipairs(players) do
				...
				if logged==1 and isPlayerStaff(arrayPlayer) then
					...
					if not getElementData(arrayPlayer, "achat:of") then
						outputChatBox("[ADM] ("..getElementData(thePlayer, "playerid")..") "..exports.mrp_global:getPlayerFullIdentity(thePlayer).." ("..getElementData(thePlayer, "account:username")..") : "..message.." ", arrayPlayer, 51, 225, 102, true)
					end
				end
			end
			exports.mrp_logs:dbLog(thePlayer, 4, affectedElements, "Staff chat - Msg: "..message)
		end
	end
end
...
function businessOOC(thePlayer, commandName, business, ...)
	...
	if (logged==1) then
		if not business then
			...
			local b = exports.mrp_business:getPlayerBusinesses( thePlayer ) or { }
			local b = b[ business ]
			if b then
				local affectedElements = { }
				...
				for index, arrayPlayer in ipairs( getElementsByType( "player" ) ) do
					if isElement( arrayPlayer ) then
						if getElementData( arrayPlayer, "bigearsbusiness" ) == b then
							outputChatBox("((" .. exports.mrp_business:getBusinessName( b ) .. ")) " .. playerName .. ": " .. message, arrayPlayer, 3, 157, 157)
						elseif exports.mrp_business:isPlayerInBusiness( arrayPlayer, b ) and getElementData(arrayPlayer, "loggedin") == 1 and getElementData(arrayPlayer, "chat:blockB") ~= 1 then
							table.insert(affectedElements, arrayPlayer)
							outputChatBox("((" .. exports.mrp_business:getBusinessName( b ) .. ")) " .. playerName .. ": " .. message, arrayPlayer, 255, 150, 255)
						end
					end
				end
				exports.mrp_logs:dbLog(thePlayer, 41, affectedElements, message)
			else
...
function toggleBusinessSelf(thePlayer, commandName)
	...
	if(logged==1) then
		...
		if (BusinessBlocked==1) then
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "chat:blockB", 0, false)
			outputChatBox("İş sohbeti artık kendiniz için etkin.", thePlayer, 0, 255, 0)
		else
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "chat:blockB", 1, false)
...
addCommandHandler("mute",
	function(player, cmd, targetPlayerNick, minute, ...)
		if getElementData(player, "loggedin") == 1 and exports.mrp_integration:isPlayerTrialAdmin(player) then
			if not targetPlayerNick or minute or (...) then
				...
				local targetPlayer, targetPlayerName = exports.mrp_global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
				if targetPlayer then
					exports.mrp_global:sendMessageToAdmins("AdmWrn: "..getElementData(player, "account:username").." adlı yetkili "..getPlayerName(targetPlayer):gsub("_", " ").." adlı oyuncuyu "..minutes.." dakika susturdu.")
					exports.mrp_global:sendMessageToAdmins("AdmWrn: Sebep: "..reason)
...
function oocDene(thePlayer)
	...
	if  math.random( 1, 2 ) == 2 then
		exports.mrp_global:sendLocalText(thePlayer, "* #00ff00Başarılı. #89be78(( " .. ismi .. " ))", 137, 190, 120)
	else
		exports.mrp_global:sendLocalText(thePlayer, "* #ff0000Başarısız. #89be78(( " .. ismi .. " ))", 137, 190, 120)
	end
end
...
function zarat(thePlayer)
	if getElementData(thePlayer, "loggedin") == 1 then
		local ismi = getElementData(thePlayer, "fakename") or getPlayerName(thePlayer)
		exports.mrp_global:sendLocalText(thePlayer, "Zar Denemesi: #ff0000"..math.random(1,9)..math.random(1,9).." #89be78(( " .. ismi .. " ))", 137, 190, 120)
	end
end
addCommandHandler("zarat",zarat)

function CevreIC(thePlayer, cmd, ...)
	if not exports.mrp_integration:isPlayerTrialAdmin( thePlayer) then
		return
	end
	if not (...) then
...
function ame(...)
	if (...) then
		exports.mrp_global:sendLocalMeAction(source, ""..(...).."")
	end
end
...
function ado(...)
	if (...) then
		exports.mrp_global:sendLocalDoAction(source, ""..(...).."")
	end
end
