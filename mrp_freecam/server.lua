...
function asyncActivateFreecam ()
	if not isEnabled(source) then
		...
		if not exports.mrp_integration:isPlayerTrialAdmin(source) then
			exports.mrp_global:sendMessageToAdmins("[FREECAM] "..exports.mrp_global:getAdminTitle1(source).." has activated temporary /freecam.")
		end
		setElementData(source, "freecam:state", true, false)
		exports.mrp_logs:dbLog(source, 4, {source}, "FREECAM")
	end
end
...
function tv(player)
	local hasTV = exports.mrp_global:hasItem(player, 104, 1)
				...
				if moveCamera(posX, posY, posZ + 1, posInt, posDim) then
					for k, v in ipairs( getElementsByType( "player" ) ) do
						if getElementData(v, "faction") == 20 then
							outputChatBox("[TRT] ".. getPlayerName(player):gsub("_", " ") .. " kamera pozisyonunu degistirdi.", v, 200, 100, 200)
						end
					end
				else
	...
	function(player)
		if getElementData(player, "faction") == 20 then
			if not isTVRunning() then
				outputChatBox("[TRT] " .. getPlayerName(player):gsub("_", " ") .. " televizyon yayinini baslatti (( /tv yazarak izle ))", getRootElement( ), 200, 100, 200)
				exports.mrp_logs:dbLog(player, 23, player, "TV START")
				watching = 0
				earnings = 0
...
addCommandHandler("endtv",
	function(player)
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				setElementDimension(marker, 65535)
				outputChatBox("[TRT] " .. getPlayerName(player):gsub("_", " ") .. " televizyon yayinini durdurdu!.", getRootElement( ), 200, 100, 200)
				...
				for k, v in ipairs( getElementsByType( "player" ) ) do
					...
					if getElementData(v, "faction") == 20 then
						outputChatBox("[TRT] Max. İzleyiciler: " .. watching .. ", Bağışlar: $" .. exports.mrp_global:formatMoney(earnings), v, 200, 100, 200)
					end
				end
				
				exports.mrp_logs:dbLog(player, 23, player, "TRT STOP - VIEWERS: " .. watching .. " EARNINGS: " .. earnings)
			else
	...
	function(player)
		if getElementData(player, "faction") == 20 then
			if isTVRunning() then
				...
				outputChatBox("[TRT] İzleyiciler: " .. count .. " televizyonda.", player, 200, 100, 200)
				outputChatBox("[TRT] Max. İzleyiciler: " .. watching .. ", Kazanç: $" .. exports.mrp_global:formatMoney(earnings), player, 200, 100, 200)
			else
...
function add( affectedPlayers )
	...
	if isTVRunning() and shownto > 0 then
		...
		exports.mrp_global:giveMoney(getTeamFromName"TRT Haber", LSN_MONEY * shownto)
		--exports.mrp_logs:dbLog(player, 23, player, "TV $" .. ( 10 * shownto ) .. " " .. message)
	else
		--exports.mrp_logs:dbLog(player, 23, player, "TV OFF " .. message)
	end
end