...
local fonte = dxCreateFont ( ":mrp_fonts/components/Cargo_Two_SF.ttf" , 18 )
local fontElement = dxCreateFont(":mrp_fonts/components/FontAwesome.ttf", 9)
local noPlateVehs = {
	[481] = "BMX",
	[509] = "Bike",
	[510] = "Mountain Bike",
...
function showNearbyVehicleDescriptions()
	...
	if enableOverlayDescription ~= "0" and enableOverlayDescriptionVeh ~= "0" then
		...
		if not showing then
			for index, nearbyVehicle in ipairs( exports.mrp_global:getNearbyElements(getLocalPlayer(), "vehicle") ) do
			...
			if getDistanceBetweenPoints3D(cx,cy,cz,x,y,z) <= viewDistance then --Within radius viewDistance
				...
				if px and isLineOfSightClear(cx, cy, cz, x, y, z, true, false, false, true, true, false, false) then				
					...
					if getElementData(theVehicle, "carshop") then
						...
						toBeShowed = toBeShowed.."Fiyat: "..exports.mrp_global:formatMoney(price).."$\n Vergi: "..exports.mrp_global:formatMoney(taxes) .. "$"
						lines = lines+ 1
					else
						...
						--GET IMPOUND
						if (exports["mrp_vehicle"]:isVehicleImpounded(theVehicle)) then
						...
						if vowner == getElementData(localPlayer, "account:id") or exports.mrp_global:isStaffOnDuty(localPlayer) or exports.mrp_integration:isPlayerScripter(localPlayer) or exports.mrp_integration:isPlayerVCTMember(localPlayer) then
							toBeShowed = toBeShowed.."\nListe ID: "..(getElementData(theVehicle, "vehicle_shop_id") or "None")
							lines = lines + 1

							local ownerName = 'Devlet'
							if vowner > 0 then
								ownerName = exports.mrp_cache:getCharacterNameFromID(vowner)
							elseif vfaction > 0 then
								ownerName = exports.mrp_cache:getFactionNameFromId(vfaction)
							end
...
--Activity / MAXIME
							...
							if vowner > 0 then 
								local protected, details = true, true, true--exports['mrp_vehicle']:isProtected(theVehicle) 
					            if protected then
					                ...
					                local active, details2, secs = true, true, true --exports['mrp_vehicle']:isActive(theVehicle)
					                if active and (powner == getElementData(localPlayer, "dbid") or exports.mrp_integration:isPlayerStaff(localPlayer)) then
					...
					local renk = tocolor(0, 0, 0, 200) -- rectangle renkleri
						if getElementData(theVehicle, "carshop") then
						exports.mrp_draw:gradient(px - 7.5, py+marg - 100,(recuzunluk + marg),65,0,0,0,255,false,true)
						exports.mrp_draw:gradient(px - 7.5, py+marg - 100,(recuzunluk + marg),24+(fontHeight+marg-7),0,0,0,255,false,true)
						dxDrawText(vehicleBrand, px + 5.5, py+marg-95, px +fontWidth+ 5.5, (py + 2000), textColor2, 1, fontElement, "center") -- burası
						else
						exports.mrp_draw:gradient(px - 7.5, py+marg - 100,(recuzunluk + marg),24,0,0,0,255,false,true)
						exports.mrp_draw:gradient(px - 7.5, py+marg - 100,(recuzunluk + marg),24+(fontHeight+marg-7),0,0,0,255,false,true)
						dxDrawText(vehicleBrand, px + 5.5, py+marg-95, px +fontWidth+ 5.5, (py + 2000), textColor2, 1, fontElement, "center") -- burası
						end
...
function showEditDescription()
	if not exports.mrp_integration:isPlayerTrialAdmin(getLocalPlayer()) then
		return
	end
	if getPedOccupiedVehicle(getLocalPlayer()) then
		...
		if exports.mrp_global:hasItem(getLocalPlayer(), 3, dbid) or exports.mrp_global:hasItem(theVehicle, 3, dbid) or getElementData(getLocalPlayer(), "faction") == factionid or exports.mrp_integration:isPlayerTrialAdmin(getLocalPlayer()) and getElementData(getLocalPlayer(), "duty_admin") == 1 then
			if dbid > 0 then
				...
				exports.mrp_hud:sendBottomNotification(getLocalPlayer(), "Vehicle Description", "You cannot set descriptions on temporary vehicles.")
			end
		else
			exports.mrp_hud:sendBottomNotification(getLocalPlayer(), "Vehicle Description", "You are not the owner of this vehicle.")
		end
	else
		exports.mrp_hud:sendBottomNotification(getLocalPlayer(), "Vehicle Description", "You must be in the vehicle you wish to change the description of.")
	end
end
