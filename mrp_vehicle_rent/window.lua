cRent = {
	...
	cRent = instance;
		if localPlayer:getData('loggedin') == 1 then
			showCursor(true)
			local font = GuiFont(':mrp_fonts/files/Roboto-Bold.ttf', 9)

			window = GuiWindow(0, 0, 850, 524, 'Araç Kiralama Noktası - Melancia', false)
			...
			preview = exports['mrp_object_preview']:createObjectPreview(previewObject, 0, 0, 180, 0.50, 0.40, 0.26, 0.20, true, true)
			...
			addEventHandler('onClientGUIScroll', scroll,
				function()
					if localPlayer:getData('rent:window') then
						...
						exports['mrp_object_preview']:setRotation(preview,0, 0, 155 + rotation * 360)
					end
			end, false)
	...
	cRent = instance;
		if localPlayer:getData('rent:window') then
			if source == close then
				...
				exports['mrp_object_preview']:destroyObjectPreview(preview)
				window:destroy()
				localPlayer:setData('rent:window', false)
				outputChatBox('[Melancia]#D0D0D0 Hmm, demek bir araba beğenemdin başka zaman tekrar gel!',195,184,116,true)
				showCursor(false)
			elseif source == buy then
				local selected = guiGridListGetSelectedItem(list)
				if selected == -1 then
					outputChatBox('[Melancia]#D0D0D0 Lütfen kiralamak istediğin aracı listeden seç!',195,184,116,true)
				else
					selected = selected + 1
					selectedTable = instance.cars[selected] or false
					if selectedTable then
