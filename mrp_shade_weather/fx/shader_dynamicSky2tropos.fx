...
PSInput VertexShaderSB(VSInput VS)
{
    ...
    float3 eyeVector = normalize(PS.WorldPos.xyz - gCameraPosition); 		 
...
float4 PixelShaderDots(PSInput PS) : COLOR0
{
    ...
    TexCoord2 = normalize(PS.TexCoord2);

    MoonDot = dot(float3(0,0,-1), TexCoord2);	
    float MoonLight = pow(max(0.0001, MoonDot), 50);
    MoonCombine = saturate(0.15 * MoonLight);	
    MoonCombine *= saturate(1 - pow(gDayTime,1.2));
    MoonCombine = saturate(MoonCombine);
	
    nightSky.rgb += MoonCombine ;
...
float4 PixelShaderClouds(PSInput PS) : COLOR0
{
    ...
    float3 normalLight = saturate(gSunColor[0] * SunLightNormal * 0.35 + MoonLightNormal * 0.9) * fadeDot;	
    normalLight *= fadeDot;
