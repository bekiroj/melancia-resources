	...
	if(isRoadblockEditAllowed(client)) then
		...
		for i = 1, maxroadblocks do
			if (roadblocksarr[i]==nil) then
				...
				exports.mrp_anticheat:changeProtectedElementDataEx(roadblocksarr[i], "roadblock", "1", false)
				...
				if (objectid == 1593) then -- Spikestrip
					...
					exports.mrp_anticheat:changeProtectedElementDataEx(shape, "type", "spikes")
					exports.mrp_anticheat:changeProtectedElementDataEx(roadblocksarr[i], "spike:shape1", shape, false)
				end
		...
		if not (slot == 0) then
			...
			exports['mrp_logs']:dbLog(client, 28, client, "Spawned Roadblock ID "..tostring(slot))
		else
	...
	if(isRoadblockEditAllowed(thePlayer)) then
		if not (id) then
			...
			if (roadblocksarr[id]==nil) then
				...
				exports['mrp_logs']:dbLog(thePlayer, 28, thePlayer, "Removed roadblock ID "..tostring(id))
...
function removeAllRoadblocks(thePlayer, commandName)
	if(exports.mrp_integration:isPlayerTrialAdmin(thePlayer)) then
		...
		roadblocksarr = { }
		exports['mrp_logs']:dbLog(thePlayer, 28, thePlayer, "Removed all roadblocks")
