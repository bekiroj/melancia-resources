local anims_list = {
	...
	['custom_6'] = {'custom/custom_6.ifp', true, price = 7, name = "Melancia Animasyon Fiziği I", engine = 1, disabled = 0}, -- Project Awesome
	['custom_7'] = {'custom/custom_7.ifp', true, price = 7, name = "Melancia Kadın Yürüme Fiziği", engine = 1, disabled = 0}, -- Female Remaked
	['custom_8'] = {'custom/custom_8.ifp', true, price = 7, name = "Çete Animasyon Fiziği II", engine = 1, disabled = 0}, -- Gang 4
	['custom_9'] = {'custom/custom_9.ifp', true, price = 6, name = "Melancia Animasyon Fiziği II", engine = 1, disabled = 0}, -- New Animations for GTA:SA
	...
	function(cmd, id, target)
		...
		if customtable[index] or (tonumber(anims_list[index].price) == 0) then
			if (tonumber(id) == 11) then
				local targetPlayer = exports.mrp_global:findPlayerByPartialNick(localPlayer, target)
...
addCommandHandler("animpanel",
	function()
		if maintance and not exports.mrp_integration:isPlayerDeveloper(localPlayer) then
		...
		if getElementData(localPlayer, "loggedin") == 1 then
			...
			window = guiCreateWindow(0, 0, 549, 335, "Melancia - Özel Animasyon Arayüzü", false)
