		...
		local rotz = getPedRotation(sourcePlayer)
			...
			for value=1,SpikeLimit,1 do
				if(Spike[value] == nil) then
					...
					exports.mrp_pool:allocateElement(Spike[value])
					...
					exports.mrp_pool:allocateElement(Shape1[value])
					Shape2[value] = createColRectangle( (nx), (ny), 5.0, 5.0 )
					exports.mrp_pool:allocateElement(Shape2[value])
					exports.mrp_anticheat:changeProtectedElementDataEx(Shape1[value], "type", "spikes")
					exports.mrp_anticheat:changeProtectedElementDataEx(Shape2[value], "type", "spikes")
					outputChatBox("Spawned spikes with ID:" .. value, sourcePlayer, 0, 194, 0)
					exports.mrp_logs:dbLog(sourcePlayer, 28, sourcePlayer, "Spawned spike ID "..tostring(value))
					break
				end
			end
		else
...
addCommandHandler("tuzakkur", ThrowSpikes)
		...
		outputChatBox("SYNTAX: /removespikes [ID]", sourcePlayer, 255, 194, 14)
			...
			if(Spike[message] ~= nil) then
				...
				if (getDistanceBetweenPoints3D(x1, y1, z1, x2, y2, z2) <= 10) then
					...
					exports.mrp_logs:dbLog(sourcePlayer, 28, sourcePlayer, "Spawned spike ID "..tostring(message))
					TotalSpikes = TotalSpikes -1
...
addCommandHandler("tuzakkaldir", RemovingSpikes)

function AdminRemovingSpikes(sourcePlayer, command)
	if	(exports.mrp_integration:isPlayerTrialAdmin(sourcePlayer)) then
		for value=1,SpikeLimit,1 do
			if(Spike[value] ~= nil) then
		...
		exports.mrp_logs:dbLog(sourcePlayer, 28, sourcePlayer, "Removed all the spawned spikestrips")
		TotalSpikes = nil
	end
end
addCommandHandler("atuzakkaldir", AdminRemovingSpikes)