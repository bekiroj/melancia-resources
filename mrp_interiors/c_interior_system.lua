...
function showIntName(name, ownerName, inttype, cost, ID, bizMsg)
	...
	elseif name then
		if (inttype==3) then -- Interior name and Owner for rented
			...
			if (exports.mrp_integration:isPlayerTrialAdmin(getLocalPlayer()) and getElementData(getLocalPlayer(), "duty_admin") == 1) or exports.mrp_global:hasItem(getLocalPlayer(), 4, ID) then
			...
			guiLabelSetHorizontalAlign(gInteriorName, "center", true)
			guiSetAlpha(gInteriorName, 0.0)
			if (exports.mrp_integration:isPlayerTrialAdmin(getLocalPlayer()) and getElementData(getLocalPlayer(), "duty_admin") == 1) or exports.mrp_global:hasItem(getLocalPlayer(), 4, ID) or exports.mrp_global:hasItem(getLocalPlayer(), 5, ID) then
		...
		if (ownerName=="None") and (inttype==3) then -- Unowned type 3 (rentable)
			gBuyMessage = guiCreateLabel(0.0, 0.915, 1.0, 0.3, "Kiralamak için E tuşuna basın "..exports.mrp_pool:getServerMoneyType(localPlayer) .. tostring(exports.mrp_global:formatMoney(cost)) .. ".", true)
		...
		elseif (ownerName=="None") and (inttype<2) then -- Unowned any other type
			gBuyMessage = guiCreateLabel(0.0, 0.915, 1.0, 0.3, "Satın almak için E tuşuna basın "..exports.mrp_pool:getServerMoneyType(localPlayer) .. tostring(exports.mrp_global:formatMoney(cost)) .. ".", true)
			guiSetFont(gBuyMessage, "default")
			guiLabelSetHorizontalAlign(gBuyMessage, "center", true)
			guiSetAlpha(gBuyMessage, 0.0)
		else
...
function showHouseMenu( absX, absY )
	rightclick = exports.mrp_rightclick
...
function onPlayerPutInInteriorSecond(dimension, interior)
	...
	if(getResourceFromName("mrp_object_system"))then
		safeToSpawn = exports['mrp_object_system']:isSafeToSpawn()
	end
...
function purchasePropertyGUI(interior, cost, isHouse, isRentable, neighborhood)
	...
	exports.mrp_global:centerWindow(purchaseProperty.window[1])
	...
	if getElementData(localPlayer, "faction") ~= -1 and getElementData(localPlayer, "factionleader") == 1 then
		btnTextSet = {"Mülkü \nkendine al", "Mülkü \nbirliğe al"}
		if exports.mrp_global:hasItem(localPlayer, 262, 1) then
	...
	if isHouse then
		local theTax = exports.mrp_payday:getPropertyTaxRate(0)
		...
		taxtax = cost * theTax
		guiSetText(purchaseProperty.label[10], ""..exports.mrp_pool:getServerMoneyType(localPlayer) ..exports.mrp_global:formatMoney(taxtax).."")
	elseif isRentable then
		...
		local theTax = exports.mrp_payday:getPropertyTaxRate(1)
	...
	guiSetText(purchaseProperty.label[8], ""..exports.mrp_pool:getServerMoneyType(localPlayer) ..exports.mrp_global:formatMoney(cost).."")
end
