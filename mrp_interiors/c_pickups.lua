...
function interiorShowPickups(interiorElement, isElevator)
    ...
    if not isElevator then
        ...
        if intStatus[INTERIOR_OWNER] > 0 then
            local ownerName = exports.mrp_cache:getCharacterNameFromID(intStatus[INTERIOR_OWNER])
            if intType == 3 then
        ...
        elseif intStatus[INTERIOR_FACTION] > 0 then
            local ownerName = exports.mrp_cache:getFactionNameFromId(intStatus[INTERIOR_FACTION])
            if intType ~= 2 then
            ...
            elseif intType == 3 then
                local intPrice = exports.mrp_global:formatMoney(intStatus[INTERIOR_COST])
                intOwner = intPrice
            else
                local intPrice = exports.mrp_global:formatMoney(intStatus[INTERIOR_COST])
        ...
        local girisucret = getElementData(interiorElement, "girispara")
        setElementData(outsidePickup, "informationicon:information", "#7f8fa6ID:#ffffff "..getElementData(interiorElement, "dbid").."\n#7f8fa6İşyeri Adı:#ffffff "..getElementData(interiorElement, "name").."\n#7f8fa6Adres:#ffffff "..getZoneNameEx(getElementPosition(interiorElement)).."\n#7f8fa6Sahibi:#ffffff "..intOwner.."\n#7f8fa6Giriş Ücreti:#ffffff ₺"..girisucret.."")
    ...
    local insidePickup = createPickup( exit[INTERIOR_X], exit[INTERIOR_Y], exit[INTERIOR_Z], 3, 1272)--tpObjectModel )
...
function canPlayerKnowInteriorOwner(theInterior)
    return  (getElementData(theInterior, "status")[INTERIOR_OWNER] == 0) -- unown.
        or  (exports.mrp_integration:isPlayerTrialAdmin(localPlayer) and (getElementData(localPlayer, "duty_admin") == 1))
...
function canPlayerSeeInteriorID(theInterior)
    return  getElementData(localPlayer, "faction") == 1 -- LSPD
        ...
        or  (exports.mrp_integration:isPlayerTrialAdmin(localPlayer) and (getElementData(localPlayer, "duty_admin") == 1))
        or  (exports.mrp_integration:isPlayerSupporter(localPlayer) and (getElementData(localPlayer, "duty_supporter") == 1))
end
...
function forcePickupSpawn()
    if exports.mrp_integration:isPlayerScripter(getLocalPlayer()) then
        initializeSoFar()
    end
end
