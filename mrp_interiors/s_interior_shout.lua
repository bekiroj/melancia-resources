    ...
    for _, interior in ipairs(possibleInteriors) do
        ...
        for _, point in ipairs( { interiorEntrance, interiorExit } ) do
            if (point[INTERIOR_DIM] == dimension) then
                local distance = getDistanceBetweenPoints3D(x, y, z, point[INTERIOR_X], point[INTERIOR_Y], point[INTERIOR_Z])
                    ...
                    for key, value in ipairs(players) do
                        ...
                        if (logged==1) then
                            ...
                            if (dimension==dbid) and (dadimension~=dimension) then
                                outputChatBox(" " .. playerName:gsub("_", " ") .. " bağırır: " .. message.."!", value, 200, 200, 200)
                            end
                        end
                    end
                end
        ...
        for _, point in ipairs( { interiorEntrance, interiorExit } ) do
            if (point[INTERIOR_DIM] == dimension) then
                ...
                if (distance <= 60) and (dimension>0) then -- what is the point of this if they are outside anyways??
                                    ...
                                    for key, value in ipairs(players) do
                                        ...
                                        if (logged==1) then
                                            ...
                                            if (isPlayerInTheCol) and (value~=thePlayer) and (dadimension~=dimension) then
                                                outputChatBox("["..languagename.."] " .. playerName:gsub("_", " ") .. " bağırır: " .. message.."!", value, 200, 200, 200)
                                            end
                                        end
                                    end
                    ...
                    {thePlayer}, mysql:getConnection(), "SELECT x, y, z, dimensionwithin, interiorwithin FROM interiors WHERE id='"..dbid.."'")
                end
            end
        end
    end
