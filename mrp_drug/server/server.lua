...
function create(thePlayer, cmd)
	if thePlayer:getData("loggedin") == 1 then
		if exports["mrp_items"]:hasItem(thePlayer, seedItemID) and exports["mrp_items"]:hasItem(thePlayer, potItemID) then
			...
			if query then
				exports.mrp_global:sendLocalMeAction(thePlayer, "yavaşça eğilerek sağ/sol elindeki saksıyı yere koyar.", false, true)
				exports.mrp_global:sendLocalMeAction(thePlayer, "sağ/sol elindeki tohumu saksıya eker.", false, true)
				exports["mrp_items"]:takeItem(thePlayer, seedItemID)
				exports["mrp_items"]:takeItem(thePlayer, potItemID)
	...
	if tonumber(ID) then
		...
		if pots[ID] then
			...
			if distance <= 5 and getElementDimension(pots[ID].object) == getElementDimension(thePlayer) and getElementInterior(pots[ID].object) == getElementInterior(thePlayer) then
				...
				if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or thePlayer:getData("faction") == 1 or owner == pdbid then
				...
				if owner == pdbid then
					if pots[ID].object:getData("state") >= 100 then
						...
						if query then
							...
							pots[ID].plant:destroy()
							pots[ID] = nil
							exports.mrp_global:sendLocalMeAction(thePlayer, "yavaşça eğilerek yerdeki saksıdaki marijuanayı toplar ve saksıyı kaldırır.", false, true)
							thePlayer:outputChat("[!]#EBEBEB "..ID.." ID'li saksıyı topladınız!",  97, 205, 85, true)
							exports["mrp_global"]:giveItem(thePlayer, marijuanaID, 3)
						else
...
function near(thePlayer, cmd)
	...
	for key, value in ipairs (pots) do
		...
		if dbid then
			...
			local ownName = exports.mrp_cache:getCharacterNameFromID(owner)
