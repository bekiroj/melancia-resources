local skinshopPeds = {
	{
	-- 161.5302734375, -81.181640625, 1001.8046875
		x = "161.5302734375",
		y = "-81.181640625",
		z = "1001.8046875",
		int = 18,
...
local firstName = { "Maria" }
local lastName = { "Brewles" }
...
function createExclusiveGUI(wName, data)
	window = guiCreateWindow(0, 0, 550, 340, "Kıyafet Mağazası ©Melancia", false)
	...
	guiGridListAddColumn(gridlist, "$", 0.20)
	
	local skinsTable = exports["mrp_mods"]:getModdedSkins()
	...
	preview_skin = exports["mrp_object_preview"]:createObjectPreview(previewSkin, 0, 0, 180, x+10+(550/2), y+25, 550/2, 230, false, false)
	...
	addEventHandler('onClientGUIScroll', scrollbar,
		function()
			...
			exports["mrp_object_preview"]:setRotation(preview_skin,0, 0, 155 + rotation * 360)
		end, false)
	...
	addEventHandler("onClientGUIClick", okButton,
		function(b)
			if (b == "left") then
				...
				if not exports["mrp_global"]:hasMoney(localPlayer, tonumber(price)) then
					guiSetText(window, "Yeterli paranız bulunmuyor. - "..wName)
					return
				end
				destroyElement(window)
				exports["mrp_object_preview"]:destroyObjectPreview(preview_skin)
