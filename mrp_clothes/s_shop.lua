...
addEventHandler('clothing:buy', resourceRoot,
	function(id)
		...
		if clothing and (canBuySkin(client, clothing) or canEdit(client)) then
			...
			if clothing.price == 0 or exports.mrp_global:hasMoney(client, clothing.price) then
				-- enough backpack space?
				if exports.mrp_global:giveItem(client, 16, clothing.skin .. ':' .. clothing.id) then
					outputChatBox('You purchased some clothing for $' .. exports.mrp_global:formatMoney(clothing.price) .. '.', client, 0, 255, 0)

					-- take & give some money
					if clothing.price > 0 then
						exports.mrp_global:takeMoney(client, clothing.price)

						local dimension = getElementDimension(client)
						if dimension > 0 then
							exports.mrp_global:giveMoney(exports.mrp_factions:getTeamFromFactionID(55), clothing.price)
						end
					end
				else
				...
				outputChatBox('You do not have the required $' .. exports.mrp_global:formatMoney(clothing.price) .. '.', client, 255, 0, 0)
			end
		end
	end, false)
