...
function createShopPed()
	...
	setElementInterior(ped, 14)--3

	setElementData(ped, 'name', 'Lara Sultan', false)
...
local function getFittingSkins()
	...
	local temp = exports['mrp_shop']:getFittingSkins()
	...
	for k, v in ipairs(list) do
		...
		if canBuySkin(localPlayer, v) or canEdit(localPlayer) then
			...
			if not only_fitting or fitting_skins[v.skin] then
				...
				guiGridListSetItemText(grid, row, 4, v.price == 0 and 'N/A' or ('$' .. exports.mrp_global:formatMoney(v.price)), false, false)
			end
		end
	end
end
				...
				if button == 'left' then
					...
					if row == -1 then
						...
						if item then
							...
							guiSetEnabled(buy, item.price == 0 or exports.mrp_global:hasMoney(localPlayer, item.price))
...
function createEditWindow()
	...
	if editing_item then
		local delete = guiCreateButton(editing_width / 2 - 50, editing_height - 30, 100, 25, 'Delete', false, editing_window)
