...
addEventHandler("onResourceStart", root,
    function(startedRes)
        if getResourceName(startedRes) == "mysql" then
            connection = exports.mrp_mysql:getConnection()
							...
							if rows > 0 then
								...
								dbQuery(
									function(qh, thePlayer)
										...
										triggerClientEvent(thePlayer, "createCheckIntWindow", thePlayer, result, exports.mrp_global:getPlayerAdminTitle(thePlayer), result2, notes)
									end,
	...
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer) then
		...
		for _, interior in ipairs(possibleInteriors) do
			if tonumber(intID) == getElementData(interior, "dbid") then
				if not exports.mrp_integration:isPlayerAdmin(thePlayer) then
					...
					--exports["mrp_interiors"]:reloadOneInterior(tonumber(intID))
					...
					if hiddenAdmin == 0 then
						exports.mrp_global:sendMessageToAdmins("[INTERIOR]: "..adminTitle.." ".. getPlayerName(thePlayer):gsub("_", " ").. " ("..adminUsername..") has restocked "..getElementData(interior, "name").." (ID#"..intID..") with "..amount.." of supplies.")
					else
						exports.mrp_global:sendMessageToAdmins("[INTERIOR]: A hidden admin has has restocked "..getElementData(interior, "name").." (ID#"..intID..") with "..amount.." of supplies.")
					end
...
function setInteriorFaction(thePlayer, cmd, ...)
	if exports.mrp_integration:isPlayerAdmin(thePlayer) then
		...
		if tonumber(clue) then
			theFaction = exports.mrp_pool:getElement("team", tonumber(clue))
		else
			theFaction = exports.mrp_factions:getFactionFromName(clue)
		end
		...
		local dbid, entrance, exit, interiorType, interiorElement = exports['mrp_interiors']:findProperty( thePlayer )
		if not isElement(interiorElement) then
			outputChatBox("No interior found here.", thePlayer, 255, 0, 0 )
			return
		end

		local can , reason = exports.mrp_global:canFactionBuyInterior(theFaction)
		if not can then
		...
		call( getResourceFromName( "mrp_items" ), "deleteAll", interiorType == 1 and 5 or 4, dbid )
		exports.mrp_global:giveItem(thePlayer, interiorType == 1 and 5 or 4, dbid)

		exports.mrp_logs:dbLog(thePlayer, 37, { "in"..tostring(dbid) } , "SETINTFACTION INTERIOR ID#"..dbid.." TO FACTION '"..factionName.."'")
		exports['mrp_interiors']:realReloadInterior(tonumber(dbid))
		triggerClientEvent(thePlayer, "createBlipAtXY", thePlayer, entrance[INTERIOR_TYPE], entrance[INTERIOR_X], entrance[INTERIOR_Y])
		exports.mrp_global:sendMessageToAdmins("[INTERIOR] "..exports.mrp_global:getPlayerFullIdentity(thePlayer).." transferred the ownership of interior '"..intName.."' ID #"..dbid.." to faction '"..factionName.."'.")
		return true
	end
end
...
function setInteriorToMyFaction(thePlayer, cmd)
	...
	local theFaction = exports.mrp_pool:getElement("team", tonumber(factionId))
	if not theFaction then
		outputChatBox("No faction found.", thePlayer, 255, 0, 0 )
		return
	end
	...
	local can , reason = exports.mrp_global:canPlayerFactionBuyInterior(thePlayer)
	if not can then
		outputChatBox(reason, thePlayer, 255, 0, 0 )
		return 
	end
	...
	exports.mrp_logs:dbLog(thePlayer, 37, { "in"..tostring(dbid) } , "SETINTTOMYFACTION INTERIOR ID#"..dbid.." TO FACTION '"..factionName.."'")
	exports['mrp_interiors']:realReloadInterior(tonumber(dbid))
	triggerClientEvent(thePlayer, "createBlipAtXY", thePlayer, entrance[INTERIOR_TYPE], entrance[INTERIOR_X], entrance[INTERIOR_Y])
	exports.mrp_global:sendMessageToAdmins("[INTERIOR] "..exports.mrp_global:getPlayerFullIdentity(thePlayer).." transferred the ownership of interior '"..intName.."' ID #"..dbid.." to his faction '"..factionName.."'.")
	return true
end
...
function cloneNote(player)
	...
	while q do
		...
		if dbExec(mysql:getConnection(),"INSERT INTO interior_notes SET note='"..(int.adminnote).."', intid="..int.id) then
