...
function draw()
	...
	if isPedInVehicle ( localPlayer ) then
		...
		if radioID and radioID >= 0 and streams[radioID] and getVehicleType(vehicle) ~= 'BMX' and getVehicleType(vehicle) ~= 'Bike' and getVehicleType(vehicle) ~= 'Quad' then
			...
			local size = dxGetTextWidth( text, 1, "default-bold" )

			--dxDrawText ( text, left, sh * 0.911, sw, sh, tocolor ( 0, 0, 0, 255 ), 1, "default-bold" )
			
				dxDrawText(text, sx/2-250/2+1, 20, 250+(sx/2-250/2)+1, 30, tocolor(0,0,0),1,"sans", "center", "top")
				dxDrawText(text, sx/2-250/2-1, 20, 250+(sx/2-250/2)-1, 30, tocolor(0,0,0),1,"sans", "center", "top")
				dxDrawText(text, sx/2-250/2, 20+1, 250+(sx/2-250/2), 30+1, tocolor(0,0,0),1,"sans", "center", "top")
				dxDrawText(text, sx/2-250/2, 20-1, 250+(sx/2-250/2), 30-1, tocolor(0,0,0),1,"sans", "center", "top")

				dxDrawText(text, sx/2-250/2, 20, 250+(sx/2-250/2), 30, tocolor(255,255,255),1,"sans", "center", "top")
	...
	if mp3Station and mp3Station ~= nil then
		...
		dxDrawImage ( left-10, top - 2, 50, 50, ":mrp_hud/images/radio.png" ,0, 0, 0, tocolor(255,255,255), true )
		dxDrawText ( text, left - 1, top, sw, sh, tocolor ( 70, 200, 14, 255 ), 1, "default-bold" )
...
function showMP3GUI()
	...
	mp3Window = guiCreateStaticImage(mp3_x, mp3_y, mp3_w, mp3_h, ":mrp_resources/window_body.png", false)
