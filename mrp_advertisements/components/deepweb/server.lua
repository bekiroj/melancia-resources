...
addEventHandler("deepweb:receive", root,
	function(player, message)
		if isTimer(timers[getElementData(player, "dbid")]) then
			exports["mrp_infobox"]:addBox(player, "error", "Yalnızca 30 dakikada bir paylaşımda bulunabilirsiniz.")
		return
		end
		
		if exports.mrp_global:hasMoney(player, 300) or player:getData("vip") > 0 then
			...
			for k,v in ipairs(advertisementMessages) do
				...
				if (found) or (found2) or (string.lower(message)==tostring(v)) then
					exports.mrp_global:sendMessageToAdmins("AdmWrn: " .. tostring(getPlayerName(player)) .. " paylaşımda bulunurken tehlikeli kelimelere rastlandı.")
					exports.mrp_global:sendMessageToAdmins("AdmWrn: Paylaşım mesajı: " .. tostring(message))
					exports["mrp_infobox"]:addBox(player, "error", "Paylaşımda bulunurken hatalı kelimelere rastladınız.")
					return
				end
			end
			local upperCount = 0
			...
			if player:getData("vip") == 0 then
				exports.mrp_global:takeMoney(player, 300)
			end
			exports["mrp_infobox"]:addBox(player, "success", "Başarıyla paylaşımda bulundunuz, 15 saniye içerisinde doğrulanıp paylaşılacak.")
			exports["mrp_infobox"]:addBox(player, "error", "İptal etmek için: /paylasimiptal "..advertID)
			exports["mrp_progressbar"]:drawProgressBar("Deep Web", "Paylaşımın doğrulanıyor.",player, 255, 0, 0, 15000)			
			exports.mrp_global:sendMessageToAdmins("AdmWrn: "..player.name.." paylaşım gönderdi:")
			exports.mrp_global:sendMessageToAdmins("AdmWrn: Paylaşım içeriği: "..message.." - Paylaşım iptali için /paylasimiptal "..advertID)
				...
				function(adID, plr)
					if isElement(player) then
						if adverts[advertID] then
							...
							exports.mrp_global:giveMoney(player, 300)
						end
					end
				end,
			...
			exports["mrp_infobox"]:addBox(player, "error", "Paylaşımda bulunabilmeniz için 300 TL'ye ihtiyacınız var.")
		end
	end
)
...
addCommandHandler("paylasimiptal",
	function(player, cmd, id)
		...
		if id and exports.mrp_integration:isPlayerTrialAdmin(player) or adverts[id] == player:getData("dbid") and adverts[id] and id and adverts[id] then
			adverts[(id)] = false
			exports["mrp_infobox"]:addBox(player, "success", "Başarıyla paylaşımı iptal ettiniz.")
			exports.mrp_global:sendMessageToAdmins("AdmWrn: ["..player.name.."] son paylaşımı iptal etti, yayınlanmayacak.")
		end
	end
)
