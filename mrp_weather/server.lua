...
function changeWeather()
	...
	if weatherArray[1] then
		...
		exports.mrp_global:sendMessageToAdmins("Weather changing to "..currentWeather[1]..":"..currentWeather[3] .." for "..	currentWeather[2] .. " second(s).")
		...
		if weatherSQLUpdate then
			exports.mrp_global:sendMessageToAdmins("[WEATHER] SQL Settings for weather successfully set to " .. weatherID .. ".")
...
function getWeatherRawr(tp)
	if exports.mrp_integration:isPlayerTrialAdmin(tp) then
...
function sw(tp, commandName, weather)
	if exports.mrp_integration:isPlayerTrialAdmin(tp) then
		...
		outputChatBox("Invalid ID entered.", tp)
			...
			local rank = exports.mrp_global:getPlayerAdminTitle(tp)
			local name = getPlayerName(tp):gsub("_"," ")
			exports.mrp_global:sendMessageToAdmins("AdmCmd: "..rank.." "..name.." set the weather to ID: "..weather)
...
function swb(tp, commandName, weather)
	if exports.mrp_integration:isPlayerAdmin(tp) then
		if not weather then
		outputChatBox("Invalid ID entered.", tp)
		else
			...
			local rank = exports.mrp_global:getPlayerAdminTitle(tp)
			local name = getPlayerName(tp):gsub("_"," ")
			exports.mrp_global:sendMessageToAdmins("AdmCmd: "..rank.." "..name.." blended the weather to ID: "..weather)
...
function swh(tp, commandName, height)
	if exports.mrp_integration:isPlayerAdmin(tp) then
		...
		outputChatBox("Invalid ID entered.", tp)
			...
			local rank = exports.mrp_global:getPlayerAdminTitle(tp)
			local name = getPlayerName(tp):gsub("_"," ")
			exports.mrp_global:sendMessageToAdmins("AdmCmd: "..rank.." "..name.." set the wave height to: "..height)
...
function sf(tp, commandName, fogLevel)
	...
	local rank = exports.mrp_global:getPlayerAdminTitle(tp)
	local name = getPlayerName(tp):gsub("_"," ")
	exports.mrp_global:sendMessageToAdmins("AdmCmd: "..rank.." Admin "..name.." set the fog level to: "..fogLevel)
...
function etan(tp, command)
	if exports.mrp_integration:isPlayerTrialAdmin(tp) then
		if not exports.mrp_integration:isPlayerAdmin(tp) then
	...
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) then
		if not snowlevel or not temperature then
			...
			if tonumber(snowlevel) and tonumber(temperature) and tonumber(snowlevel) >= 0 and tonumber(snowlevel) <= 4 then
				...
				exports.mrp_global:sendMessageToAdmins(getPlayerName(thePlayer).." has set snow level to "..snowlevel.." and temperature to "..temperature)
			else
