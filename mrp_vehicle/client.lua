...
function requestInventory(element)
	if (element) and (getElementType(element)=="vehicle") then
		...
		elseif type(getElementData(vehicle, "Impounded")) == "number" and isVehicleImpounded(vehicle) and not exports.mrp_global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) then
			outputChatBox("Aracı aramak için anahtara ihtiyacın var.", 255, 0, 0)
		elseif (getElementData(vehicle, "owner") ~= getElementData(localPlayer, "dbid")) and not exports.mrp_global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) then 
			outputChatBox("#575757Melancia: #ffffffBu işlemi yapabilmek için aracın anahtarına sahip olmalısınız.", 255, 0, 0, true)
		else
...
function lockUnlock(button, state)
    if (button=="left") then
        ...
        elseif exports.mrp_global:hasItem(localPlayer, 3, getElementData(vehicle, "dbid")) or (exports["mrp_factions"]:isPlayerInFaction(localPlayer, getElementData(vehicle, "faction"))) then
    ...
    if(source == localPlayer) then
        ...
        for k,v in ipairs(vehicles) do
            ...
            if(model == 525) then --towtruck
                ...
                return true
            --stage 2: Check items
            elseif(exports.mrp_global:hasItem(v, 144)) then --single yellow strobe (airport, etc.)
...
addEventHandler("showVehiclesPanel", root,
	function(table)
		...
		window = guiCreateWindow(0, 0, 389, 366, "İnaktif Araçlarım - Melancia", false)
        	...
        	function(b)
        		if (source == ok) then
        			...
        			if row ~= -1 and col ~= -1 then
        				...
        				outputChatBox(exports.mrp_pool:getServerSyntax(false, "s").."Araç başarıyla aktif edildi.", 255, 255, 255, true)
        			else
        				outputChatBox(exports.mrp_pool:getServerSyntax(false, "e").."Aktif etmek istediğiniz araç ID'yi girin.", 255, 255, 255, true)
        			end
        		end
        	end
        )
