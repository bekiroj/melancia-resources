...
function getPlayerThirst(player) -- Hatasız
	if (tonumber(getElementData(getLocalPlayer(), "loggedin")) == 1) then
		if (player == getLocalPlayer()) then
			return getElementData(player, "drink")
		end
	end
end
	...
	if (tonumber(getElementData(getLocalPlayer(), "loggedin")) == 1) then
		if (player == getLocalPlayer()) then
			if not (getPlayerThirst(player) <= 0) then
				triggerServerEvent("anticheat:changeEld", player, player, "drink", getPlayerThirst(player) - 1)
			end
		end
	end
end
...
function bugFixSusama1() -- Hatasız
	if (tonumber(getElementData(getLocalPlayer(), "loggedin")) == 1) then
		if (getPlayerThirst(getLocalPlayer()) >= 101) then
			triggerServerEvent("anticheat:changeEld", getLocalPlayer(), getLocalPlayer(), "drink", 100)
		end
	end
end
...
function bugFixSusama2() -- Hatasız
	if (tonumber(getElementData(getLocalPlayer(), "loggedin")) == 1) then
		if (getPlayerThirst(getLocalPlayer()) <= -1) then
			triggerServerEvent("anticheat:changeEld", getLocalPlayer(), getLocalPlayer(), "drink", 0)
		end
	end
end
