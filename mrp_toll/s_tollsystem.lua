...
function startSystem()
	for key, group in ipairs(tollPorts) do
		...
		for dataKey, dataGroup in ipairs (group.data) do
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "talk",1, true)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "name", pedName:gsub("_", " "), true)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "ped:type", "toll", true)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "ped:tollped",true, true)

			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "languages.lang1" , 1, false)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "languages.lang1skill", 100, false)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "languages.lang2" , 2, false)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "languages.lang2skill", 100, false)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "languages.current", 1, false)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "toll:object", tollElements[key][dataKey]["object"], false)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "toll:data", dataGroup, false)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "toll:busy", false, false)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "toll:state", false, false)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "toll:name", group.name, false)
			exports.mrp_anticheat:changeProtectedElementDataEx(tollElements[key][dataKey]["ped"], "toll:key", key, false)
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(col, "toll:ped", tollElements[key][dataKey]["ped"], false)
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(col2, "toll:ped", tollElements[key][dataKey]["ped"], false)
...
function triggerGate(gateKeeperPed, earlyOpenZone) -- OPEN GATE
	...
	exports.mrp_anticheat:changeProtectedElementDataEx(gateKeeperPed, "toll:busy", true, false)
	...
	exports.mrp_anticheat:changeProtectedElementDataEx(gateKeeperPed, "toll:opened", true, false)
	...
	if earlyOpenZone then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(gateKeeperPed, "earlyOpened", true, false)
		timerEarlyOpen[gateKeeperPed] = setTimer(function()
			exports.mrp_anticheat:changeProtectedElementDataEx(gateKeeperPed, "earlyOpened", false, false)
...
function processCloseTolls(tollKey, thePed, thePlayer, payByBank) -- CLOSE GATE
	...
	moveObject ( tollObject, gateSpeed, newX, newY, newZ, offsetRX, offsetRY, offsetRZ )
	exports.mrp_anticheat:changeProtectedElementDataEx(thePed, "toll:opened", false, false)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePed, "toll:busy", true, false)
...
function startTalkToPed ()
	...
	if (isPedInVehicle(thePlayer)) then
		...
		if (exports['mrp_vehicle']:isVehicleWindowUp(theVehicle)) then
...
function processOpenTolls(tollKey, thePed, thePlayer, payByBank)
	if not tollElementsLocked[tollKey] or exports.mrp_global:hasItem(thePlayer, 64) or exports.mrp_global:hasItem(thePlayer, 65) or exports.mrp_global:hasItem(thePlayer, 112) then
		if payByBank then
			if not exports.mrp_global:hasItem(getPedOccupiedVehicle(thePlayer), 118) then
		...
		triggerGate(thePed)
		exports.mrp_global:giveMoney( getTeamFromName("İstanbul Büyükşehir Belediyesi"), 3 )
		exports.mrp_global:giveMoney( getTeamFromName("İstanbul Emniyet Müdürlüğü"), 2 )
...
function testToggle(thePlayer, commandName, gateID, silent)
	...
	if factionID ~= 1 and factionID ~= 78 and not exports.mrp_integration:isPlayerTrialAdmin(thePlayer) then
		return
	end

	if not gateID or not tonumber(gateID) then
		outputChatBox("Kullanım: /"..commandName.." [ID]", thePlayer)
		--return ]]
		for tollID, tollName in ipairs(tollElementsName) do
			...
			outputChatBox(" ID:"..tostring(tollID).. " - Bölge: "..tollName .." - Durum: "..state, thePlayer)
		end
		return
	end
	...
	if not tollElementsName[gateID] then
		outputChatBox("Gişe bulunamadı.", thePlayer)
	end
	...
	if tollElementsLocked[gateID] then
		...
		for k, arrayPlayer in ipairs(getPlayersInTeam(getTeamFromName ("İstanbul Emniyet Müdürlüğü"))) do
			outputChatBox("[İEM]#ffffff "..getPlayerName(thePlayer):gsub("_", " ").." konuşuyor, "..tollElementsName[gateID].." bölgedesindeki gişeyi kapatmak gerekiyor.",arrayPlayer,100,100,255,true)
		end
		for k, lscsdplayer in ipairs(getPlayersInTeam(getTeamFromName ("İstanbul İl Jandarma Komutanlığı"))) do
			outputChatBox("[JANDARMA]#ffffff "..getPlayerName(thePlayer):gsub("_", " ").."  konuşuyor, "..tollElementsName[gateID].." bölgedesindeki gişeyi kapatmak gerekiyor.",lscsdplayer,74,104,44,true)
		end
		for _, thePed in ipairs(getElementsByType('ped')) do
			...
			if tollKey and tollKey == gateID then
				if first then
					for k, arrayPlayer in ipairs(getPlayersInTeam(getTeamFromName ("İstanbul Emniyet Müdürlüğü"))) do
						outputChatBox("[İEM]#ffffff Gişe kontrol noktası konuşuyor, "..tollElementsName[gateID].." bölgedesindeki gişeyi kapatıyorum.",arrayPlayer,100,100,255,true)
					end
					for k, lscsdplayer in ipairs(getPlayersInTeam(getTeamFromName ("İstanbul İl Jandarma Komutanlığı"))) do
						outputChatBox("[JANDARMA]#ffffff "..getPlayerName(thePlayer):gsub("_", " ").."  konuşuyor, "..tollElementsName[gateID].." bölgedesindeki gişeyi kapatıyorum.",lscsdplayer,74,104,44,true)
					end
					first = false
				end
			end
		...
		local first = true
		for k, arrayPlayer in ipairs(getPlayersInTeam(getTeamFromName ("İstanbul Emniyet Müdürlüğü"))) do
			outputChatBox("[İEM]#ffffff "..getPlayerName(thePlayer):gsub("_", " ").." konuşuyor, "..tollElementsName[gateID].." bölgedesindeki gişeyi aktif etmek gerekiyor.",arrayPlayer,100,100,255,true)
		end
		for k, lscsdplayer in ipairs(getPlayersInTeam(getTeamFromName ("İstanbul İl Jandarma Komutanlığı"))) do
			outputChatBox("[JANDARMA]#ffffff "..getPlayerName(thePlayer):gsub("_", " ").."  konuşuyor, "..tollElementsName[gateID].." bölgedesindeki gişeyi aktif etmek gerekiyor.",lscsdplayer,74,104,44,true)
		end
		for _, thePed in ipairs(getElementsByType('ped')) do
			local tollKey = getElementData(thePed, "toll:key")
			if tollKey and tollKey == gateID then
				if first then
					for k, arrayPlayer in ipairs(getPlayersInTeam(getTeamFromName ("İstanbul Emniyet Müdürlüğü"))) do
						outputChatBox("[İEM]#ffffff  Gişe kontrol noktası konuşuyor, "..tollElementsName[gateID].." bölgedesindeki gişeyi tekrardan aktif ediyorum.",arrayPlayer,100,100,255,true)
					end
					for k, lscsdplayer in ipairs(getPlayersInTeam(getTeamFromName ("İstanbul İl Jandarma Komutanlığı"))) do
						outputChatBox("[JANDARMA]#ffffff  Gişe kontrol noktası konuşuyor, "..tollElementsName[gateID].." bölgedesindeki gişeyi tekrardan aktif ediyorum.",lscsdplayer,74,104,44,true)
					end
					first = false
				end
			end
...
addCommandHandler("gisekapat", testToggle)
...
function setConvoState(thePlayer, state)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "toll:convoState", state, false)
end
...
function processMessage(thePed, message, language)
	...
	exports['mrp_chat']:localIC(thePed, message, language)
end

function processRadio(thePed, message, source)
	local name = getElementData(thePed, "name") or getPlayerName(thePed)
	exports['mrp_global']:sendLocalText(source, "[English] " .. name .. "'s Radio: "..message, 255, 255, 255)
end

function processMeMessage(thePed, message, source)
	local name = getElementData(thePed, "name") or getPlayerName(thePed)
	exports['mrp_global']:sendLocalText(source, " *" ..  string.gsub(name, "_", " ").. ( message:sub( 1, 1 ) == "'" and "" or " " ) .. message, 255, 51, 102)
end

function processDoMessage(thePed, message, source)
	local name = getElementData(thePed, "name") or getPlayerName(thePed)
	exports['mrp_global']:sendLocalText(source, " * " .. message .. " *      ((" .. name:gsub("_", " ") .. "))", 255, 51, 102)
end
...
function resetBusyState(theGate)
	...
	if (isGateBusy) then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(theGate, "toll:busy", false, false)
	end
end
	...
	if duty > 0 and getElementDimension( thePlayer ) == 0 then
		...
		for key, value in ipairs( tollElements ) do
			...
			if name then
				...
				for k, v in ipairs(value) do
					if v.ped then
						exports.mrp_logs:dbLog(thePlayer, 35, v.ped, "TOLL OPEN " .. name)
