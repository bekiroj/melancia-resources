local connection = exports.mrp_mysql:getConnection()
...
function dolapCmd(plr , cmd , arg1)
	...
	elseif arg1 == "goruntule" then
		...
		if ( dim < 19000 and ( exports["mrp_items"]:hasItem(plr, 5, dim) or exports["mrp_items"]:hasItem(plr, 4, dim) ) ) or ( dim >= 20000 and exports["mrp_items"]:hasItem(plr, 3, dim-20000) ) then
	...
	elseif arg1 == "istek" then 
		...
		for k , v in ipairs(getElementsByType("player")) do 
			if ( dim < 19000 and ( exports["mrp_items"]:hasItem(v, 5, dim) or exports["mrp_items"]:hasItem(v, 4, dim) ) ) or ( dim >= 20000 and exports["mrp_items"]:hasItem(v, 3, dim-20000) ) then
...
function dolapServerGUI(plr , boole , dim , state)
	...
	dbQuery(
		function(qh)
			...
			triggerLatentClientEvent(plr ,  "dolapGUI" , 5000 , false , plr , boole , dolapItems , exports["mrp_items"]:getItems(plr) , state)
...
addEventHandler("dolapAktar" , root , function(itemID , itemValue , dim , text , wep , value)
	...
	local success, error = exports["mrp_items"]:loadItems( element )
	if success then


		local success, slot = exports["mrp_items"]:hasItem(element, itemID , itemValue)
		if success then

			exports["mrp_items"]:takeItemFromSlot(element, slot)
							...
							if v.text == text and v.item == itemID then
								...
								if not state then 
									...
									if tonumber(v.item) == 115 then
										...
										exports["mrp_items"]:giveItem(plr, 115, (v.weapon)..":"..serial..":"..(v.text).."::")
										
									else 

										exports["mrp_items"]:giveItem(plr , tonumber(v.item) , v.value)
