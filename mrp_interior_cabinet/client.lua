...
GUI.window[1] = guiCreateWindow((sx-630)/2, (sy-535)/2, 630, 535, "Melancia - Dolap Sistemi", false)
...
addEventHandler("dolapGUI" , root , function(state , dolapItems , playerItems , state2)
    ...
    for k , v in ipairs(playerItems) do 
        ...
        if itemIDs[v[1]] then 
            ...
            local itemname = exports["mrp_items"]:getItemName(v[1] , 1)
            local itemwp   = 0
...
addEventHandler("onClientGUIClick" , root , function()
    ...
    elseif GUI.button[2] == source then -- Dolaba Ekleme
        ...
        local items = exports["mrp_items"]:getItems(localPlayer)
