...
function removeAnimation(player)
	exports.mrp_global:removeAnimation(player)
end
...
function removeOOC(text)
	if not text then
		outputChatBox("Error code 0001 occurs, please report to Melancia MTA Scripting Team at https:/www.Melanciaroleplay.com/ucp")
		return ""
	end
	...
	if itemID then
		...
		elseif (itemID==3) then -- car key
			...
			if veh and getElementData(veh, "dbid") == itemValue then
				...
				-- unlock nearby cars
				local value = exports.mrp_pool:getElement( "vehicle", itemValue )
				if value then
		...
		elseif (itemID==4) or (itemID==5) then -- house key or business key
			...
			if found then
				local dbid, entrance, exit, interiorType, interiorElement = exports['mrp_interiors']:findProperty( source, found )
				...
				if locked == 0 then
					...
					if not (exports.mrp_global:hasItem(source, 4, found)) and not (exports.mrp_global:hasItem(source, 5, found)) then
						exports.mrp_logs:logMessage("[INTERIOR-UNLOCK] Interior #" .. found .. " was unlocked by " .. getPlayerName(source), 21)
					end
				else
					...
					if not (exports.mrp_global:hasItem(source, 4, found)) and not (exports.mrp_global:hasItem(source, 5, found)) then
						exports.mrp_logs:logMessage("[INTERIOR-LOCK] Interior #" .. found .. " was locked by " .. getPlayerName(source), 21)
					end
				end
		...
		exports["mrp_oocshop"]:silahKasa(source)
		elseif (itemID==583) then
		exports["mrp_oocshop"]:premiumKasa(source)		

		elseif (itemID==8) then -- sandwich
			giveHunger(source, 70)
			takeThirst(source, 7)
			
			exports.mrp_global:applyAnimation(source, "food", "eat_burger", 4000, false, true, true)
		...
		elseif (itemID==9) then -- sprunk
			...
			exports.mrp_global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			toggleAllControls(source, true, true, true)
			triggerClientEvent(source, "onClientPlayerWeaponCheck", source)
		...
		elseif (itemID==10) then -- red dice
			...
			exports.mrp_global:sendLocalText(source, " ✪ " .. getPlayerName(source):gsub("_", " ") .. " bir " .. (max == 6 and "" or (max .. "-yüzlü ")) .. " zar atar ve '" .. math.random( 1, max ) .."' sayısı gelir.", 255, 51, 102)
		elseif (itemID==11) then -- adana
			...
			exports.mrp_global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==591) then -- pizza
			giveHunger(source, 60)
			takeThirst(source, 30)
			
			exports.mrp_global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "bir Orta Boy pizza yer.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==592) then -- piliç
			giveHunger(source, 70)
			takeThirst(source, 40)
			
			exports.mrp_global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "bir tabak piliç yer.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==593) then -- omlet
			giveHunger(source, 80)
			takeThirst(source, 20)
			
			exports.mrp_global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "bir tabak omlet yer.")
			takeItemFromSlot(source, itemSlot)
		...
		elseif (itemID==16) then -- clothes
			...
			--if fittingskins[gender] and fittingskins[gender][race] and fittingskins[gender][race][skin] then
				...
				exports.mrp_anticheat:changeProtectedElementDataEx(source, "clothing:id", clothingid, true)
		...
		elseif (itemID==17) then -- watch
			...
			exports.mrp_global:applyAnimation(source, "COP_AMBIENT", "Coplook_watch", 4000, false, true, true)
		elseif (itemID==20) then -- STANDARD FIGHTING
			...
			dbExec(mysql:getConnection(), "UPDATE characters SET fightstyle = 4 WHERE id = " .. getElementData( source, "dbid" ) )
		elseif (itemID==21) then -- BOXING
			...
			dbExec(mysql:getConnection(), "UPDATE characters SET fightstyle = 5 WHERE id = " .. getElementData( source, "dbid" ) )
		elseif (itemID==22) then -- KUNG FU
			...
			dbExec(mysql:getConnection(), "UPDATE characters SET fightstyle = 6 WHERE id = " .. getElementData( source, "dbid" ) )
		elseif (itemID==23) then -- KNEE HEAD
			...
			--dbExec(mysql:getConnection(), "UPDATE characters SET fightstyle = 7 WHERE id = " .. getElementData( source, "dbid" ) )
		elseif (itemID==24) then -- GRAB KICK
			...
			dbExec(mysql:getConnection(), "UPDATE characters SET fightstyle = 15 WHERE id = " .. getElementData( source, "dbid" ) )
		elseif (itemID==25) then -- ELBOWS
			...
			dbExec(mysql:getConnection(), "UPDATE characters SET fightstyle = 16 WHERE id = " .. getElementData( source, "dbid" ) )
		elseif (itemID==27) then -- FLASHBANG
			takeItemFromSlot(source, itemSlot)

			local obj = createObject(343, unpack(additional))
			exports.mrp_pool:allocateElement(obj)
		...
		elseif (itemID==28) then -- GLOWSTICK
			...
			exports.mrp_pool:allocateElement(marker)
			...
			--if (getElementData(source, "duty") > 0) and (getElementModel(source) == 285) then
				...
				if not (found) then
					...
					elseif (interiorStatus[3]) then
						...
						exports.mrp_anticheat:changeProtectedElementDataEx(found, "status", interiorStatus, true)
						...
						exports.mrp_global:sendMessageToAdmins("Player "..getPlayerName(source).." opened interior "..dbid.." with a doorram")
						exports.mrp_logs:dbLog(source, 31, { source, found }, "UNLOCK WITH DOORRAM")
					else
		...
		elseif (itemID==34) then
			...
			setPedArmor(source, getPedArmor(source) + 3)
			setElementData(source, "gecici_armor", 3)
			setTimer(
				function() 
					setElementData(source, "gecici_armor", 0)
					if (getPedArmor(source) - 3 < 0) then
		...
		elseif (itemID==36) then
			...
			setPedArmor(source, getPedArmor(source) + 15)
			setTimer(
				function() 
					if (getPedArmor(source) - 15 < 0) then
					setPedArmor(source, 0)
					else
						setPedArmor(source, getPedArmor(source) - 15)
					end
                ...
                if sucess then
                    if (tonumber(itemvalue) >= 1) then
                        if (hasSpaceForItem(source, 182, 1)) then
                            ...
                            exports.mrp_global:sendLocalMeAction(source, "grabs a marijuana bud out of the bag, begining to roll the joint afterwards.")
		...
		elseif (itemID==55) then -- Stevie's business card
			...
			outputChatBox("The card okur: 'Steven Pullman - L.V. Freight Depot, Tel: 12555'", source, 255, 51, 102)
		elseif (itemID==57) then -- FUEL CAN
			local nearbyVehicles = exports.mrp_global:getNearbyElements(source, "vehicle")
			...
			if found then
				...
				outputChatBox("[!] #ffffffAraçtan uzaksın.", source, 255, 0, 0, true)
			end
		elseif (itemID==58) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "bir şişe Bira'dan yudum almaya başlar")
			setElementHealth(source,getElementHealth(source)-5)
			triggerEvent("realism:startdrinking", source, 0)
		...
		elseif (itemID==60) then
			...
			local dimension = getElementDimension(source)
			local retval = call(getResourceFromName("mrp_interiors"), "addSafeAtPosition", source, x,y,z, rz) --0 no error, 1 safe already exists, 2 player does not own interior
			if (retval == 0) then
		...
		elseif (itemID==69) then -- Dictionary
			local learned = call(getResourceFromName("mrp_languages"), "learnLanguage", source, itemValue, true)
			local lang = call(getResourceFromName("mrp_languages"), "getLanguageName", itemValue)
		...
		elseif (itemID==75) then
			...
			for k, v in pairs( getElementsByType( "object", getResourceRootElement( ) ) ) do
				if isElement( v ) and getElementData( v, "itemID" ) == 74 and getElementData( v, "itemValue" ) == itemValue then
					...
					if getDistanceBetweenPoints3D( x, y, z, px, py, pz ) < 200 then
						dbExec(mysql:getConnection(), "DELETE FROM worlditems WHERE id = " .. getElementData( v, "id" ) )
						createExplosion( x, y, z, 10, source )
						createExplosion( x, y, z, 10, source )
						destroyElement( v )
					end
			...
			for k, v in pairs( exports.mrp_global:getNearbyElements(source, "vehicle", 200) ) do
			...
			for k, v in pairs( exports.mrp_global:getNearbyElements(source, "player", 200) ) do
				if hasItem( v, 74, itemValue ) then
					local x, y, z = getElementPosition( v )
					createExplosion( x, y, z, 10, source )
					createExplosion( x, y, z, 10, source )
		...
		elseif (itemID==77) then -- Card Deck
			...
			exports.mrp_global:sendLocalText(source, " ✪ " .. getPlayerName(source):gsub("_", " ") .. " bir kart çeker ve kart ".. sign[snumber] .. ( number == 1 and "n" or "" ) .. " " .. cards[number] .." gelmiştir.", 255, 51, 102)
		elseif (itemID==79) then -- Porn tape
			exports.mrp_global:applyAnimation( source, "PAULNMAC", "wank_loop", -1, true, false, false)
		...
		elseif (itemID==104) then -- portable TV
			...
			if isTvUsed == nil or isTvUsed == false then
				...
				exports.mrp_anticheat:changeProtectedElementDataEx(source, "isTvUsed", true, false)
			else
				exports.mrp_anticheat:changeProtectedElementDataEx(source, "isTvUsed", false, false)
			end
		...
		elseif (itemID == 106) then -- Cigarette
			if hasItem( source, 107 ) then
				...
				outputChatBox("#660099Sunucu: #bcbcbc/sigaraat ile sigarayı atabilirsiniz, '1' '2' tuşları ile sigarayı içebilirsiniz.", source, 255, 194, 15, true)
                exports.mrp_global:applyAnimation(source, "SMOKING", "M_smk_in", 6000, false, true, true)
				...
				outputChatBox( "Bunu kullanabilmek için çakmak gerekli!", source, 255, 0, 0 )
			end
		...
		elseif itemID == 134 then
			outputChatBox("#575757Melancia:#f9f9f9 Cüzdanında toplam " .. exports.mrp_global:formatMoney(itemValue) .. " TL paran var.", source, 255, 0, 255, true)
		...
		elseif masks[itemID] then
			...
			if itemID == 56 and not getElementData(source, "faction") == 1 then
				outputChatBox("#575757Melancia: #ffffffBu öğeyi sadece VIP'ler veya İEM üyeleri kullanabilir. VIP satın almak için ( /market )", source, 255, 0, 0, true)
				return false
			end
			local mask = getElementData(source, data[1])
			if itemID == 26 and not getElementData(source, "faction") == 1 then
				outputChatBox("#575757Melancia: #ffffffBu öğeyi sadece VIP'ler veya İEM üyeleri kullanabilir. VIP satın almak için ( /market )", source, 255, 0, 0, true)
				return false
			end
			
			if mask then
				triggerEvent('sendAme', source, "Sağ elini yüzüne götürerek maskesini çıkarır.")
				exports.mrp_anticheat:changeProtectedElementDataEx(source, data[1], false, true)
			...
			elseif getPedOccupiedVehicle(source) and getElementData(getPedOccupiedVehicle(source), "job") > 0 and getVehicleOccupant(getPedOccupiedVehicle(source)) == source then
				...
				triggerEvent('sendAme', source, "Sağ elindeki maskeyi yüzüne geçirir.")
				exports.mrp_anticheat:changeProtectedElementDataEx(source, data[1], true, true)

				if itemID == 90 then -- Helmet
					if getElementData(source, "bikerhelmet") then
						exports.mrp_anticheat:changeProtectedElementDataEx(source, "bikerhelmet", false, true)
						triggerEvent("artifacts:remove", source, source, "bikerhelmet")
					end
					if getElementData(source, "fullfacehelmet") then
						exports.mrp_anticheat:changeProtectedElementDataEx(source, "fullfacehelmet", false, true)
						triggerEvent("artifacts:remove", source, source, "fullfacehelmet")
					end
				...
				elseif itemID == 171 then -- Biker Helmet
					if getElementData(source, "helmet") then
						exports.mrp_anticheat:changeProtectedElementDataEx(source, "helmet", false, true)
						triggerEvent("artifacts:remove", source, source, "helmet")
					end
					if getElementData(source, "fullfacehelmet") then
						exports.mrp_anticheat:changeProtectedElementDataEx(source, "fullfacehelmet", false, true)
		...
		elseif itemID == 580 then
			outputChatBox("Melancia:#f9f9f9 Tamir kiti geçici olarak devre dışı bırakılmıştır.", source, 255, 0, 0, true)
		--[[
			local itemValue = tonumber(itemValue)
			local veh = getPedOccupiedVehicle(source)
						if (getElementData(veh, "engine") == 1) then
								outputChatBox("[-]#f9f9f9 Tamir kitini kullanabilmen için aracının motorunu durdurman gerekiyor.",source,255,102,102,true)
						return end
			if source.vehicle then
				...
				if itemValue >= 1 then
					takeItemFromSlot(source, itemSlot)
					giveItem(source, itemID, itemValue-1)
					triggerEvent("tamir->aracTamirEt", source, source, 1)
					--outputChatBox("#575757Melancia:#ffffff Aracınızı başarıyla tamir ettirdiniz.", source, 0, 255, 0, true)
				else
					takeItemFromSlot(source, itemSlot)
				end
			end
       ...
       elseif itemID == 181 then
    	...
    	elseif (itemID == 182) then
            if hasItem( source, 107 ) then -- If da player has a lighter, how else would he smoke it dawg
                exports.mrp_global:sendLocalMeAction(source, "lights up a rolled joint.")
                ...
                exports.mrp_global:sendLocalMeAction(source, "bir sarma ruloya bakar.")
                outputChatBox("[!] #ffffffSarma sigaranızı yakmak için çakmağa ihtiyacınız var.", source, 255, 0, 0, true)
            end
		...
		elseif (itemID==210) then -- Coca-Cola Christmas Edition
			...
			exports.mrp_bone_attach:attachElementToBone(bottleObj,source,11,0,0,0,0,0,0)
			thePlayer = source
			setTimer(function()
				exports.mrp_global:applyAnimation(thePlayer, "VENDING", "VEND_Drink_P", 4000, false, true, true)
				toggleAllControls(thePlayer, true, true, true)
				triggerClientEvent(thePlayer, "onClientPlayerWeaponCheck", thePlayer)
				exports.mrp_global:sendLocalMeAction(thePlayer, "drinks a coke.")
		...
		elseif (itemID==213) then --pinnekjott
			...
			takeItemFromSlot(source, itemSlot)
			--local thisAnimFunc = exports.mrp_global:applyAnimation
			setTimer(function()
				exports.mrp_global:applyAnimation(source, "BSKTBALL", "BBALL_Jump_Shot", 4000, false, true, true)
			end, 5000, 1)
			if getResourceState(getResourceFromName("mrp_xmas")) == "running" then
		...
		elseif (itemID>=452) and (itemID<=503) then
			exports["mrp_wearable-system"]:triggerWearableSystem(source, itemID, itemSlot)
			...
			if not (getElementData(source, "Atatürk Bayrağı")) then
				...
				if texture then
					for k,v in ipairs(texture) do
						exports["mrp_item_texture"]:addTexture(bayrak, v[2], v[1])
					end
				end
				--exports.mrp_bone_attach:attachElementToBone(bayrak, source, 11, 0, 0, 0, 0, 180, 100)
				exports.mrp_bone_attach:attachElementToBone(bayrak, source, 12, 0, 0, 0, 0, 200, 30)
				...
				exports.mrp_bone_attach:detachElementFromBone(bayrak)
		...
		if(hasItem) then
			...
			if badges[itemID] then
				if ( getElementData( source, badges[itemID][1] ) ) then
					exports.mrp_anticheat:changeProtectedElementDataEx(source, badges[itemID][1], false, true)
					if pedname then
						--exports.mrp_global:sendLocalMeAction(source, "removes " .. badges[itemID][2] .. ".")
						exports.mrp_global:sendLocalText(source, "* "..tostring(pedname)..", "..badges[itemID][2].." isimli şeyi çıkartır.", 255, 51, 102, 5)
					end
				else
					for key, badge in pairs ( badges ) do
						if key ~= itemID then
							if ( getElementData ( source, badge[1] ) ) then
								exports.mrp_anticheat:changeProtectedElementDataEx( source, badge[1], false, true )
								if pedname then
									--exports.mrp_global:sendLocalMeAction( source, "removes " .. badge[2] .. "." )
									exports.mrp_global:sendLocalText(source, "* "..tostring(pedname)..", "..badge[2].." isimli şeyi çıkartır.", 255, 51, 102, 5)
								end
							end
						end
					end

					exports.mrp_anticheat:changeProtectedElementDataEx( source, badges[itemID][1], removeOOC(tostring(itemValue)), true )
					if pedname then
						--exports.mrp_global:sendLocalMeAction( source, "puts on " .. badges[itemID][2] .. "." )
						exports.mrp_global:sendLocalText(source, "* "..tostring(pedname).." puts on "..badges[itemID][2]..".", 255, 51, 102, 5)
					end
					return true
				end
				--exports.mrp_global:updateNametagColor(source)
			elseif masks[itemID] then
				local data = masks[itemID]
				local mask = getElementData(source, data[1])

				if mask then
					--exports.mrp_global:sendLocalMeAction(source, data[3] .. ".")
					exports.mrp_anticheat:changeProtectedElementDataEx(source, data[1], false, true)
					if pedname then
						exports.mrp_global:sendLocalText(source, "* "..tostring(pedname).." "..data[3]..".", 255, 51, 102, 5)
					end
					return false
				elseif getPedOccupiedVehicle(source) and getElementData(getPedOccupiedVehicle(source), "job") > 0 and getVehicleOccupant(getPedOccupiedVehicle(source)) == source then
					--outputChatBox("You can't use this in a Civilian vehicle.", source, 255, 0, 0)
					...
					--exports.mrp_global:sendLocalMeAction(source, data[2] ..".")
					exports.mrp_anticheat:changeProtectedElementDataEx(source, data[1], true, true)
					if pedname then
						exports.mrp_global:sendLocalText(source, "* "..tostring(pedname).." "..data[2]..".", 255, 51, 102, 5)
					end
					return true
				end
			else
...
function explodeFlash(obj)
	local players = exports.mrp_global:getNearbyElements(obj, "player")
...
function destroyItem(itemID)
	...
	if itemID and itemID > 0 then
		...
		if item then
			...
			if itemID == 134 then
				exports.mrp_global:takeMoney(source, itemValue)
			end
		...
		if itemID == -100 then
			...
			exports.mrp_global:takeWeapon(source, tonumber(-itemID))
			itemName = getWeaponNameFromID( -itemID )
		end
	end
	outputChatBox("#575757Melancia:#f9f9f9 " .. itemName .. " isimli nesneyi çöp kutusuna bıraktın.", source, 255, 194, 14, true)
...
function showItem(itemName)
	if isPedDead(source) or getElementData(source, "injuriedanimation") then return end
	triggerEvent('sendAme', source, "Herkese "..(isVowel(itemName) and "an" or "a").." " .. removeOOC(itemName) .. " nesnesini gösterir.")
end
...
function showInventoryRemote(thePlayer, commandName, targetPlayer)
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) then
		if not (targetPlayer) then
			...
			local targetPlayer = exports.mrp_global:findPlayerByPartialNick(thePlayer, targetPlayer)
...
function isWatchingTV( player )
	return exports['mrp_freecam']:isPlayerFreecamEnabled( player )
end
...
function toggleBadge(source, badges, itemID , itemValue)
	if ( getElementData( source, badges[itemID][1] ) ) then
		...
		if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 or itemID == 158 or itemID == 168 then
			triggerEvent('sendAme',  source, "" .. badges[itemID][1] .. " isimli öğeyi kafasından çıkartır." )
		else
			triggerEvent('sendAme', source, "" .. badges[itemID][1] .. " isimli öğeyi çıkartır.")
		end
	else
		for key, badge in pairs ( badges ) do
			if key ~= itemID then
				if ( getElementData ( source, badge[1] ) ) then
					exports.mrp_anticheat:changeProtectedElementDataEx( source, badge[1], false, true )
					if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 or itemID == 158 or itemID == 168 then
					triggerEvent('sendAme',  source, "" .. badge[1] .. " isimli öğeyi kafasından çıkartır." )
					else
					triggerEvent('sendAme',  source, "" .. badge[1] .. " isimli öğeyi çıkartır." )
					end
				end
			end
		end
		...
		if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 or itemID == 158 or itemID == 168 then
		exports.mrp_anticheat:changeProtectedElementDataEx( source, badges[itemID][1], true )
		triggerEvent('sendAme',  source, "" .. badges[itemID][1] .. " kafasına giydirir." )
		else
			if getElementData(source, "spamblock") then outputChatBox("#575757Melancia:#f9f9f9 İşlem yaparken biraz yavaş olmalısın. (15 sn)", source, 255, 0, 0, true) return end
			exports.mrp_anticheat:changeProtectedElementDataEx( source, badges[itemID][1], removeOOC(itemValue), true )
			setElementData(source, "spamblock", true)
			setTimer(function()
			triggerEvent('sendAme',  source, " yavaşça rozeti yakasına doğru götürür." )
			end, 500, 1)
			setTimer(function()
			triggerEvent('sendAme',  source, "" .. badges[itemID][1] .. " isimli öğeyi yakasına takar." )
			end, 1200, 1)
			setTimer(function()
				setElementData(source, "spamblock", nil)
			end, 15000, 1)
		end
	end
	exports.mrp_global:updateNametagColor(source)
...
function christmasCoke(thePlayer)
	...
	if(#santaCars > 0) then
		...
		for k,v in ipairs(santaCars) do
			...
			if math.random(100) >= thisChance then --if no car won
				...
				exports.mrp_global:sendLocalMeAction(thePlayer, "finds a car key in "..tostring(gendertext).." christmas coca-cola bottle.")
	...
	if math.random(100) >= 40 then
		...
		exports.mrp_global:sendLocalMeAction(thePlayer, "finds a lottery ticket in "..tostring(gendertext).." christmas coca-cola bottle.")
	end
end

function christmasLotteryUseTicket(santa)
	local thePlayer = client
	local hasItem, itemSlot, itemValue = exports.mrp_global:hasItem(thePlayer, santaLotteryTicketID)
	if hasItem then
		...
		if prizes[prize] then
			...
			exports.mrp_global:sendLocalMeAction(santa, "sağ elinde bulunan "..tostring(itemName).." isimli öğeyi "..tostring(playerName).." kişisine verir.")
		else
...
function christmasGetCokeFromSanta(santa)
	...
	local playerName = getPlayerName(thePlayer):gsub("_", " ")
	exports.mrp_global:sendLocalMeAction(santa, "gives "..tostring(playerName).." a coke.")
end
...
-- beyaz yeşil mavi kırmızı
function istiridye(player)
	if exports.mrp_global:takeItem(player, 555) then
		local itemName = ""
		local itemID
		local randomNumber = math.random(0,9)
		exports.mrp_global:sendLocalMeAction(player, "yavaşça sağ eliyle istiridyeyi açar.")
		
		setTimer(function()
			if ((randomNumber==1) or (randomNumber==4) or (randomNumber == 5)) then
				exports.mrp_global:sendLocalDoAction(player, "Elimdeki istiridyemin içerisinden hiç bir şey bulamadım.")
			elseif ((randomNumber==2) or (randomNumber==8)) then
				itemName = "Yeşil İnci"
				itemID = 558
				exports.mrp_global:sendLocalDoAction(player, "Elimdeki istiridyenin içerisinde "..itemName.." bulunmaktadır.")
				exports.mrp_global:giveItem(player, itemID, 1)	
			elseif (randomNumber==3) then
				itemName = "Kırmızı İnci"
				itemID = 556
				exports.mrp_global:sendLocalDoAction(player, "Elimdeki istiridyenin içerisinde "..itemName.." bulunmaktadır.")
				exports.mrp_global:giveItem(player, itemID, 1)		
			...
			elseif (randomNumber==8) then
				...
				outputChatBox("Melancia:#f9f9f9 İstiridyenin içerisi boş çıktı.", player, 75, 75, 75, true)
			end
		end, 1000, 1)
	else
		outputChatBox("Melancia: #f9f9f9Bu komutu kullanabilmek için bir adet istiridyeye ihtiyacın var.", player, 75, 75, 75, true)
	end
end
addCommandHandler("istiridyeac", istiridye)
addEvent("istiridye:open", true)
addEventHandler("istiridye:open", root, istiridye)