...
function canPickup(player, item)
	...
	elseif isProtected(player, item) then
		...
		if isElement(item) then
			if exports['mrp_item_world']:can(player, "pickup", item) then
				return true
			else
				return false
			end
...
function canMove(player, item)
	...
	elseif isProtected(player, item) then
		...
		if isElement(item) then
			if exports['mrp_item_world']:can(player, "move", item) then
				return true
			else
				return false
			end
...
function protectItem(faction, item, slot)
	...
	if getElementData(source, "itemID") then
		...
		if type(faction) == "number" and exports.mrp_global:isStaffOnDuty(client) then
			local protected = getElementData(source, "protected")
			local out = 0
			if protected then
				exports.mrp_anticheat:changeProtectedElementDataEx(source, "protected", false)
				outputChatBox("Unset", client, 0, 255, 0)
				out = 0
			else
				exports.mrp_anticheat:changeProtectedElementDataEx(source, "protected", faction)
...
function dropItem(itemID, x, y, z, ammo, keepammo)
	...
	if not ammo then
		...
		--ANTI ALT-ALT / MAXIME
		if not exports.mrp_global:isStaffOnDuty(source) then
		...
		if itemID == 162 then
			...
			if hoursPlayed < 9999999 then
				...
				exports.mrp_global:sendMessageToAdmins("|| Melancia || BUG Koruma > ".. getPlayerName(client) .. ", Yere Body Armor atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
		...
		if itemID == 56 then
			...
			if hoursPlayed < 9999999 then
				outputChatBox("Bu nesneyi yere atmaniz sunucu tarafindan yasaklanmistir.", source, 255, 0, 0)
				exports.mrp_global:sendMessageToAdmins("|| Melancia || BUG Koruma > ".. getPlayerName(client) .. ", Yere Yüz Maskesi atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.mrp_global:takeMoney(source, itemValue)
		...
		if itemID == 134 then
			...
			if hoursPlayed < 9999999 then
				...
				exports.mrp_global:sendMessageToAdmins("|| Melancia || BUG Koruma > ".. getPlayerName(client) .. ", Yere para atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.mrp_global:takeMoney(source, itemValue)
		...
		if itemID == 254 then
			...
			--outputDebugString("Check")
			if hoursPlayed < 9999999 then
				outputChatBox("#575757Melancia: #ffffffMalesef, bu eşyayı yere atamazsınız.", source, 255, 0, 0, true)
				--outputChatBox("Yetkililere bunu yapmaya calistigin bildirildi.", source, 255, 0, 0)
				--exports.mrp_global:sendMessageToAdmins("|| Melancia || BUG Koruma > ".. getPlayerName(client) .. ", Yere para atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
		...
		if itemID == 116 then
			...
			if hoursPlayed < 9999999 then
				outputChatBox("Bu nesneyi yere atmaniz sunucu tarafindan yasaklanmistir.", source, 255, 0, 0)
				outputChatBox("Yetkililere bunu yapmaya calistigin bildirildi.", source, 255, 0, 0)
				exports.mrp_global:sendMessageToAdmins("|| Melancia || BUG Koruma > ".. getPlayerName(client) .. ", Yere mermi atmaya calisti.")
				return
				triggerClientEvent( source, "finishItemDrop", source )
			else
				exports.mrp_global:takeMoney(source, itemValue)
		...
		if itemID == 115 then
			local val = exports.mrp_global:explode(":", itemValue)
			...
			--exports.mrp_global:sendMessageToAdmins("|| Melancia || BUG Koruma > ".. getPlayerName(client) .. ", Yere silah atmaya calisti.")
			--return
			triggerClientEvent( source, "finishItemDrop", source )
		end
		...
		if ( itemID == 115) then -- Weapons
			local itemCheckExplode = exports.mrp_global:explode(":", itemValue)
			-- itemCheckExplode: [1] = gta weapon id, [2] = serial number, [3] = weapon name
			local weaponDetails = exports.mrp_global:retrieveWeaponDetails( itemCheckExplode[2]  )
		...
		elseif ( itemID == 116) then -- Ammo
			local ammoDetails = exports.mrp_global:explode( ":", itemValue  )
		...
		elseif itemID == 139 then
			...
			--[[if not (hasItem(source, itemID)) then
				...
				exports.mrp_logs:logMessage(haxStr, 32)
				exports.mrp_global:sendMessageToAdmins("AdmWrn: " .. haxStr)		
			end]]
			...
			elseif itemID == 134 then
				if not exports.mrp_global:takeMoney(source, itemValue) then
					outputDebugString(getPlayerName(source) .. ' (' .. getPlayerSerial(source) .. ') tried dropping money he doesnt have')

					while exports['mrp_items']:takeItem(source, 134) do
					end

					local amount = exports.mrp_global:getMoney(source)
					if amount > 0 then
						exports.mrp_global:giveItem(source, 134, amount)
					end
			...
			elseif (itemID == 147) then -- Picture Frame
				local dimension = getElementDimension(source)
				if dimension > 0 or (exports.mrp_integration:isPlayerDeveloper(source) and exports.mrp_global:isAdminOnDuty(source)) or exports.mrp_integration:isPlayerScripter(source) then
					local split = exports.mrp_global:explode(";", itemValue)
					local url = split[1]
					local texture = split[2]
					if url and texture then
						if exports['mrp_textures']:newTexture(source, url, texture) then
			...
			elseif itemID == 169 then --Keypad Door Lock / Maxime
				...
				if not validInterior then
					exports.mrp_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "The interior that this device is made for is no longer existed. Thus far this device is not usable anymore.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end

				if not validDropper then
					exports.mrp_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "Interior '"..interiorName.."' is no longer owned by you. Thus far this device is not usable anymore.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end
				...
				if isIntLocked then
					exports.mrp_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "Interior '"..interiorName.."' is locked, please unlock it first.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end
				...
				if not doorOutside or not doorInside then
					exports.mrp_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "Interior '"..interiorName.."' is defected, please notify scripters.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end
				...
				if interior == doorOutside[4] and dimension == doorOutside[5] then
					if getDistanceBetweenPoints3D(x,y,z,doorOutside[1], doorOutside[2], doorOutside[3]) > maxRange then
						exports.mrp_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "This device can only be installed within "..math.floor(maxRange/2).." meters from its out-door.")
						triggerClientEvent( source, "finishItemDrop", source )
						return false
					end
				...
				elseif interior == doorInside[4] and dimension == tonumber(itemValue) then
					if getDistanceBetweenPoints3D(x,y,z,doorInside[1], doorInside[2], doorInside[3]) > maxRange then
						exports.mrp_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "This device can only be installed within "..math.floor(maxRange/2).." meters from its in-door.")
						triggerClientEvent( source, "finishItemDrop", source )
						return false
					end
				else
					exports.mrp_hud:sendBottomNotification(source, getItemName( itemID, itemValue ), "This device can only be installed within "..math.floor(maxRange/2).." meters from its door.")
					triggerClientEvent( source, "finishItemDrop", source )
					return false
				end
			end
			...
			if insert then
				...
				-- Animation
				exports.mrp_global:applyAnimation(source, "CARRY", "putdwn", 500, false, false, true)
				...
				if itemID == 134 then
					outputChatBox("" .. exports.mrp_global:formatMoney(itemValue) .. "$ yere bırakır.", source, 255, 194, 14)
				...
				local obj = exports['mrp_item_world']:createItem(id, itemID, itemValue, modelid, x, y, z + zoffset - 0.05, rx, ry, rz+rz2)
				...
				if objTexture then
					for objTexKey, objTexVal in ipairs(objTexture) do
						exports['mrp_item_texture']:addTexture(obj, objTexVal[2], objTexVal[1])
					end
				end
				
				exports.mrp_anticheat:changeProtectedElementDataEx(obj, "creator", getElementData(source, "dbid"), false)

				local permissions = { use = 1, move = 1, pickup = 1, useData = {}, moveData = {}, pickupData = {} }
				exports.mrp_anticheat:changeProtectedElementDataEx(obj, "worlditem.permissions", permissions)
				...
				--exports.mrp_logs:logMessage( getElementData(source, "account:username").."\\"..getPlayerName(source) .. " -> ~World #" .. getElementID(obj) .. " - " .. getItemName( itemID ) .. " - " .. itemValue, 17)
				exports.mrp_logs:dbLog(source, 39, source, getPlayerName(source) .. " -> ~World #" .. getElementID(obj) .. " - " .. getItemName( itemID ) .. " - " .. itemValue )
				...
				if itemID == 134 then
					triggerEvent('sendAme', source, "" .. exports.mrp_global:formatMoney(itemValue) .. "$ bırakır.")
		...
		if tonumber(getElementData(source, "duty")) > 0 then
			outputChatBox("#575757Melancia: #ffffffSilahlarınızı görev başındayken bırakamazsınız.", source, 255, 0, 0, true)
		elseif tonumber(getElementData(source, "job")) == 4 and itemID == 41 then
			outputChatBox("#575757Melancia: #ffffffBu sprey kutusunu bırakamazsın.", source, 255, 0, 0, true)
		else
			...
			--[[local totalAmmoPosessed = exports.mrp_global:getWeaponCount(source, itemID) 
			...
			if query then
				...
				exports.mrp_global:takeWeapon(source, itemID)
				if keepammo then
					exports.mrp_global:giveWeapon(source, itemID, keepammo)
				end
				...
				local obj = exports['mrp_item_world']:createItem(id, -itemID, ammo, modelid, x, y, z - 0.4, 75, -10, rz2)
				exports.mrp_pool:allocateElement(obj)
				...
				exports.mrp_logs:dbLog(source, 39, source, getPlayerName(source).. " -> ~World #" .. getElementID(obj) .. " - " .. getItemName( -itemID ) .. " - " .. ammo)
			end
		end
	end
...
function doItemGiveawayChecks(player, itemID, itemValue)
	...
	if mask and getElementData(source, mask[1]) and not hasItem(source, itemID) then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(source, mask[1])
	end
	...
	--if itemID == 16 and not hasItem(source, 16, getPedSkin(source)) then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(source, 'clothing:id', nil, true)
	...
	if badge and getElementData(source, badge[1]) and not hasItem(source, itemID, removeOOC(getElementData(source, badge[1]))) then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(source, badge[1])
...
local function moveItem(item, x, y, z)
	...
	if not exports.mrp_integration:isPlayerTrialAdmin(source) and (itemID == 81 or itemID == 103) then
		return
	end
	
	if itemID == 138 then
		if not exports.mrp_integration:isPlayerAdmin(source) then
			outputChatBox("Only a Lead+ admin can move this item.", source, 255, 0, 0)
			return
		end
	end
	
	if itemID == 139 and not exports.mrp_integration:isPlayerTrialAdmin(source) then
...
addEventHandler('item:move:save', root,
	function(x, y, z, rx, ry, rz)
		...
		if exports.mrp_global:isStaffOnDuty(client) or exports.mrp_integration:isPlayerDeveloper(client) or exports.mrp_integration:isPlayerScripter(client) then
			...
			local itemID = getElementData(source, "itemID")
			if itemID == 138 and not exports.mrp_integration:isPlayerDeveloper(client) then
				outputChatBox("Only a Lead+ admin can move this item.", client, 255, 0, 0)
				reset()
				return
			end
			
			if itemID == 139 and not exports.mrp_integration:isPlayerDeveloper(client) then
...
local function rotateItem(item, rz)
	if not exports.mrp_integration:isPlayerTrialAdmin(source) then
		return
	end
...
function pickupItem(object, leftammo)
	...
	exports.mrp_anticheat:setEld(object, "transfering", true, "none") --No sync at all.
	--if true then return end
	-- Inventory Tooltip
	if (getResourceFromName("mrp_tooltips"))then
	...
	if not canPickup(source, object) then
		outputChatBox("#575757Melancia: #ffffffBu öğeyi alamazsınız. F2 ile bir yetkiliye başvurun.", source, 255, 0, 0, true)
		exports.mrp_anticheat:changeEld(object, "transfering", nil)
		return
	end
	...
	if ((itemID >= 31) and itemID <= 43) or itemBannedByAltAltChecker[itemID] then
		...
		if hoursPlayedTo and hoursPlayedTo < 10 and not exports.mrp_global:isStaffOnDuty(source) then
			outputChatBox("#575757Melancia: #ffffff"..getItemName( itemID ).." yerden alabilmek için 10 saat oynaman olmalıdır.", source, 255, 0, 0, true)
			exports.mrp_anticheat:changeEld(object, "transfering", nil)
			return false
		end
		...
		if creator ~= picker then
			local accountCreator = exports.mrp_cache:getAccountFromCharacterId(creator)
			if tonumber(accountCreator.id) == getElementData(source, "account:id") then
				outputChatBox("#575757Melancia: #ffffffVarlıkların aynı hesaptaki karakterleri veya bir oyuncunun sahip olduğu çoklu hesaplar arasında aktarılması kesinlikle yasaktır! ", source, 255, 0, 0, true)
				exports.mrp_global:sendMessageToAdmins("[ANTI-ALT->ALT]: Detected illegal assets transferring on account name '"..getElementData(source, "account:username").."'.")
				exports.mrp_global:sendMessageToAdmins("[ANTI-ALT->ALT]: Suspect is trying to transfer a "..getItemName( itemID ).." between alternatives.")
				exports.mrp_logs:dbLog(source, 5, {object}, "TRIED TO ALT-ALT " .. getItemName( itemID ) .. " between alternatives.")
				exports.mrp_anticheat:changeEld(object, "transfering", nil)
				return false
			end
		end
	...
	if itemID == 115 then
		if isThisGunDuplicated(itemValue, source) then
			destroyElement(object)
			dbExec(mysql:getConnection(), "DELETE FROM worlditems WHERE id='" .. id .. "'")
			outputChatBox("#575757Melancia: #ffffffSilah çoğaltılması tespit edilerek silahınız silindi, üzgünüz.", source, 255,0,0, true)
			return false
		end
	end
	...
	if itemID == 138 then
		if not exports.mrp_integration:isPlayerAdmin(source) then
			outputChatBox("#575757Melancia: #ffffffBu öğeyi sadece bir yetkili alabilir.", source, 255, 0, 0, true)
			exports.mrp_anticheat:changeEld(object, "transfering", nil)
			return false
		end
	end
	...
	exports.mrp_global:applyAnimation(source, "CARRY", "liftup", 600, false, true, true)
	if itemID > 0 then
		dbExec(mysql:getConnection(), "DELETE FROM worlditems WHERE id='" .. id .. "'")
		...
		if itemID == -100 then
			...
			if leftammo and itemValue > leftammo then
				...
				exports.mrp_anticheat:changeProtectedElementDataEx(object, "itemValue", itemValue)
				dbExec(mysql:getConnection(), "UPDATE worlditems SET itemvalue=" .. itemValue .. " WHERE id=" .. id)
				itemValue = leftammo
			else
				dbExec(mysql:getConnection(), "DELETE FROM worlditems WHERE id='" .. id .. "'")
			end
			exports.mrp_global:giveWeapon(source, -itemID, itemValue, true)
		end
	end

	exports.mrp_logs:dbLog(source, 39, {object}, "~World #" ..getElementID(object) .. "->" .. getPlayerName(source) .. " - " ..getItemName(itemID).. " - " ..itemValue)
	destroyElement(object)
	
	if itemID == 134 then -- money
		exports.mrp_global:giveMoney(source, itemValue)
		outputChatBox("" .. exports.mrp_global:formatMoney(itemValue) .. "$ alır.", source, 255, 194, 14)
		triggerEvent('sendAme', source, "" .. exports.mrp_global:formatMoney(itemValue) .. "$ alır.")
	elseif itemID == 178 then
		local yup = split(itemValue, ":")
...
function removeItemTransferingState()
	for i, object in pairs(exports.mrp_pool:getPoolElementsByType("object")) do
		exports.mrp_anticheat:changeEld(object, "transfering", nil)
	end
end
