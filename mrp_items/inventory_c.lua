...
local function getHoverElement( resource, force )
	...
	for _, acceptProtected in ipairs({false, true}) do
		...
		if element and getElementParent(getElementParent(element)) == getResourceRootElement(getResourceFromName(resource or "mrp_item_world")) then
			return element
		elseif b and c and d and not resource then
			element = nil
			local x, y, z = nil
			local maxdist = 0.34
			for key, value in ipairs(getElementsByType("object",getResourceRootElement(getResourceFromName(resource or "mrp_item_world")))) do
...
local function getOverlayText( itemID, itemValue, isGrouped ) -- envanter
	...
	--elseif itemID == 134 then -- Money / MAXIME
		--text = "$"..exports.mrp_global:formatMoney(itemValue)
		...
		elseif not guiGetInputEnabled( ) and not isMTAWindowActive( ) and isCursorShowing( ) then
					...
					if not isCursorOverInventory then
						...
						elseif hoverElement then
							...
							elseif getElementType( hoverElement ) == "ped" and  getElementData(hoverElement,"customshop") then --  import products to custom shop
								color = background_movetoelement_color
							elseif getElementParent(getElementParent(hoverElement)) ~= getResourceRootElement(getResourceFromName("mrp_item_world")) or hoverElement == getHoverElement() then
								color = full_color
							elseif tonumber(getElementData(hoverElement, "itemID")) == 166 and item[1] == 165 then -- Video disc into video player /Exciter
								color = background_movetoelement_color
							else
			...
			elseif not tooltipYet and getPedOccupiedVehicle(localPlayer) then
				...
				if element then
					...
					if not any and getElementType(element) == "vehicle" and element ~= getPedOccupiedVehicle( localPlayer ) and exports['mrp_vehicle-system']:hasVehiclePlates( element ) and getDistanceBetweenPoints3D( px, py, pz, getElementPosition( element ) ) < 10 then
...
addEventHandler( "onClientClick", getRootElement( ),
	function( button, state, cursorX, cursorY, worldX, worldY, worldZ )
		if not waitingForItemDrop then
			if button == "left" or ( button == "middle" and exports.mrp_integration:isPlayerTrialAdmin( getLocalPlayer( ) ) ) then
						...
						elseif not clickItemSlot.group then
							...
							elseif hoverAction == ACTION_SHOW then
								...
								elseif item[1] == 150 then--ATM card
									...
									local owner = exports.mrp_cache:getCharacterNameFromID(itemExploded[2])
							...
							elseif hoverAction == ACTION_SPLIT then
								...
								elseif itemID == 115 then
									if exports.mrp_integration:isPlayerTrialAdmin(localPlayer) then -- restrict this to trial admin+ / maxime
										renameWeapon(item)
									else
				...
				elseif button == "left" and isCursorOverInventory and hoverAction and state == "down" then
					if show then
						...
						elseif activeTab == hoverAction then
							show = false
							showCursor( false )
							--exports["mrp_realism-system"]:showSpeedo()
						else
							activeTab = hoverAction
						end
					else
...
addEventHandler("finishItemDrop", getLocalPlayer(),
	function( )
		waitingForItemDrop = false
		inventory = false
		--playSound(":mrp_resources/item_drop.mp3")
	end
)
...
function renameWeapon(item)
	...
	if (checkString == " (D)")  then
		...
		guiEditSetMaxLength(nameEdit, 25)
			if weaponName ~= name and not exports.mrp_integration:isPlayerAdmin(localPlayer) then 
		...
		elseif calibre and string.len(calibre) > 3 and not exports.mrp_integration:isPlayerAdmin(localPlayer) then
			guiSetEnabled(calibreEdit, false)
		end
		if calibre then
...
function playSoundInvOpen()
	setSoundVolume(playSound(":mrp_resources/inv_open.mp3"), 0.3)
end

function playSoundInvClose()
	setSoundVolume(playSound(":mrp_resources/inv_close.mp3"), 0.3)
end