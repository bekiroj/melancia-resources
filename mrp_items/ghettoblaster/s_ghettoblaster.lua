...
local function updateWorldItemValue(item, station, volume)
    if station < 0 or station > #exports.mrp_carradio:getStreams() then return end
    ...
    exports.mrp_anticheat:changeProtectedElementDataEx(item, "itemValue", newvalue)
    dbExec(exports.mrp_mysql:getConnection(), "UPDATE worlditems SET itemvalue='" .. newvalue .. "' WHERE id=" .. getElementData(item, "id"))
    triggerClientEvent("toggleSound", item)
end

function changeTrack(item, step)
    local streams = exports.mrp_carradio:getStreams()
	...
	if not tuned[item] then
		exports.mrp_global:sendLocalMeAction(source, "retunes the Ghettoblaster.")
		tuned[item] = true
	else
