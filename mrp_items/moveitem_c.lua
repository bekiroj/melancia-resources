...
local function forceUpdate( )
	...
	if getElementType(element)=="interior" then
		...
		if not hasKey(getElementData(element, "dbid"), element) and interiorType~=2 then
			...
			if interiorType==2 then
				if (not exports.mrp_factions:isInFactionType(getLocalPlayer(), 2) and not exports.mrp_factions:isInFactionType(getLocalPlayer(), 3) and getElementData(getLocalPlayer(), "duty_admin") ~= 1) then
					local row = guiGridListAddRow(gElementItems)
	...
	if getElementType( element ) == "vehicle" then
		...
		if exports.mrp_integration:isPlayerTrialAdmin(getLocalPlayer()) then
...
function hasKey( key, element )
	...
	if exports.mrp_global:hasItem(getLocalPlayer(), 4, key) or exports.mrp_global:hasItem(getLocalPlayer(), 5,key) then
		return true, false
	elseif getElementData(getLocalPlayer(), "duty_admin") == 1 then
		...
		if interiorType==2 then
			if (not exports.mrp_factions:isInFactionType(getLocalPlayer(), 2) and not exports.mrp_factions:isInFactionType(getLocalPlayer(), 3)) then
				return false, false
			else
				return true, true
			end
...
local function moveFromElement( button )
	...
	if button == "left" and col ~= -1 and row ~= -1 then
		...
		if slot < 0 --[[and exports.mrp_integration:isPlayerTrialAdmin(localPlayer)]] then
			...
			if item then
				...
				if (hasKey(getElementData(element, "dbid"), element) and getElementType(element)=="interior") or getElementType(element)~="interior" then
					if itemID < 0 and itemID ~= -100 then -- weapon
						local free, totalfree = exports.mrp_weaponcap:getFreeAmmo( -itemID )
						local cap = exports.mrp_weaponcap:getAmmoCap( -itemID )
						if totalfree == 0 then
...
local function adminCheck( button )
	...
	if button == "right" and col ~= -1 and row ~= -1 then
		...
		if exports.mrp_global:isStaffOnDuty(getLocalPlayer()) then
...
local function openElementInventory( ax, ay )
	if not getElementData(localPlayer, "exclusiveGUI") then
		...
		local w, h = 500, 500
		elementtype = getElementModel(source) == 2147 and "Fridge" or getElementModel(source) == 743 and "Saksi" or getElementModel(source) == 3761 and "Shelf" or (( getElementType(source) == "vehicle" and "Vehicle" ) or ( getElementType(source) == "interior" and "Mailbox" ) or getElementModel(source) == 2332 and "Safe" or false )
		if not elementtype then
			if getElementParent(getElementParent(source)) == getResourceRootElement(getResourceFromName("mrp_item_world")) then
		...
		wInventory = guiCreateStaticImage(ax-w/2, ay-h/2, w, h, ":mrp_resources/window_body.png",false)
		outputChatBox("[-]#f9f9f9 Güvenlik sebebiyle 60 saniye sonra kasa ekranın otomatik olarak kapanacak.", 116, 175, 252, true)
		lYou = guiCreateLabel(0.05, 0.1, 0.45, 0.05, "SEN "..getCarriedWeight(localPlayer).."/"..getMaxWeight(localPlayer).."kg", true, wInventory)
		...
		bCloseInventory = guiCreateButton(0.05, 0.9, 0.9, 0.075, "Arayüzü Kapat", true, wInventory)
		...
		showCursor( true )
		setTimer( function()
			outputChatBox("[-]#f9f9f9 60 saniye geçtiği için kasa otomatik olarak kapatıldı.", 255, 0, 0, true)
			hideMenu()
			setElementData(localPlayer, "exclusiveGUI", false, false)
		...
		outputChatBox("[-]#f9f9f9 Bu kasaya erişemezsiniz.", 255, 0, 0, true)
	end
end
...
function collisionFix()
	for key, value in pairs( getElementsByType( "object", getResourceRootElement( getResourceFromName("mrp_item_world") ) ) ) do
