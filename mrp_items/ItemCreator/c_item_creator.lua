...
function showItemList()
	if (exports.mrp_integration:isPlayerSeniorAdmin(localPlayer) or exports.mrp_integration:isPlayerTrial(localPlayer) or exports.mrp_integration:isPlayerScripter(localPlayer)) then
		if not (wItemsList) then
			local width, height = 800,600
			local scrWidth, scrHeight = guiGetScreenSize()
			local x = scrWidth/2 - (width/2)
			local y = scrHeight/2 - (height/2)
			wItemsList = guiCreateWindow(x, y, width, height,"Envanter Eşya Yönetim Arayüzü | © Melancia",false)
			...
			bSpawnClose = guiCreateButton(569,45,111,23,"Üret & Kapat",false,wItemsList)
					...
					if (row==-1) or (col==-1) then
						...
						if id then
							...
							if setClipboard (copyingContent ) then
								outputChatBox("Pano kopyalandı - '"..copyingContent.."'")
							end
						else
