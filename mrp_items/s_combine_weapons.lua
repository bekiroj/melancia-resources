		...
		if (itemCheck[1] == 115) then -- Weapon
			...
			if firstClipBullets == 0 then
				...
				if not (getElementData(thePlayer,  "cf:"..gunDetails[1])) then
					exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "cf:"..gunDetails[1], true, false, false)
				end
			else
				if (getElementData(thePlayer,  "cf:"..gunDetails[1])) then
					exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "cf:"..gunDetails[1], false, false, false)
				end
			end
...
function reloadA(thePlayer,weapon,ammocalc)
	...
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reloading", true, false, false)
	setTimer(checkFalling, 100, 10, thePlayer)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "cf:"..weapon, false, false, false)
	if not (isPedDucked(thePlayer)) and not isPedInVehicle(thePlayer) then
		exports.mrp_global:applyAnimation(thePlayer, "BUDDY", "buddy_reload", 1000, false, true, true)
		...
		if not foundGun then
			for _, clientWeaponRow in ipairs(ammoTable) do
				if tonumber(clientWeaponRow[1]) == tonumber(weaponID) then
					...
					--exports.mrp_global:sendMessageToAdmins("[AntiCheat] 0x00000010 detected on "..getPlayerName(client) .. " (has clientside weapon ("..tostring(weaponID)..") that dont exist serverside)")
				end
			end
		end
	...
	for i, v in ipairs( SARJORLER ) do
		...
		if (itemCheck[1] == 116) then
			...
			if tonumber(weaponBulletDetails[1]) == weapon and taken < wasted then				
				...
				local userVIP = tonumber(getElementData(client, "vip")) or 0
...
function checkFalling(thePlayer)
	...
	if not (isPedOnGround(thePlayer)) and (reloading) then
		-- reset state
		exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reloading.timer", false, false)
		exports.mrp_global:removeAnimation(thePlayer)
		exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reloading", false, false)
...
function giveReload(thePlayer, weapon, ammo)
	...
	--exports.mrp_global:setWeaponAmmo(thePlayer, weapon, ammo, clipsize) -- fix for the ammo adding up bug
