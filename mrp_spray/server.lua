	...
	if (getElementData(thePlayer, "tamirci" ) == 1) then
		if (getElementInterior(thePlayer) == 107) then
			...
			if komut == "sprey" then
				local para = 150
				local sprey = 41
				if (exports.mrp_global:takeMoney(thePlayer,para)) then
					local serial1 = tonumber(getElementData(thePlayer, "account:character:id"))
					local mySerial = exports.mrp_global:createWeaponSerial( 1, serial1)
					exports.mrp_global:giveItem(thePlayer, 115, sprey..":"..mySerial..":"..getWeaponNameFromID(sprey).."::")
...
if (getElementData(thePlayer, "tamirci" ) == 1) then
	if ( getElementInterior(thePlayer) == 107 ) then
		...
		if (araba) then
		    if (exports.mrp_global:takeMoney(thePlayer, 150)) then
