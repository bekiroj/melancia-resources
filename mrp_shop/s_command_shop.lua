...
function createShop ( id, x, y, z, int, dim, type )
    ...
    exports.mrp_anticheat:changeProtectedElementDataEx(shop, "id", id, false, true)
    exports.mrp_anticheat:changeProtectedElementDataEx(shop, "shop", true, false, true)
    exports.mrp_anticheat:changeProtectedElementDataEx(shop, "type", type, false, true)
end
...
function makeShop ( thePlayer, commandName, type )
    if not exports.mrp_integration:isPlayerLeadAdmin(thePlayer) then    return end
...
function delShop( thePlayer, commandName, shopID )
    if not exports.mrp_integration:isPlayerSeniorAdmin(thePlayer) then return end
                ...
                for k, s in ipairs (shops[type]) do
                    --for k, s in ipairs (shops[i]) do
                        if itemname == s[1]:lower() then
                            if exports.mrp_global:hasMoney(thePlayer, s[2]) then
                                if s[3] == "eat" then
                                    hunger = getElementData(thePlayer, "hunger")
                                    exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "hunger", hunger+s[4])
                                    exports.mrp_global:applyAnimation(thePlayer, "FOOD", "EAT_Burger", 2000, false, true, true)
                                    --break
                                elseif s[3] == "drink" then
                                    ...
                                    exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "thirst", thirst+s[4])
                                    exports.mrp_global:applyAnimation(thePlayer, "BAR", "dnk_stndM_loop", 2000, false, true, true)
                                    --break
                                elseif s[3] == "item" then
                                    exports.mrp_global:giveItem(thePlayer, s[4], s[5])
                                end
                                exports.mrp_global:takeMoney(thePlayer, s[2])

                            break
                            else
                                outputChatBox(exports.mrp_pool:getServerSyntax(false, "w").." Cebindeki bozukluklar mı bitti adamım?.",thePlayer,255, 255, 255, true)
                            end
                        else
                           -- outputChatBox(tostring(s[1]:lower()),thePlayer)
                           -- outputChatBox(exports.mrp_pool:getServerSyntax(false, "w").." girdiğiniz yemek ismi yanlış.",thePlayer,255, 255, 255, true)
                        end
                    --end
                end
            --else
