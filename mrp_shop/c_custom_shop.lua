...
function addItemToShop(source, item, slot, worldItem, npcID, shopElement )
	...
	if tonumber(item[1]) == 16 then --clothes
		local value = exports.mrp_global:explode(":",tostring(item[2]))
		local skinid = tonumber(value[1]) or 1
		itemURL = ":mrp_auth/img/" .. ("%03d"):format( skinid or 1 ) .. ".png"
	...
	local itemName = exports["mrp_items"]:getItemName( tonumber(item[1]), tostring(item[2]) ) or ""
	local itemValue = ""
	if not exports["mrp_items"]:getItemHideItemValue(tonumber(item[1])) then
		itemValue = exports["mrp_items"]:getItemValue( tonumber(item[1]), tostring(item[2]) ) or ""
	end
	...
	local mDesc = guiCreateMemo(147,99,280,56,exports["mrp_items"]:getItemDescription( tonumber(item[1]), tostring(item[2]) ) or "",false,wAddingItemsToShop)
...
function customShopBuy(products, proID, shopElement)
	...
	local ePrice = guiCreateLabel(189,62,238,20,exports.mrp_global:formatMoney(itemPrice),false,wCustomShopBuy)
...
addEventHandler("shop:playPayWageSound", root,
	function()
		setSoundVolume(playSound("files/sounds/playPayWageSound.mp3"), 0.2)
	end
)
...
addEventHandler("shop:playCollectMoneySound", root,
	function()
		setSoundVolume(playSound("files/sounds/playCollectMoneySound.mp3"), 0.2)
	end
)
...
addEventHandler("shop:playBuySound", root,
	function()
		playSound("files/sounds/playBuySound.mp3")
	end
)