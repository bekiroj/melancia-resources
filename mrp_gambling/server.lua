...
function invite(player, cmd, arg, betAmount)
    ...
    if tonumber(arg) then
        local invitedPlayer = exports.mrp_global:findPlayerByPartialNick(player, arg)
        ...
        if not exports.mrp_global:hasMoney(player, betAmount) then
             outputChatBox("[!] #FFFFFFBahis yaptığınız kadar para üzerinizde yok.", player, 255, 0, 0, true)
            return
        end
		
		if not exports.mrp_global:hasMoney(invitedPlayer, betAmount) then
...
function spin(player)
    ...
    if not exports.mrp_global:hasMoney(player, getElementData(player, "betAmount")) then
    ...
    for i, player in ipairs({player, opponent}) do
        ...
        if exports.mrp_global:hasMoney(player, getElementData(player,"betAmount")) then
            exports.mrp_global:takeMoney(player,getElementData(player,"betAmount"))
        else
        ...
        if winner then
            ...
            if winner == player then
                exports.mrp_global:giveMoney(winner,getElementData(player,"betAmount")*2)
            end
        else
            outputChatBox("[!] #FFFFFFBu raundu kimse kazanamadı (berabere)", player, 0, 55, 255, true)
            exports.mrp_global:giveMoney(player, getElementData(player, "betAmount"))
        end
