...
function triggerLiftGUI(key, keyState, pickup)
	--outputDebugString("elevators/c_elevator_lift: triggerLiftGUI")
	if(keyState == "down" and not wElevator) then
		--outputDebugString("elevators/c_elevator_lift: triggerLiftGUI is OK")
		local px,py,pz = getElementPosition(getLocalPlayer())
...
function gotoFloor(button, state)
	if (button=="left") then
		...
		if (row==-1) or (col==-1) then
			...
			local data = exports.mrp_global:split(data, ",")
