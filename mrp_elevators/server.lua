...
function createElevator(thePlayer, commandName, oneway)
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerScripter(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer)) then
		...
		if id then
			...
			if (query) then
				...
				exports.mrp_global:giveItem(thePlayer, 73, id)
...
function createElevatorWithFriend(thePlayer, commandName, targetPlayer)
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer)) then
		if not (targetPlayer) then
			...
			local targetPlayer, tragetPlayerName = exports.mrp_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			local x, y, z = getElementPosition(thePlayer)
			local interiorwithin = getElementInterior(thePlayer)
			local dimensionwithin = getElementDimension(thePlayer)
			...
			for index, row in ipairs(res) do
				...
				if massLoad then
					...
					if loadedElevators == 1 or loadedElevators == stats_numberOfElevators or initializeSoFarDetector ~= newInitializeSoFarDetector then
						triggerClientEvent("elevator:initializeSoFar", root, loadedElevators, stats_numberOfElevators)
				...
				exports.mrp_pool:allocateElement(elevatorElement, tonumber(row.id), true)
			end
		end
	)
	return true
...
function enterElevator(goingin)
	...
	if ( ( vehicle and elevatorStatus[INTERIOR_TYPE]  ~= 0 and getVehicleOccupant( vehicle ) == player ) or not vehicle ) then
		...
		local dbid, entrance, exit, interiorType, interiorElement  = call( getResourceFromName( "mrp_interiors" ), "findProperty", player, otherCP[INTERIOR_DIM] )
		if dbid > 0 then

		else
			dbid, entrance, exit, interiorType, interiorElement  = call( getResourceFromName( "mrp_interiors" ), "findProperty", player, currentCP[INTERIOR_DIM] )
		end
		...
		if vehicle then
			...
			if interiorElement and isElement(interiorElement) and getElementType(interiorElement) == "interior" then
				exports.mrp_anticheat:changeProtectedElementDataEx(interiorElement, "lastused", exports.mrp_datetime:now(), true)
				dbExec(mysql:getConnection(),"UPDATE interiors SET lastused=NOW() WHERE id="..dbid)
				exports.mrp_logs:dbLog("SYSTEM", 31, { interiorElement, player, vehicle } , "ENTERED/EXITED")
				exports["mrp_interior_manager"]:addInteriorLogs(dbid, "ENTERED/EXITED", player)
			end
		elseif isElement(player) then
			if movingInSameInt then
				...
				exports["mrp_interiors"]:setPlayerInsideInterior(interiorElement, player, otherCP,movingInSameInt)
			end
			return true
		else
...
function warpVehicleIntoInteriorfunction(vehicle, interior, dimension, offset, x,y,z,pickup,other, interiorElement,movingInSameInt)
	if isElement(vehicle) then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(vehicle, "health", getElementHealth(vehicle), false)
		...
		setTimer(function ()
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(vehicle, "health")
...
function deleteElevator(thePlayer, commandName, id)
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer) or exports.mrp_integration:isPlayerScripter(thePlayer)) or commandName == "PROPERTYCLEANUP" then
...
function getNearbyElevators(thePlayer, commandName)
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer) or exports.mrp_integration:isPlayerScripter(thePlayer)) then
...
function delElevatorsFromInterior(thePlayer, commandName, intID)
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer)) or commandName == "PROPERTYCLEANUP" then
		if (not tonumber(intID) or tonumber(intID)%1~= 0 ) and commandName ~= "PROPERTYCLEANUP" then
			...
			if tonumber(intID) == 0 and not exports.mrp_integration:isPlayerSeniorAdmin(thePlayer) then
...
addEventHandler( "toggleCarTeleportMode", getRootElement(),
	function( player )
		...
		if query then
			...
			exports.mrp_anticheat:changeProtectedElementDataEx( source, "status", elevatorStatus, false )
			if mode == 0 then
