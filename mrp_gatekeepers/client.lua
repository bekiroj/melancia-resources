...
function konus (element)
	...
	elseif (ped == "Ali Soyyiğit") then
		triggerEvent("shop-panel-ac",localPlayer)
    ...
    elseif (ped=="Lara Sultan") then 
       	triggerServerEvent('clothing:list', getResourceRootElement(getResourceFromName("mrp_clothes")))								
	...
	elseif (ped=="Mike Moore") then
		...
		exports.mrp_hud:sendBottomNotification(getLocalPlayer(),(ped or "NPC").." diyor ki:", "'Sana yardımcı olabileceğim bir konu yok, üzgünüm.'")
	end
end
