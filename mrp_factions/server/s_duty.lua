	...
	if foundPackage and canPlayerUseDutyPackage(thePlayer, foundPackage[1]) then
		...
		for itemIndexID, itemTableContent in ipairs(itemTable) do
			if itemTableContent[2] > 0 then -- its a real item
				exports.mrp_global:giveItem(thePlayer, itemTableContent[2], itemTableContent[3]) 
			else -- Its a weapon :O!
				if itemTableContent[2] == -100 then
					...
					local weaponSerial = exports.mrp_global:createWeaponSerial(2, characterDatabaseID)
					exports.mrp_global:giveItem(thePlayer, 115, gtaWeaponID ..":".. weaponSerial ..":" .. getWeaponNameFromID ( gtaWeaponID ) .. " (D)"  ) 
					
					local gtaweaponcap = exports.mrp_weaponcap:getGTACap(gtaWeaponID)
					...
					if packsToGive > 0 and hasAmmo then
						for i=1,packsToGive do 
							exports.mrp_global:giveItem(thePlayer, 116, gtaWeaponID ..":".. gtaweaponcap ..":Ammopack for " .. getWeaponNameFromID ( gtaWeaponID ) .. " (D)"  ) 
						end
					end
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "duty", grantID, true)
...
function dutyOffduty()
	...
	if tonumber(grantID) > 0 then
		...
		local correction = 0
		local items = exports['mrp_items']:getItems( thePlayer ) -- [] [1] = itemID [2] = itemValue
		for itemSlot, itemCheck in ipairs(items) do
			if (itemCheck[1] == 115) then -- Weapon
				local itemCheckExplode = exports.mrp_global:explode(":", itemCheck[2])
				local serialNumberCheck = exports.mrp_global:retrieveWeaponDetails(itemCheckExplode[2])
				if (tonumber(serialNumberCheck[2]) == 2) then -- /duty spawned
					exports['mrp_items']:takeItemFromSlot(thePlayer, itemSlot - correction, false)
		...
		if foundPackage then
			for itemIndexID, itemTableContent in ipairs(foundPackage[5]) do
				if itemTableContent[2] > 0 then -- its a real item
					exports.mrp_global:takeItem(thePlayer, itemTableContent[1], itemTableContent[2]) 
				end
			end
		end
		...
		if savedSkin then
			...
			exports['mrp_items']:doItemGiveawayChecks(thePlayer, 16)

			dbExec(mysql:getConnection(), "UPDATE characters SET duty = '0' WHERE id = '" .. (getElementData( thePlayer, "dbid" )).."'" )	
		end
		
		exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "duty", 0, true)
...
addEvent("duty.setArmor", true)
addEventHandler("duty.setArmor", root,
