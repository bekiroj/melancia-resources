...
function getFactionFromName(factionName)
	for k,v in ipairs(exports.mrp_pool:getPoolElementsByType("team")) do
...
function getTeamFromFactionID(factionID)
	...
	return exports.mrp_pool:getElement("team", factionID)
end
		...
		function(qh)
			...
			Async:foreach(res,
				function(row)
					...
					if theTeam then
						exports.mrp_pool:allocateElement(theTeam, id)
						exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "type", factionType, true)
						exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "money", money, true)
						exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "id", id, true)
						...
						local motd = row.motd
						exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "ranks", factionRanks, true)
						exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "wages", factionWages, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "motd", motd, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "note", row.note == nil and "" or row.note, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "fnote", row.fnote == nil and "" or row.fnote, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "phone", row.phone ~= nil and row.phone or nil, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "max_interiors", tonumber(row.max_interiors), false, true)
				...
				function()
					...
					exports.mrp_pool:allocateElement(citteam, -1)
					
					setElementData(getResourceRootElement(getResourceFromName("mrp_factions")), "factionDuty", custom)
					setElementData(getResourceRootElement(getResourceFromName("mrp_factions")), "factionLocations", locations)
	
					local players = exports.mrp_pool:getPoolElementsByType("player")
					for k, thePlayer in ipairs(players) do
						if getElementData(thePlayer, "loggedin") == 1 then
							setPlayerTeam(thePlayer, exports.mrp_pool:getElement("team", (getElementData(thePlayer, "faction") or -1)) or citteam)
						end
					end
				end
			)
...
function showFactionMenuEx(source, factionID, fromShowF)
	...
	if (logged==1) then
		...
		if (menuVisible==0) then
			...
			if (factionID~=-1) then
				local theTeam = exports.mrp_pool:getElement("team", factionID)
				if theTeam then
							...
							if rows > 0 then
								...
								for index, value in ipairs(getElementsByType("vehicle")) do
									if value and isElement(value) and tonumber(value:getData("faction")) == tonumber(factionID) then
										...
										local veh = exports.mrp_pool:getElement("vehicle", value:getData("dbid"))
										vehicleModels[j] = exports.mrp_global:getVehicleName(veh)
										if exports.mrp_global:hasItem(veh, 139) and getElementDimension(veh) == 0 and getElementInterior(veh) == 0 then
											vehicleLocations[j] = exports.mrp_global:getElementZoneName(veh) 
										else
				...
				outputChatBox("#575757Melancia: #f4f4f4Herhangi bir birlikte bulunmuyorsunuz.", source, 255, 0, 0, true)
			end
		else
...
function callbackUpdateRanks(ranks, wages)
	...
	if (wages) then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "wages", wages, false)
	end
	...
	exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "ranks", ranks, false)
...
function callbackRespawnVehicles()
	...
	if not (factionCooldown) then
		...
		for k, v in ipairs(teamPlayers) do
			outputChatBox(username:gsub("_"," ") .. " tüm boş birlik araçlarını yeniden canlandırdı.", v, 255, 194, 14)
		end

		setTimer(resetFactionCooldown, 60000, 1, theTeam)
		exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "cooldown", true, false)
	else
...
function resetFactionCooldown(theTeam)
	exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "cooldown")
end

function callbackRespawnOneVehicle(vehicleID)
	local theTeam = getPlayerTeam(source)
	local theTeamID = getElementData(theTeam, "id")
	local theVehicle = exports.mrp_pool:getElement("vehicle", tonumber(vehicleID))
	...
	if theVehicle then
		...
		if (theTeamID == theVehicleID and not getVehicleOccupant(theVehicle, 0) and not getVehicleOccupant(theVehicle, 1) and not getVehicleOccupant(theVehicle, 2) and not getVehicleOccupant(theVehicle, 3) and not getVehicleTowingVehicle(theVehicle)) then
			...
			exports.mrp_logs:dbLog(source, 6, theVehicle, "FACTIONRESPAWN")
...
function callbackUpdateNote(note)
	...
	if (factionID~=-1) then
		if dbExec(mysql:getConnection(), "UPDATE factions SET note='" .. tostring((note)) .. "' WHERE id='" .. factionID .. "'") then
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "note", note, false)
		else
...
function callbackUpdateFNote(fnote)
	...
	if (factionID~=-1) then
		if dbExec(mysql:getConnection(), "UPDATE factions SET fnote='" .. tostring((fnote)) .. "' WHERE id='" .. factionID .. "'") then
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(theTeam, "fnote", fnote, false)
		else
...
function callbackRemovePlayer(removedPlayerName)
	...
	if dbExec(mysql:getConnection(), "UPDATE characters SET faction_id='-1', faction_leader='0', faction_rank='1', duty = 0 WHERE charactername='" .. (removedPlayerName) .. "'") then
		...
		if (removedPlayer) then -- Player is online
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(removedPlayer, "faction", -1, false)
			exports.mrp_anticheat:changeProtectedElementDataEx(removedPlayer, "factionleader", 0, false)
			exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, "factionleader", 0, false)
...
function callbackToggleLeader(playerName, isLeader)
	...
	if (isLeader) then -- Make player a leader
		...
		if dbExec(mysql:getConnection(), "UPDATE characters SET faction_leader='1' WHERE charactername='" .. (playerName) .. "'") then
			...
			-- Send message to everyone in the faction
			--exports.mrp_factions:sendNotiToAllFactionMembers(factionID, username:gsub("_", " ") .. " promoted " .. playerName:gsub("_", " ") .. " to leader of your faction '"..getTeamName(theTeam).."'.")
			
			local thePlayer = getPlayerFromName(playerName)
			if(thePlayer) then -- Player is online, tell them
				exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "factionleader", 1, true)
			end
		else
		...
		if dbExec(mysql:getConnection(), "UPDATE characters SET faction_leader='0' WHERE charactername='" .. (playerName) .. "'") then
			...
			if(thePlayer) then -- Player is online, tell them
				...
				exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "factionleader", 0, true)
			end
			
			-- Send message to everyone in the faction
		--exports.mrp_factions:sendNotiToAllFactionMembers(factionID, username:gsub("_", " ") .. " demoted " .. playerName:gsub("_", " ") .. " from leader to member of your faction '"..getTeamName(theTeam).."'.")
		else
...
function callbackPromotePlayer(playerName, rankNum, oldRank, newRank)
	...
	if dbExec(mysql:getConnection(), "UPDATE characters SET faction_rank='" .. rankNum .. "' WHERE charactername='" .. (playerName) .. "'") then
		...
		if(thePlayer) then -- Player is online, set his rank
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "factionrank", rankNum, false)
		end
		
		-- Send message to everyone in the faction
		--exports.mrp_factions:sendNotiToAllFactionMembers(factionID, playerName:gsub("_", " ") .. " was promoted from '" .. oldRank .. "' to '" .. newRank .. "' by "..username:gsub("_", " ").." of '"..getTeamName(theTeam).."'")
	else
...
function callbackDemotePlayer(playerName, rankNum, oldRank, newRank)
	...
	if dbExec(mysql:getConnection(), "UPDATE characters SET faction_rank='" .. rankNum .. "' WHERE charactername='" .. safename .. "'") then
		...
		-- Send message to everyone in the faction
		--exports.mrp_factions:sendNotiToAllFactionMembers(factionID, playerName:gsub("_", " ") .. " was demoted from '" .. oldRank .. "' to '" .. newRank .. "' by "..username:gsub("_", " ").." of '"..getTeamName(theTeam).."'")
	else
...
function callbackQuitFaction()
	...
	elseif dbExec(mysql:getConnection(), "UPDATE characters SET faction_id='-1', faction_leader='0', duty = 0, faction_perks='{}' WHERE charactername='" .. safename .. "'") then
		exports["mrp_infobox"]:addBox(client, "success","'" .. theTeamName .. "' isimli birlikten çıkış yaptın.")
		exports.mrp_global:takeItem(client, 87)
		exports.mrp_global:takeItem(client, 87)
		exports.mrp_global:takeItem(client, 87)
		exports.mrp_global:takeItem(client, 86)
		exports.mrp_global:takeItem(client, 86)
		exports.mrp_global:takeItem(client, 86)
		exports.mrp_global:takeItem(client, 64)
		exports.mrp_global:takeItem(client, 64)
		exports.mrp_global:takeItem(client, 64)
		local newTeam = getTeamFromName("Citizen")
		setPlayerTeam(client, newTeam)
		exports.mrp_anticheat:changeProtectedElementDataEx(client, "faction", -1, false)
		exports.mrp_anticheat:changeProtectedElementDataEx(client, "factionrank", 1, false)
		exports.mrp_anticheat:changeProtectedElementDataEx(client, "factionleader", 0, false)
		exports.mrp_anticheat:changeProtectedElementDataEx(client, "factionphone", nil, false)
		exports.mrp_anticheat:changeProtectedElementDataEx(client, "factionPackages", {}, false)
...
function callbackInvitePlayer(invitedPlayer)
	...
	if dbExec(mysql:getConnection(), "UPDATE characters SET faction_leader = 0, faction_id = " .. factionID .. ", faction_rank = 1 WHERE charactername='" .. safename .. "'") then
		...
		if (targetTeam~=nil) and (getTeamName(targetTeam)~="Citizen") then
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(invitedPlayer, "faction", factionID, false)
			...
			if	(invitedPlayer) then
				...
				exports.mrp_anticheat:changeProtectedElementDataEx(invitedPlayer, "factionrank", 1, false)
				exports.mrp_anticheat:changeProtectedElementDataEx(client, "factionphone", nil, false)
...
function hideFactionMenu()
	exports.mrp_anticheat:changeProtectedElementDataEx(client, "factionMenu", 0, false)
end
	...
	if hasPlayerAccessOverFaction(client, factionID) then
		...
		dbQuery(
			function(qh, client)
				...
				if rows > 0 then
					for index, row in ipairs(res) do
						...
						elseif tonumber(row["from"]) then
							...
							if num < 0 then
								from = getTeamName(exports.mrp_pool:getElement("team", -num)) or "-"
						...
						elseif tonumber(row["to"]) and tonumber(row["to"]) < 0 then
							to = getTeamName(exports.mrp_pool:getElement("team", -tonumber(row["to"])))
						end
				...
				local bankmoney = exports.mrp_global:getMoney(faction)
...
addEventHandler('factionmenu:setphone', root,
	function(playerName, number)
		...
		if dbExec(mysql:getConnection(), "UPDATE characters SET faction_phone=" .. (tonumber(number) or "NULL") .. " WHERE charactername='" .. safename .. "'") then
			local thePlayer = getPlayerFromName(playerName)
			if(thePlayer) then -- Player is online, tell them
				exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "factionphone", tonumber(number) or nil, false)
			end
		end
	end)
...
function fromDatetime(string)
	local split1 = exports.mrp_global:split(string, " ")
	local date = split1[1]
	local time = split1[2]

	local datesplit = exports.mrp_global:split(date, "-")
	local year = tonumber(datesplit[1])
	local month = tonumber(datesplit[2])
	local day = tonumber(datesplit[3])

	local timesplit = exports.mrp_global:split(date, ":")
...
function refreshClient(message, factionID, dontSendToClient)
	...
	local resource = getResourceRootElement(getResourceFromName("mrp_factions"))
	if resource then
...
function addDuty(dutyItems, finalLocations, dutyNewSkins, name, factionID, dutyID)
	...
	if dutyID == 0 then
		...
		exports.mrp_logs:dbLog(client, 35, "fa"..tostring(factionID), "Added duty "..name.." Database ID #"..index)
	else
		...
		exports.mrp_logs:dbLog(client, 35, "fa"..tostring(factionID), "Revised duty "..name.." Database ID #"..dutyID)
	end
end
...
function addLocation(x, y, z, r, i, d, name, factionID, index)
	local interiorElement = exports.mrp_pool:getElement("interior", d) or d == 0
	if interiorElement then
		...
		if tonumber(interiorF) == tonumber(factionID) or d == 0 then
			if not index then -- Index is used if the event is from a edit
				...
				exports.mrp_logs:dbLog(client, 35, "fa"..tostring(factionID), "Added location, Name:"..name.." Database ID:"..newIndex.." x:"..x.." y:"..y.." z:"..z.." radius:"..r.." interior:"..i.." dimension:"..d)
			else
				...
				exports.mrp_logs:dbLog(client, 35, "fa"..tostring(factionID), "Revised location ID #"..index.." x:"..x.." y:"..y.." z:"..z.." radius:"..r.." interior:"..i.." dimension:"..d)
			end
		else
...
function addVehicle(vehicleID, factionID)
	local element = exports.mrp_pool:getElement("vehicle", vehicleID)
	if element then
		if getElementData(element, "faction") == factionID then
			...
			exports.mrp_logs:dbLog(client, 35, "fa"..tostring(factionID), "Added Vehicle #"..vehicleID.." Database ID:"..newIndex)
...
function removeLocation(removeID, factionID)
	...
	exports.mrp_logs:dbLog(client, 35, "fa"..tostring(factionID), "Removed Location #"..removeID)
...
function removeDuty(removeID, factionID)
	...
	exports.mrp_logs:dbLog(client, 35, "fa"..tostring(factionID), "Removed duty #"..removeID)
...
function factionActive(thePlayer, commandName)
			...
			if takim and takim > 0 then
				local takimveri = exports.mrp_pool:getElement("team", takim)
				local takimisim = getTeamName(takimveri)
				local takimoyunculari = getPlayersInTeam(takimveri)
				
				if #takimoyunculari == 0 then
					outputChatBox("#575757 Melancia:#b9c9bf".. takimisim .."' maalesef oluşumunda hiç çevrimiçi oyuncu bulamadım.", thePlayer, 255, 255, 255, true)
				else
					...
					for k, takimoyuncular in ipairs(takimoyunculari) do
						...
						outputChatBox("#575757 Melancia: #b9c9bf  Karakter : #FFFFFF".. getPlayerName(takimoyuncular) .." #b9c9bf Rütbe : #FFFFFF".. oyuncurutbe .. " #b9c9bf Telefon : - #FFFFFF ", thePlayer, 255, 255, 255, true)
					end
				end
			else
				outputChatBox("#575757 Melancia: #b9c9bf Herhangi bir oluşumda değilsiniz, lütfen oluşuma girdikten sonra tekrar deneyiniz.", thePlayer, 255, 255, 255, true)
			end
	end
addCommandHandler("birlikaktif", factionActive)

function birlikOnayla(thePlayer, commandName, factionID, onay)
	if (exports.mrp_integration:isPlayerSeniorAdmin(thePlayer) or exports.mrp_integration:isPlayerScripter(thePlayer)) then
		if not (factionID) or not (onay)  then
			...
			if factionID and factionID > 0 then
				local theTeam = exports.mrp_pool:getElement("team", factionID)
				if (theTeam) then
					dbExec(exports.mrp_mysql:getConnection(),"UPDATE factions SET onay='" .. tonumber(onay) .. "' WHERE id='" .. factionID .. "'")
					local oldName = getTeamName(theTeam)
					setElementData(theTeam, "birlik_onay", tonumber(onay))
					
					if tonumber(onay) == 1 then
						exports.mrp_global:sendMessageToAdmins(exports.mrp_global:getPlayerFullIdentity(thePlayer).." '" .. oldName .. "' isimli birliği onayladı!")
					elseif tonumber(onay) == 0 then
						exports.mrp_global:sendMessageToAdmins(exports.mrp_global:getPlayerFullIdentity(thePlayer).." '" .. oldName .. "' isimli birliği onayını kaldırdı!")
					end
				else
