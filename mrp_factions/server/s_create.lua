...
setElementData(birlikPickup, 'informationicon:information', '#AC2D2D/birlikkur#ffffff\nBirlik Oluşturma Bölgesi')

function birlikKurTrigger(thePlayer, cmd)
	local playerTeam = getElementData(thePlayer, "faction")
	
	if playerTeam ~= -1 then
		outputChatBox("#575757Melancia: #FFFFFFZaten bir birliğiniz var!", thePlayer, 255, 0, 0, true)
		return
	end
...
function birlikSeviyeTrigger(thePlayer, cmd)
	if isElementWithinColShape(thePlayer, birlikCol) then
		...
		if birlik_seviye == 6 then
			outputChatBox("#575757Melancia: #FFFFFFBirliğiniz son seviyedir!", thePlayer, 255, 0, 0, true)
			return
		end
		...
		if (oyuncu_birlik) and (birlikLider > 0) then
			...
			outputChatBox("#575757Melancia: #FFFFFFHerhangi bir birliğin lideri değilsiniz!", thePlayer, 255, 0, 0, true)
		end
	end
end
...
function adminBirlikKur(thePlayer, birlikName, birlikType)
	...
	if string.len(birlikName) < 4 then
		outputChatBox("#575757Melancia: #FFFFFFBirlik ismi en az 4 karakterden oluşmalıdır!", thePlayer, 255, 0, 0, true)
		return false
	elseif string.len(birlikName) > 36 then
		outputChatBox("#575757Melancia: #FFFFFFBirlik ismi en fazla 36 karakterden oluşmalıdır!", 255, 0, 0, true)
		return false
	end
		...
		for index, value in ipairs(getElementsByType("team")) do
			if getElementData(value, "name") == factionName then
				outputChatBox("#575757Melancia: #FFFFFFMaalesef, birlik ismi kullanımda!", thePlayer, 255, 0, 0, true)
				return false
			end
		end
		...
		if theTeam then
			if dbExec(mysql:getConnection(), "INSERT INTO factions SET name='" .. tostring(factionName) .. "', bankbalance='0', type='" .. (factionType) .. "', level='1'") then
					...
					function(qh, thePlayer, theTeam, factionName, factionType)
						...
						if rows > 0 then
							for index, value in ipairs(res) do
								...
								outputChatBox("#575757Melancia: #FFFFFF'" .. factionName .. "' isimli birliğiniz başarıyla oluşturuldu! ID #" .. id .. ".", thePlayer, 0, 255, 0, true)
								setElementData(theTeam, "type", tonumber(factionType))
								setElementData(theTeam, "id", tonumber(id))
								setElementData(theTeam, "money", 0)
								...
								exports.mrp_global:sendMessageToAdmins("AdmWarn: " .. getPlayerName(thePlayer) .. " yeni birlik oluşturdu! (/makefaction) Birlik Ismi: '" .. factionName .. "' Birlik ID #" .. id)
							end
						end
					end,
				...
				outputChatBox("#575757Melancia: #FFFFFFBirliğinizi oluştururken bir hata meydana geldi.", thePlayer, 255, 0, 0, true)
			end
		else
			outputChatBox("#575757Melancia: #FFFFFF'" .. tostring(factionName) .. "' isimli birlik zaten var.", thePlayer, 255, 0, 0, true)
		end
end
...
function birlikKur(thePlayer, birlikName, birlikType)
	local para = exports.mrp_global:getMoney(thePlayer)	
	...
	if para >= 10000 then
					...
					function(qh, thePlayer, theTeam, factionName, factionType)
						...
						if rows > 0 then
							for index, value in ipairs(res) do
								...
								setElementData(theTeam, "note", "", false)
								exports.mrp_logs:dbLog(thePlayer, 4, theTeam, "MAKE FACTION")
								exports.mrp_global:takeMoney(thePlayer, 10000)
								exports.mrp_global:sendMessageToAdmins("AdmWarn: " .. getPlayerName(thePlayer) .. " yeni birlik oluşturdu! Birlik Ismi: '" .. factionName .. "' Birlik ID #" .. id)
							end
						end
					end,
				{thePlayer, theTeam, factionName, factionType}, mysql:getConnection(), "SELECT * FROM factions WHERE id = LAST_INSERT_ID()")
		...
		outputChatBox("#575757Melancia: #FFFFFFMaalesef, birlik kuracak paranız yok.", thePlayer, 255, 0, 0, true)
	end
end
...
function birlikSeviye(thePlayer, birlikIsmi, birlikSeviye, birlikFiyat)
	...
	if para >= birlikFiyat then
		if birlikIsmi and birlikSeviye then
			...
			if not result then
				outputChatBox("#575757Melancia: #FFFFFFBirliğinizin seviyesini arttırırken bir hata meydana geldi.", thePlayer, 255, 0, 0, true)
			end
			
			if result and birlikLevelArttir then
				outputChatBox("#575757Melancia: #FFFFFFBirliğinizin seviyesi başarıyla arttırılmıştır!", thePlayer, 0, 255, 0, true)
				exports.mrp_global:takeMoney(thePlayer, birlikFiyat)
			end
		end
	else
		outputChatBox("#575757Melancia: #FFFFFFYeterli paranız yok.", thePlayer, 255, 0, 0, true)
	end
end
...
function aracimiBirligeVer(thePlayer, cmd, vehID)
	if vehID then
		...
		local vehElement = exports.mrp_pool:getElement("vehicle", vehID)
			...
			if vehOwner == playerID then
				...
				if playerBirlik then
					...
					if elementSet and query then
						exports["mrp_items"]:deleteAll(3, vehID)
						outputChatBox("#575757Melancia: #f0f0f0Aracınız başarıyla birliğe verilmiştir!", thePlayer, 0, 255, 0, true)
					end
				else
					outputChatBox("#575757Melancia: #f0f0f0Bir birlikte değilsiniz.", thePlayer, 255, 0, 0, true)
				end
			else
				outputChatBox("#575757Melancia: #f0f0f0Araç size ait değil.", thePlayer, 255, 0, 0, true)
			end
		else
			outputChatBox("#575757Melancia: #f0f0f0Araç zaten bir birliğe ait.", thePlayer, 255, 0, 0, true)
		end
	else
		outputChatBox("#575757Melancia: #f0f0f0/"..cmd.." [Araç ID]", thePlayer, 255, 0, 0, true)
	end
end
		...
		if playerBirlik then
			if playerBirlik == vehFaction then
				if factionLeader == 1 then
					...
					if elementSet and query then
						outputChatBox("#575757Melancia: #f0f0f0Aracınız başarıyla sahibine geri verilmiştir!", thePlayer, 0, 255, 0, true)
					end
				else
					outputChatBox("#575757Melancia: #f0f0f0Aracı sahibine geri verebilmek için birlik lideri olmalısınız.", thePlayer, 255, 0, 0, true)
				end
			end
		else
			outputChatBox("#575757Melancia: #f0f0f0Bir birlikte değilsiniz.", thePlayer, 255, 0, 0, true)
...
function aracBirlikYonetim(thePlayer, cmd, vehID, rankID)
	if not (vehID or rankID) then
		outputChatBox("#575757Melancia: #f0f0f0/"..cmd.." [Araç ID] [Rank ID]", thePlayer, 255, 0, 0, true)
	else
		local playerID = getElementData(thePlayer, "dbid")
		local vehElement = exports.mrp_pool:getElement("vehicle", vehID)
		local vehOwner = getElementData(vehElement, "owner")
		...
		if playerBirlik then
			if playerBirlik == vehFaction then
				if factionLeader == 1 then
					...
					if elementSet and query then
						outputChatBox("#575757Melancia: #f0f0f0Araç başarıyla ayarlanmıştır!", thePlayer, 0, 255, 0, true)
					end
				else
					outputChatBox("#575757Melancia: #f0f0f0Aracı yönetebilmek için birlik lideri olmalısınız.", thePlayer, 255, 0, 0, true)
				end
			end
		else
			outputChatBox("#575757Melancia: #f0f0f0Bir birlikte değilsiniz.", thePlayer, 255, 0, 0, true)
