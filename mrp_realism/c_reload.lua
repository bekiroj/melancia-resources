...
function drawText()
	...
	dxDrawText("'R' tuşuna basarak mermi yenileyin.", 1, 1, scrWidth, scrHeight, tocolor(0, 0, 0), 1, font, "center", "bottom")
	dxDrawText("'R' tuşuna basarak mermi yenileyin.", 0, 0, scrWidth, scrHeight, tocolor(255, 255, 255), 1, font, "center", "bottom")
end
...
function checkReloadStatus ()
	...
	if  getPedAmmoInClip (getLocalPlayer()) == 1 and isGun(weaponID) then -- getElementData(source, "r:cf:"..tostring(weaponID)) or 
		if not handled then
			font = dxCreateFont(":item-system/Roboto.ttf", 11)
