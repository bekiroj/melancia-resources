	...
	if (handbrake == 0) then
		if getVehicleType(vehicle) == 'BMX' or getVehicleType(vehicle) == 'Bike' then
			if not kickstand then
				outputChatBox('Bu araçta el freni yok.', player, 255, 0, 0)
			elseif not isVehicleOnGround(vehicle) and not forceOnGround then
				outputChatBox('Bunun çalışması için yerde olmalısın.', player, 255, 0, 0)
			elseif math.floor(exports.mrp_global:getVehicleVelocity(vehicle)) > 2 then
				outputChatBox("Bu sürüş sırasında işe yaramaz...", player, 255, 0, 0)
			else
				exports.mrp_anticheat:changeProtectedElementDataEx(vehicle, "handbrake", 1, true)
		...
		elseif (isVehicleOnGround(vehicle) or forceOnGround) or getVehicleType(vehicle) == "Boat" or getVehicleType(vehicle) == "Helicopter" or someExceptions[getVehicleType(vehicle)] then
			if kickstand then
				outputChatBox('Ayaklığı yok.', player, 255, 0, 0)
				return false
			end
		...
		if getVehicleType(vehicle) == 'BMX' or getVehicleType(vehicle) == 'Bike' then
			...
			if kickstand then
				outputChatBox('Araçta ayaklık yok.', player, 255, 0, 0)
				return
			end
		end
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(vehicle, "handbrake", 0, true)
