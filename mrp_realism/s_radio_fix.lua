...
function syncRadio(station)
	...
	if (vehicle) then
		exports.mrp_anticheat:changeProtectedElementDataEx(vehicle, "radiostation", station, false)
...
function setRadioOnEnter(player)
	if not (lawVehicles[getElementModel(source)]) then
		...
		if not station then
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(source, "radiostation", station, false)
		end
