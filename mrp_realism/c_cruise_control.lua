...
function deactivateCruiseControl()
    ...
    setPedControlState(localPlayer, "accelerate",false)
    ccEnabled = false
    exports.mrp_hud:sendBottomNotification(localPlayer, "Cruise Control", "Cruise Control disabled.")
end
				...
				if (ccEnabled) then
					...
					if targetSpeed > 10 then
						if (getVehicleType(theVehicle) == "Automobile" or getVehicleType(theVehicle) == "Bike" or getVehicleType(theVehicle) =="Boat" or getVehicleType(theVehicle) == "Train" or getVehicleType(theVehicle) == "Plane" or getVehicleType(theVehicle) == "Helicopter") then
							exports.mrp_hud:sendBottomNotification(localPlayer, "Cruise Control Enabled", "Use - and + to adjust the speed.")
							activateCruiseControl()
						end
					else
                        exports.mrp_hud:sendBottomNotification(localPlayer, "Cruise Control", "Cruise Control can be used for maintaining speed, not pulling up.")
					end
				end
            else
                exports.mrp_hud:sendBottomNotification(localPlayer, "Cruise Control", "The engine is not turned on!")
			end
		end
	end
end
...
function restrictBikes(manual) 
    ...
    if (tV) then
        local maxSpeed = limitSpeed[getElementModel(tV)]
        if maxSpeed then 
            tS = exports.mrp_global:getVehicleVelocity(tV) 
            if tS > maxSpeed then 
