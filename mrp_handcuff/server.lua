	...
	if  plr:getTeam():getName() == "İstanbul Emniyet Müdürlüğü" or plr:getTeam():getName() == "İstanbul İl Jandarma Komutanlığı" then
		...
		if target then
			...
			if getElementData(target, "kelepce") then
				kelepcele(target, "al")
				exports.mrp_global:sendLocalMeAction(plr, "Sağ ve sol eli ile şahsın bileğindeki kelepçeyi çözer.")
			return end

			exports.mrp_global:sendLocalMeAction(plr, "Sağ elini teçhizat kemerine atar ve bir kelepçe çıkarıp şahsın bileklerine geçirir.")
