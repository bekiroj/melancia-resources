	...
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) then
		if not (targetPlayer) then
			...
			if not (rx) or not (ry) or not (rz) or not (reconrot) or not (recondimension) or not (reconinterior) then
				outputChatBox("Melancia: /recon [Player Partial Nick]", thePlayer, 255, 194, 14)
			else
				...
				exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reconx", nil, false)
				exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "recony", nil, false)
				exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reconz", nil, false)
				exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reconrot", nil, false)
				setCameraTarget(thePlayer, thePlayer)
				setElementAlpha(thePlayer, 255)
				exports["mrp_infobox"]:addBox(thePlayer, "error", "Aktif izlemeyi sonlandırdın.")
			end
		else
			local targetPlayer, targetPlayerName = exports.mrp_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			local freecamEnabled = exports.mrp_freecam:isPlayerFreecamEnabled (thePlayer)
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if getPedOccupiedVehicle ( thePlayer ) then
						exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "realinvehicle", 0, false)
					...
					if ( not getElementData(thePlayer, "reconx") or getElementData(thePlayer, "reconx") == true ) and not getElementData(thePlayer, "recony") then
						...
						exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reconx", x, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "recony", y, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reconz", z, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reconrot", rot, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "recondimension", dimension, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reconinterior", interior, false)
					end
					...
					if not (success) then
						...
						if hiddenAdmin == 0 and not exports.mrp_integration:isPlayerAdmin(thePlayer) then
							local adminTitle = exports.mrp_global:getPlayerAdminTitle(thePlayer)
							exports.mrp_global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. getPlayerName(thePlayer) .. " started reconning " .. targetPlayerName .. ".")
						end
...
function fuckRecon(thePlayer, commandName, targetPlayer)
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer)) then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reconx", false, false)
		exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "recony", false, false)
		exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reconz", false, false)
		exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reconrot", false, false)
...
function toggleInvisibility(thePlayer)
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerScripter(thePlayer) then
		...
		if (enabled == true) then
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "invisible", false, false)
			exports.mrp_logs:dbLog(thePlayer, 4, thePlayer, "DISAPPEAR DISABLED")
		elseif (enabled == false or enabled == nil) then
			setElementAlpha(thePlayer, 0)
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "reconx", true, false)
			outputChatBox("You are now invisible.", thePlayer, 0, 255, 0)
			exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "invisible", true, false)
			exports.mrp_logs:dbLog(thePlayer, 4, thePlayer, "DISAPPEAR ENABLED")
		else
	...
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer) then
		if exports.mrp_global:isStaffOnDuty(thePlayer) then
			...
			if (enabled == true) then
				...
				exports.mrp_logs:dbLog(thePlayer, 4, thePlayer, "RP SUPERVISOR DISABLED")
				exports.mrp_global:sendWrnToStaff("[AdmCmd] "..getElementData(thePlayer, "account:username").." has disabled RP supervisor mode.")
			...
			elseif (enabled == false or enabled == nil) then
				...
				exports.mrp_logs:dbLog(thePlayer, 4, thePlayer, "RP SUPERVISOR ENABLED")
				exports.mrp_global:sendWrnToStaff("[AdmCmd] "..getElementData(thePlayer, "account:username").." has enabled RP supervisor mode.")
...
function asyncReconActivate(cur)
	local target = exports.mrp_pool:getElement("player", cur.target)
	if not target then
		triggerClientEvent(source, "admin:recon", source)
		return 
	end
	removePedFromVehicle(source)
	if exports.mrp_freecam:isEnabled(source) then
