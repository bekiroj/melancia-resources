...
function doCheck(sourcePlayer, command, ...)
	if (exports.mrp_integration:isPlayerTrialAdmin(sourcePlayer) or exports.mrp_integration:isPlayerSupporter(sourcePlayer)) then
		if not (...) then
			outputChatBox("Kullanım: /" .. command .. " [Karakter Adı & ID]", sourcePlayer, 255, 194, 14)
		else
			local noob = exports.mrp_global:findPlayerByPartialNick(sourcePlayer, table.concat({...},"_"))
			if (noob) then
									...
									function(qh)
										...
										dbQuery(
											function(qh)
												...
												local adminlevel = exports.mrp_global:getPlayerAdminTitle(noob)
				...
				exports.mrp_logs:dbLog(thePlayer, 4, ..., "CHECK")
			end
		end
	end
end
...
function savePlayerNote( target, text )
	if exports.mrp_integration:isPlayerTrialAdmin(client) or exports.mrp_integration:isPlayerSupporter(client) then
...
function showAdminHistory( target )
	...
	if not (exports.mrp_integration:isPlayerTrialAdmin( client ) or exports.mrp_integration:isPlayerSupporter( client )) then
				...
				if (tonumber(sqlQuery["action"]) == 4) then -- Warning
					...
					for i, player in pairs(getElementsByType("player")) do
						if getElementData(player, "account:id") == tonumber(accountNumber) then
							...
							exports.mrp_anticheat:changeProtectedElementDataEx(player, "warns", warns, false)
							break
						end
					end
				end
...
addCommandHandler( "history", 
	function( thePlayer, commandName, ... )
		if not (exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer)) then
			if (...) then
		...
		if (...) then
			targetPlayer = exports.mrp_global:findPlayerByPartialNick(thePlayer, table.concat({...},"_"))
		end
