	...
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer)) then
		if not (targetPlayer) then
			...
			if targetPlayer then
				...
				if (logged==0) then
					outputChatBox(exports.mrp_pool:getServerSyntax(false, "e").."Karşı kullanıcı giriş yapmadığı için işlem gerçekleştirilemedi.", thePlayer, 255, 0, 0, true)
				else
					if not getElementData(targetPlayer, "kelepce") and not getElementData(targetPlayer, "ipbagli") then
						...
						exports.mrp_handcuff:kelepcele(targetPlayer, "al")
						exports.mrp_handcuff:kelepcele(targetPlayer, "ipal")
						exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncunun kelepçesini çözdü.")
					end
				end
			end
		end
...
function adminUnblindfold(thePlayer, commandName, targetPlayer)
	...
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer)) or uyk then
		if not (targetPlayer) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Player Partial Nick / ID]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.mrp_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if (blindfolded==0) then
						...
						exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "UNBLINDFOLD")
						exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncunun göz bandını çözdü.")
					end
				end
			end
		end
	...
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer)) then
		if not (targetPlayer) then
			...
			if targetPlayer then
				...
				if (logged==0) then
					exports["mrp_infobox"]:addBox(thePlayer, "warning", "Oyuncu henüz giriş yapmamış.")
				else
					local any = false
					...
					for key, value in pairs(masks) do
						if getElementData(targetPlayer, value[1]) then
							any = true
							exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, value[1], false, true)
						end
					end
					...
					if any then
						...
						exports["mrp_infobox"]:addBox(thePlayer, "success", targetPlayerName .. " isimli oyuncunun maskesi çıkartılmıştır.")
						exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "UNMASK")
					else
...
function disarmPlayer(thePlayer, commandName, targetPlayer)
	if (exports.mrp_integration:isPlayerAdminIII(thePlayer)) then
		if not (targetPlayer) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Player Partial Nick / ID]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.mrp_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			if targetPlayer then
				...
				elseif (logged==1) then
					for i = 115, 116 do
						while exports['mrp_items']:takeItem(targetPlayer, i) do
						end
					end
					local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
					...
					exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "DISARM")
					exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncunun silahlarını sildi.")
				end
			end
		end
	end
...
local function showIPAlts(thePlayer, ip)
	local result = exports.mrp_global:getCache("accounts", ip, "ip")
	if result then
		local count = 0
...
function findAltAccIP(thePlayer, commandName, ...)
	if exports.mrp_integration:isPlayerTrialAdmin( thePlayer ) then
		if not (...) then
			outputChatBox("|| Melancia || /" .. commandName .. " [Partial Player Nick]", thePlayer, 255, 194, 14)
		else
			local targetPlayerName = table.concat({...}, "_")
			local targetPlayer = exports.mrp_global:findPlayerByPartialNick(nil, targetPlayerName)
			
			if not targetPlayer or getElementData( targetPlayer, "loggedin" ) ~= 1 then
				targetPlayerName = table.concat({...}, " ")
				
				-- select by accountname
				local result = exports.mrp_global:getCache("accounts", targetPlayerName, "username")
				if result then
				...
				-- select by ip
				local result = exports.mrp_global:getCache("accounts", targetPlayerName, "ip")
				if result then
					showIPAlts(thePlayer, result.ip)
					return
				end
				...
				if not exports.mrp_integration:isPlayerDeveloper(targetPlayer) then
...
local function showAlts(thePlayer, id, creation)
	local result = exports.mrp_global:getCache("characters", id, "account")
	if result then
		local name = exports.mrp_global:getCache("accounts", id, "id")
		if name then
            ...
            function(qh)
                ...
                if rows > 0 then
                    ...
                    for index, row in ipairs(res) do
                        ...
                        if faction > 0 and exports.mrp_integration:isPlayerAdmin( thePlayer ) then -- Maxime | Hide faction from Trial and below
                            local theTeam = exports.mrp_pool:getElement("team", faction)
                            if theTeam then
...
function findAltChars(thePlayer, commandName, ...)
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer) then
		if not (...) then
			...
			local targetPlayerName = table.concat({...}, "_")
			local targetPlayer = targetPlayerName == "*" and thePlayer or exports.mrp_global:findPlayerByPartialNick(nil, targetPlayerName)
			
			if not targetPlayer or getElementData( targetPlayer, "loggedin" ) ~= 1 then
				-- select by character name
				local result = exports.mrp_global:getCache("characters", targetPlayerName, "charactername")
				if result then
...
local function showSerialAlts(thePlayer, serial)
	local result = exports.mrp_global:getMultipleCache("accounts", serial, "mtaserial")
...
function findAltAccSerial(thePlayer, commandName, ...)
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) then
		if not (...) then
			outputChatBox("|| Melancia || /" .. commandName .. " [Player Nick/Serial]", thePlayer, 255, 194, 14)
		else
			local targetPlayerName = table.concat({...}, "_")
			local targetPlayer = exports.mrp_global:findPlayerByPartialNick(nil, targetPlayerName)
			...
			if not targetPlayer then --or getElementData( targetPlayer, "loggedin" ) ~= 1 then
				...
				-- select by serial
				local result = exports.mrp_global:getCache("accounts", targetPlayerName, "mtaserial")
				if result then
                    showSerialAlts(thePlayer, result.mtaserial)
                    return
				end
   ...
   if (exports.mrp_integration:isPlayerAdminI(thePlayer)) then
      if not (gender) or not (targetPlayer) then
		 ...
		 setElementData(targetPlayer, "gender", gender, true)
			...
			exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, commandName.." "..genderint)
         end
      end
   end
end
   ...
   if (exports.mrp_integration:isPlayerAdminI(thePlayer)) then
      if not (age) or not (targetPlayer) then
         ...
         if (ageint>150) or (ageint<1) then
			...
			exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, commandName.." "..age)
         end
      end
   end
end
	...
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer)) then
		...
		if not (targetPlayer) or not dob or not mob then
			...
			if dbExec(mysql:getConnection(), "UPDATE `characters` SET `day`='" ..(dob) .. "', `month`='" .. (mob) .. "' WHERE id = '" .. (dbid).."' " ) then
				exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, "day", dob, true)
				exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, "month", mob, true)
				outputChatBox("You changed " .. targetPlayerName .. "'s date of birth to " .. exports.mrp_global:getPlayerDoB(targetPlayer) .. ".", thePlayer, 0, 255, 0)
				outputChatBox("Your date of birth was set to " .. exports.mrp_global:getPlayerDoB(targetPlayer) .. ".", targetPlayer, 0, 255, 0)
				exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, commandName.." "..dob.."/"..mob)
			else
...
function asetPlayerRace(thePlayer, commandName, targetPlayer, race)
       if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer)) then
         ...
         dbExec(mysql:getConnection(), "UPDATE characters SET skincolor='" .. (race) .. "' WHERE id = " .. (dbid))
				...
				exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, commandName.." "..raceint)
             end
          end
       end
end
       ...
       if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer)) then
          if not (height) or not (targetPlayer) then
            ...
            dbExec(mysql:getConnection(), "UPDATE characters SET height='" .. (height) .. "' WHERE id = " .. (dbid))
				exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, "height", height, true)
				...
				exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, commandName.." "..height)
             end
          end
       end
end
...
function adminDuty(thePlayer, commandName)
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) then
		...
		if adminduty == 1 then
			...
			exports.mrp_global:updateNametagColor(thePlayer)
			exports.mrp_global:sendMessageToAdmins("ADMIN DUTY: " .. username .. " Artık görevde değil.")
			...
			exports.mrp_global:sendMessageToAdmins("ADMIN DUTY: " .. username .. " Şuanda Görevde.")
...
function gmDuty(thePlayer, commandName)
	if exports.mrp_integration:isPlayerSupporter(thePlayer) or exports.mrp_integration:isPlayerHeadAdmin(thePlayer) then
		...
		if gmDuty == 1 then
			...
			exports.mrp_global:sendMessageToAdmins("REHBER DUTY: " .. username .. " Artık görevde değil.")
		else
			setElementData(thePlayer, "duty_supporter", 1)
			exports.mrp_global:sendMessageToAdmins("REHBER DUTY: " .. username .. " Şuanda Görevde.")
	...
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer) then
		if not (target) then
			...
			if targetPlayer then
				...
				local username = getPlayerName(thePlayer)
				exports.mrp_global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. username .. " froze " .. targetPlayerName .. ".")
				exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "FREEZE")
			end
		end
	end
end
	...
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer) then
		if not (target) then
			...
			if targetPlayer then
				...
				local username = getPlayerName(thePlayer)
				exports.mrp_global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. username .. " unfroze " .. targetPlayerName .. ".")
				exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "UNFREEZE")
			end
		end
	end
end
	...
	if exports.mrp_integration:isPlayerDeveloper( thePlayer) then
		if not (target) or not money or not (...) then
			...
			if targetPlayer then
				...
				local amount = exports.mrp_global:formatMoney(money)
				if not exports.mrp_global:takeMoney(targetPlayer, money) then
					outputChatBox("Oyuncudan $"..amount.." miktar para alınamadı.", thePlayer, 255, 0, 0)
					return false
				end
				exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "TAKEMONEY " ..money)
				...
				if tonumber(money) >= 5000 then
					exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncudan $" .. amount .. " miktar para kesti.("..reason..")")
				else
					exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncudan $" .. amount .. " miktar para kesti.("..reason..")")
				end
			end
	...
	if exports.mrp_integration:isPlayerDeveloper( thePlayer) then
		if not (target) or not money or not tonumber(money) or not (...) then
			...
			if targetPlayer then
				...
				if not exports.mrp_global:setMoney(targetPlayer, money) then
					outputChatBox("Could not set that amount.", thePlayer, 255, 0, 0)
					return false
				end
				exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "SETMONEY "..money)
				local amount = exports.mrp_global:formatMoney(money)
				reason = table.concat({...}, " ")
				...
				if tonumber(money) >= 5000 then
					exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncunun parasını $" .. amount .. " olarak değiştirdi .("..reason..")")
				else
					exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncunun parasını $" .. amount .. " olarak değiştirdi .("..reason..")")
				end
			end
	...
	if exports.mrp_integration:isPlayerDeveloper( thePlayer) then
		if not (target) or not money or not (...) then
			...
			if targetPlayer then
				...
				if not exports.mrp_global:giveMoney(targetPlayer, money) then
				...
				if tonumber(money) >= 100000 then
					exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncuya $" .. amount .. " para verdi.("..reason..")")
				else
					exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncuya $" .. amount .. " para verdi.("..reason..")")
				end
		...
		if not (targetPlayer) or not (...) then
			...
			if targetPlayer then
				...
				if (targetPlayerPower <= thePlayerPower) then
					...
					exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "PKICK "..reason)
					if (hiddenAdmin)==0 then
						if commandName ~= "skick" then
							local adminTitle = exports.mrp_global:getPlayerAdminTitle(thePlayer)
							exports.mrp_global:sendMessageToAdmins("[KICK]: " .. targetPlayerName .. " isimli oyuncu " .. adminTitle .. " " .. playerName .. " tarafından oyundan atıldı.")
							exports.mrp_global:sendMessageToAdmins("[KICK]: Sebep: " .. reason .. ".")

						end
						kickPlayer(targetPlayer, thePlayer, reason)
					else
						if commandName ~= "skick" then
							exports.mrp_global:sendMessageToAdmins("[KICK]: "..targetPlayerName.." oyundan atıldı.")
							exports.mrp_global:sendMessageToAdmins("[KICK]: Sebep: " .. reason .. ".")
						end
		...
		if not (targetPlayer) then
			...
			if targetPlayer then
				...
				elseif (targetPlayerPower > thePlayerPower) then -- Check the admin isn't slapping someone higher rank them him
					...
					if (hiddenAdmin==0) then
						local adminTitle = exports.mrp_global:getPlayerAdminTitle(thePlayer)
						exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncuyu tokatladı.")
					end
					exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "SLAP")
				end
			end
		end
	end
...
function hideAdmin(thePlayer, commandName)
	if exports.mrp_integration:isPlayerLeadAdmin(thePlayer) then
		...
		if (hiddenAdmin==0) then
			...
			exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili gizli yetkiyi açtı.")
		elseif (hiddenAdmin==1) then
			setElementData(thePlayer, "hiddenadmin", 0, true)
			outputChatBox("Hidden Admin - OFF", thePlayer, 255, 194, 14)
			exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili gizli yetkiyi kapattı.")
		end
		exports.mrp_global:updateNametagColor(thePlayer)
	...
	if (exports.mrp_integration:isPlayerAdminI(thePlayer)) then
		if not (...) or not (targetPlayer) then
			...
			if targetPlayer then
				...
				if hoursPlayed > 5 and not exports.mrp_integration:isPlayerAdmin(thePlayer) then
				...
				if newName == targetPlayerName then
					...
					local dbid = getElementData(targetPlayer, "dbid")
						...
						if (name) then
							exports['mrp_cache']:clearCharacterName( dbid )
							...
							if (hiddenAdmin==0) then
								exports.mrp_global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. getPlayerName(thePlayer) .. " changed " .. targetPlayerName .. "'s Name to " .. newName .. ".")
							...
							exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncunun ismini değiştirdi.")
						else
...
function setPlayerHealth(thePlayer, commandName, targetPlayer, health)
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerScripter(thePlayer)) then
		if not tonumber(health) or not (targetPlayer) then
			...
			if targetPlayer then
				...
				elseif not setElementHealth(targetPlayer, tonumber(health)) then
					...
					exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "SETHP "..health)
					exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncunun sağlık değerini değiştirdi.")
				end
			end
		end
	end
...
function adminHeal(thePlayer, commandName, targetPlayer)
	if (exports.mrp_global:isStaffOnDuty(thePlayer)) then
		...
		if not (targetPlayer) then
			...
			targetPlayer, targetPlayerName = exports.mrp_global:findPlayerByPartialNick(thePlayer, targetPlayer)
		end
		...
		if targetPlayer then
			...
			outputChatBox("Player " .. targetPlayerName .. " has received " .. health .. " Health.", thePlayer, 0, 255, 0)
			triggerEvent("onPlayerHeal", targetPlayer, true)
			exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "AHEAL "..health)
			exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncunun canını doldurdu.")
		end
	end
end
...
function setPlayerArmour(thePlayer, theCommand, targetPlayer, armor)
	if (exports.mrp_integration:isPlayerAdminII(thePlayer) or exports.mrp_integration:isPlayerScripter(thePlayer)) then
				...
				if (logged==1) then
					if (tostring(type(tonumber(armor))) == "number") then
						...
						if (targetPlayerFaction == 1) or (targetPlayerFaction == 15) or (targetPlayerFaction == 59) then
							...
							exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "SETARMOR " ..tostring(armor))
						elseif (targetPlayerFaction ~= 1) or (targetPlayerFaction ~= 15) or (targetPlayerFaction ~= 59) then
							if (exports.mrp_integration:isPlayerAdmin(thePlayer)) then
								local setArmor = setPedArmor(targetPlayer, tonumber(armor))
								outputChatBox("" .. targetPlayerName .. " isimli oyuncuya " .. armor .. " miktar zırh verildi.", thePlayer, 0, 255, 0)
								exports.mrp_logs:dbLog(thePlayer, 4, tagetPlayer, "SETARMOR " ..tostring(armor))
								exports.mrp_global:sendMessageToAdmins("Adm: "..getPlayerName(thePlayer):gsub("_", " ").." isimli yetkili "..targetPlayerName.." isimli oyuncunun zırh değerini değiştirdi.")
							else
							end
						end
					else
...
function givePlayerGun(thePlayer, commandName, targetPlayer, ...)
	if exports.mrp_integration:isPlayerHeadAdmin(thePlayer) then  
		...
		if not (targetPlayer) or (#args < 2) then
			exports["mrp_infobox"]:addBox(thePlayer, "warning", "/makegun [Karakter Adı & ID] [Silah ID] [Tek Kelimeli Sebep]")
		else
			local targetPlayer, targetPlayerName = exports.mrp_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			...
			if targetPlayer then
				...
				if weaponID == nil then
					...
					elseif cWeaponName == "fireextinguisher" then
						...
						exports["mrp_infobox"]:addBox(thePlayer, "error", "Hatalı Silah Adı veya ID'si, /gunlist'ten kontrol edin.")
							return
						else
				...
				if (logged==0) then
						exports["mrp_infobox"]:addBox(thePlayer, "error", "Oyuncu henüz giriş yapmamış.")
				elseif (logged==1) then
					...
					for variable = 1, quantity, 1 do
						local mySerial = exports.mrp_global:createWeaponSerial( 1, adminDBID, playerDBID)
						--outputChatBox(mySerial)
						give, error = exports.mrp_global:giveItem(targetPlayer, 115, weaponID..":"..mySerial..":"..getWeaponNameFromID(weaponID).."::")
						if give then
					
							exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "GIVEWEAPON "..getWeaponNameFromID(weaponID).." "..tostring(mySerial))
					...
					if count > 0 then
						...
						if (hiddenAdmin==0) then
							...
							exports.mrp_global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. getPlayerName(thePlayer) .. ", " .. targetPlayerName .. " isimli oyuncuya "..count.." adet " .. getWeaponNameFromID(weaponID) .. " verdi. Sebep: '"..tostring(reason).."'")
							exports.mrp_global:sendMessageToAdmins("AdmCmd: Silahın Seriali: '"..allSerials.."'")
						else 
...
function givePlayerGunAmmo(thePlayer, commandName, targetPlayer, ...)
	if exports.mrp_integration:isPlayerDeveloper(thePlayer) then
		...
		if not (targetPlayer) or (#args < 1) then
			exports["mrp_infobox"]:addBox(thePlayer, "warning", "/makeammo [Karakter Adı & ID] [Silah ID] [Mermi Miktarı] [Şarjör] [Tek Kelimeli Sebep]")
		else
			local targetPlayer, targetPlayerName = exports.mrp_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			...
			if targetPlayer then
				...
				elseif getAmmoPerClip(weaponID) == tostring(0)  then-- if weapon doesn't need ammo to work
						exports["mrp_infobox"]:addBox(thePlayer, "error", "Bu silahın mermisini alamazsın.")
					return
				else
				end
				...
				elseif (logged==1) then
					...
					local give, error = ""
					for variable = 1, quantity, 1 do
						give, error = exports.mrp_global:giveItem(targetPlayer, 116, weaponID..":"..ammo..":Ammo for "..getWeaponNameFromID(weaponID))
						if give then				
							exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "GIVEBULLETS "..getWeaponNameFromID(weaponID).." "..tostring(bullets))
					...
					if count > 0 then
						...
						if (hiddenAdmin==0) then
							...
							outputChatBox("#575757Melancia:#f9f9f9 Verdiğin mermi logu veritabanına kaydedildi.", thePlayer, 0, 255, 0, true)
							outputChatBox("#575757Melancia:#f9f9f9 Bir yetkili senin "..getWeaponNameFromID(weaponID).." silahına toplam "..ammo.." mermi iadesinde bulundu.", targetPlayer, 0, 255, 0, true)
							exports.mrp_global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. getPlayerName(thePlayer) .. ", " .. targetPlayerName .. " isimli oyuncuya "..ammo.." adet " .. getWeaponNameFromID(weaponID) .. " mermisi verdi. Sebep: '"..tostring(reason).."'")
...
function givePlayerItem(thePlayer, commandName, targetPlayer, itemID, ...)
	if (exports.mrp_integration:isPlayerDeveloper(thePlayer) or exports.mrp_integration:isPlayerScripter(thePlayer)) then
		if not (itemID) or not (...) or not (targetPlayer) then
			...
			if ( itemID == 74 or itemID == 150 or itemID == 75 or itemID == 78 or itemID == 2) and not exports.mrp_integration:isPlayerSeniorAdmin( thePlayer) then -- Banned Items
				exports.mrp_hud:sendBottomNotification(thePlayer, "Banned Items", "Only Lead+ Admin can spawn this kind of item.")
				return false
			end
			local itemValue = table.concat({...}, " ")
			itemValue = tonumber(itemValue) or itemValue

			if itemID == 114 and exports['mrp_shop']:getDisabledUpgrades()[tonumber(itemValue)] then
				outputChatBox("This item is temporarily disabled.", thePlayer, 255, 0, 0)
				return false
			end
			local targetPlayer, targetPlayerName = exports.mrp_global:findPlayerByPartialNick(thePlayer, targetPlayer)

			local preventSpawn = exports["mrp_items"]:getItemPreventSpawn(itemID, itemValue)
			if preventSpawn then
				exports.mrp_hud:sendBottomNotification(thePlayer, "Non-Spawnable Item", "This item cannot be spawned. It might be temporarily restricted or only obtainable IC.")
				return false
			end
			...
			if targetPlayer then
				...
				if ( itemID == 84 ) and not exports.mrp_integration:isPlayerAdmin( thePlayer ) then
				elseif itemID == 114 and not exports.mrp_integration:isPlayerTrialAdmin( thePlayer ) then
				...
				elseif (logged==0) then
					exports["mrp_infobox"]:addBox(thePlayer, "warning", "Oyuncu henüz giriş yapmamış.")
				elseif (logged==1) then
					local name = call( getResourceFromName( "mrp_items" ), "getItemName", itemID, itemValue )

					if itemID > 0 and name and name ~= "?" then
						local success, reason = exports.mrp_global:giveItem(targetPlayer, itemID, itemValue)
						if success then
							outputChatBox("Player " .. targetPlayerName .. " has received a " .. name .. " with value " .. itemValue .. ".", thePlayer, 0, 255, 0)
							exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "GIVEITEM "..name.." "..tostring(itemValue))
...
function givePedItem(thePlayer, commandName, ped, itemID, ...)
	if (exports.mrp_integration:isPlayerDeveloper(thePlayer) or exports.mrp_integration:isPlayerScripter(thePlayer)) then
		if not (itemID) or not (...) or not (ped) then
			...
			if ped then
				local element = exports.mrp_pool:getElement("ped", tonumber(ped))
				...
				if ( itemID == 74 or itemID == 150 or itemID == 75 or itemID == 78 or itemID == 2) and not exports.mrp_integration:isPlayerSeniorAdmin( thePlayer) then -- Banned Items
					exports.mrp_hud:sendBottomNotification(thePlayer, "Banned Items", "Only Lead+ Admin can spawn this kind of item.")
					return false
				elseif ( itemID == 84 ) and not exports.mrp_global:isPlayerAdmin( thePlayer ) then
				elseif itemID == 114 and not exports.mrp_global:isPlayerTrialAdmin( thePlayer ) then
				else
					local name = call( getResourceFromName( "mrp_items" ), "getItemName", itemID, itemValue )
					
					if itemID > 0 and name and name ~= "?" then
						local success, reason = exports.mrp_global:giveItem(element, itemID, itemValue)
						if success then
							outputChatBox("Ped "..tostring(pedname) or "".." (".. tostring(ped) ..") now has a " .. name .. " with value " .. itemValue .. ".", thePlayer, 0, 255, 0)
							exports.mrp_logs:dbLog(thePlayer, 4, ped, "GIVEITEM "..name.." "..tostring(itemValue))
							if element then
								exports['mrp_items']:npcUseItem(element, itemID)
							else
		...
		if not (itemID) or not (...) or not (targetPlayer) then
			...
			if targetPlayer then
				...
				if (logged==0) then
					exports["mrp_infobox"]:addBox(thePlayer, "warning", "Oyuncu henüz giriş yapmamış.")
				elseif (logged==1) then
					if exports.mrp_global:hasItem(targetPlayer, itemID, itemValue) then
						outputChatBox("You took item " .. itemID .. " with the value of (" .. itemValue .. ") from " .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
						exports.mrp_global:takeItem(targetPlayer, itemID, itemValue)
						exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "TAKEITEM "..tostring(itemID).." "..tostring(itemValue))
...
function setPlayerSkinCmd(thePlayer, commandName, targetPlayer, skinID, clothingID)
	if exports.mrp_integration:isPlayerDeveloper(thePlayer) then
		if not (skinID) or not (targetPlayer) then -- Clothing ID is a optional argument
			...
			local targetPlayer, targetPlayerName = exports.mrp_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			if not exports.mrp_integration:isPlayerDeveloper(thePlayer) then
			...
			if targetPlayer then
				...
				elseif tostring(type(tonumber(skinID))) == "number" and tonumber(skinID) ~= 0 then
					...
					if not (skin) and tonumber(oldSkin) ~= tonumber(skin) then
						...
						exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "SETSKIN "..tostring(skinID).." CLOTHING "..tostring(clothingID))
					end
				else
...
function nudgePlayer(thePlayer, commandName, targetPlayer)
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer)) then
		if not (targetPlayer) then
			...
			local logged = getElementData(targetPlayer, "loggedin")
			if (logged==0) then
			   exports["mrp_infobox"]:addBox(thePlayer, "error", "Bu oyuncu oyunda değil.")			
			   else
				triggerClientEvent ( "durtmeSesi", targetPlayer)
				outputChatBox("#575757Melancia:#f9f9f9 '" .. targetPlayerName .. "' isimli oyuncuyu dürttünüz.", thePlayer, 255, 194, 14, true)
		...
		if not (targetPlayer) then
			...
			if (logged==0) then
			   exports["mrp_infobox"]:addBox(thePlayer, "error", "Bu oyuncu oyunda değil.")			
				...
				outputChatBox("#575757Melancia:#f9f9f9 '" .. targetPlayerName .. "' isimli oyuncuyu ahlatarak dürttünüz.", thePlayer, 255, 194, 14, true)
...
addCommandHandler("paradagit",
	function(player, cmd, amount)
		if exports["mrp_integration"]:isPlayerDeveloper(player) then
...
function makeGenericItem(thePlayer, commandName, price, quantity, ...)
	if exports.mrp_global:isStaffOnDuty(thePlayer) and getElementData(thePlayer, "loggedin") == 1 then
		if not (price) or not (...) or not tonumber(price) or not (tonumber(price) > 0) or not (quantity) or not (tonumber(quantity) > 0) then
			...
			local fPrice = exports.mrp_global:formatMoney(price)
			if not exports.mrp_global:takeMoney(thePlayer, price) then
				outputChatBox("You could not afford $"..fPrice.." for a '"..itemValue.."'.", thePlayer, 255, 0, 0)
				return false
			end

			local success, reason = setTimer ( function ()
				exports.mrp_global:giveItem(thePlayer, 80, itemValue)
			end, 250, quantity )
			if success then
				local playerName = exports.mrp_global:getAdminTitle1(thePlayer)
				exports.mrp_global:sendWrnToStaff(playerName.." has created "..quantity.." '"..itemValue.."' to themselves for $"..fPrice.." (total: "..tonumber(quantity)*tonumber(fPrice).."$).", "MAKEGENERIC")

				exports.mrp_logs:dbLog(thePlayer, 4, thePlayer, commandName.." x"..quantity.." "..itemValue.." for "..fPrice)
...
function makeGenericItemCargo(thePlayer, commandName, price, ...)
	if getElementData(thePlayer, "factionleader") == 1 and getElementData(thePlayer, "loggedin") == 1 and tostring(getTeamName(getPlayerTeam(thePlayer))) == "Cargo Group" then
		...
		if not (price) or not (...) or not tonumber(price) or not (tonumber(price) > 0) then
			...
			local fPrice = exports.mrp_global:formatMoney(price)
			if not exports.mrp_global:takeMoney(getTeamFromName("Cargo Group"), price) then
				outputChatBox("You could not afford $"..fPrice.." for a '"..itemValue.."'.", thePlayer, 255, 0, 0)
				return false
			end
			local success, reason = exports.mrp_global:giveItem(thePlayer, 80, itemValue)
			if success then
				local playerName = exports.mrp_global:getPlayerFullIdentity(thePlayer)
				exports.mrp_global:sendMessageToStaff("[CARGO GROUP] "..playerName.." has created a '"..itemValue.."' to themselves for $"..fPrice..".")
				exports.mrp_logs:dbLog(thePlayer, 4, thePlayer, "Cargo Group "..commandName.." "..itemValue.." for "..fPrice)
				triggerClientEvent(thePlayer, "item:updateclient", thePlayer)
				return true
			else
				outputChatBox("Failed to created generic item. Reason: " .. tostring(reason), thePlayer, 255, 0, 0)
			...
			if (tplayer) then
					...
					if (newl) then
						if (newl>0) then
							...
							exports.mrp_logs:dbLog(thePlayer, 4, tplayer, "SETVEHLIMIT "..oldvl.." "..newl)
						else
