...
function ckPlayer(thePlayer, commandName, targetPlayer, ...)
	if exports.mrp_integration:isPlayerDeveloper(thePlayer) then
		if not (targetPlayer) or not (...) then
			outputChatBox("* Melancia * /" .. commandName .. " [Player Partial Nick / ID] [Cause of Death]", thePlayer, 255, 194, 14)
		else
			...
			if targetPlayer then
				...
				elseif (logged==1) then
					...
					local desc = look[5]
					call( getResourceFromName( "mrp_realism" ), "addCharacterKillBody", x, y, z, rotation, skin, getElementData(targetPlayer, "dbid"), targetPlayerName, getElementInterior(targetPlayer), getElementDimension(targetPlayer), getElementData(targetPlayer, "age"), getElementData(targetPlayer, "race"), getElementData(targetPlayer, "weight"), getElementData(targetPlayer, "height"), desc, info, getElementData(targetPlayer, "gender"))
					...
					exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "CK with reason: "..(tostring(info)))
...
function unckPlayer(thePlayer, commandName, ...)
	if exports.mrp_integration:isPlayerDeveloper(thePlayer) then
		if not (...) then
			outputChatBox("* Melancia * /" .. commandName .. " [Full Player Name]", thePlayer, 255, 194, 14)
		else
			...
			elseif (mysql:num_rows(result)==0) then
				...
				--exports.mrp_logs:logMessage("[/UNCK] " .. getElementData(thePlayer, "account:username") .. "/".. getPlayerName(thePlayer) .." UNCK'ED ".. targetPlayer , 4)
				exports.mrp_logs:dbLog(thePlayer, 4, "ch"..row["id"], "UNCK")
			end
