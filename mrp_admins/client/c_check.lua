...
function OpenCheck( ip, adminreports, donPoints, note, history, warns, transfers, bankmoney, money, adminlevel, hoursPlayed, accountname, hoursAcc )
	...
	if not exports.mrp_integration:isPlayerTrialAdmin(getLocalPlayer()) then
		...
		guiSetText ( Label[3], "Money: $" .. exports.mrp_global:formatMoney(money) .. " (Bank: $" .. exports.mrp_global:formatMoney(bankmoney) .. ")")
	end
	guiSetText ( Button[5], history )
	guiSetText ( Label[20], "GameCoins: " .. exports.mrp_global:formatMoney(donPoints) )
	guiSetText ( Label[19], "Hours Char: " .. ( hoursPlayed or "N/A" ) .. "\n~ Total: " .. ( hoursAcc or "N/A" ) )
	
	if (player == getLocalPlayer()) and not exports.mrp_integration:isPlayerAdmin(getLocalPlayer()) then
		guiSetText ( memo, "-You can not view your own note-")
		guiMemoSetReadOnly(memo, true)
		guiSetEnabled(Button[4], false)
	elseif not exports.mrp_integration:isPlayerTrialAdmin(getLocalPlayer()) then
...
addEventHandler( "onClientRender", getRootElement(),
	function()
		if guiGetVisible(Window) and isElement( player ) then
			local x, y, z = 0, 0, 0
			if not exports.mrp_integration:isPlayerSupporter(getLocalPlayer()) and getElementAlpha(player) ~= 0 or exports.mrp_integration:isPlayerLeadAdmin(getLocalPlayer()) then 
			...
			if vehicle and not exports.mrp_integration:isPlayerSupporter(getLocalPlayer()) then
				guiSetText ( Label[10], "Vehicle: " .. exports.mrp_global:getVehicleName( vehicle ) .. " (" ..getVehicleName( vehicle ).." - "..getElementData( vehicle, "dbid" ) .. ")" )
			else
				guiSetText ( Label[10], "Vehicle: N/A")
			end
			if not exports.mrp_integration:isPlayerSupporter(getLocalPlayer()) then
				guiSetText ( Label[12], "Location: " .. getZoneNameEx( x, y, z ) )
	...
	function( info, targetID )
		if wHist then
			...
			addEventHandler( "onClientGUIClick", bremove,
				function( button, state )
					if exports.mrp_integration:isPlayerTrialAdmin(localPlayer) or exports.mrp_integration:isPlayerSupporter(localPlayer) then
						...
						if row ~= -1 and col ~= -1 then
							...
							if not exports.mrp_integration:isPlayerLeadAdmin(localPlayer) and tonumber(record[8]) ~= getElementData(localPlayer, "account:id") then
