...
local staffTitles = exports.mrp_integration:getStaffTitles()
local currentPos = nil
local futurePos = nil

function openStaffManager(staffInfo1, staffTeams1, globalChangelogs1) --/ bekiroj 8 mayıs 05:46
	if canPlayerAccessStaffManager(localPlayer) then
		staffTitles = exports.mrp_integration:getStaffTitles()
		if GUIEditor.window[1] and isElement(GUIEditor.window[1]) then
	        ...
	        GUIEditor.window[1] = guiCreateWindow(538, 144, 700, 461+yExtend, "Melancia - Yetkili Yönetim Arayüzü © bekiroj", false)
	        guiWindowSetSizable(GUIEditor.window[1], false)
	        exports.mrp_global:centerWindow(GUIEditor.window[1])
	        GUIEditor.tabpanel[1] = guiCreateTabPanel(9, 29, 700, 386+yExtend, false, GUIEditor.window[1])
	        
	        GUIEditor.tab.home = guiCreateTab("Yetkilendirme", GUIEditor.tabpanel[1])
	        
	        GUIEditor.label[1] = guiCreateLabel(10, 16, 56, 28, "Arat:", false, GUIEditor.tab.home)
	        	...
	        	if source == GUIEditor.edit[1] then
	        		...
	        		if string.len(text) >= 3 then
		        		...
		        		timer.username = setTimer(function() 
		        			local username = exports.mrp_cache:getUsername(text)
		        			if username then
		        				guiSetText(GUIEditor.label[2], "Bulundu: "..username..". Veritabanında kayıtlı.")
		        				guiLabelSetColor ( GUIEditor.label[2], 0,255,0 )
		        				killTimerIfExisted(timer.username)
		        				triggerServerEvent("staff:getStaffInfo", localPlayer, username)
		        			else
		        				if count > 4 then
		        					guiSetText(GUIEditor.label[2], "Kullanıcı adı bulunamadı.")
	        ...
	        GUIEditor.label[2] = guiCreateLabel(10, 44, 306, 23, "Lütfen kullanıcı adı giriniz", false, GUIEditor.tab.home)
	        ...
	        for i, rank in ipairs(staffTitles[1]) do
	       	...
	       	exports.mrp_global:guiComboBoxAdjustHeight(GUIEditor.combobox.admin, count)
	        ...
	        for i, rank in ipairs(staffTitles[2]) do
	       	...
	       	exports.mrp_global:guiComboBoxAdjustHeight(GUIEditor.combobox.sup, count)
	        ...
	        for i, rank in ipairs(staffTitles[3]) do
	       	...
	       	exports.mrp_global:guiComboBoxAdjustHeight(GUIEditor.combobox.vct, count)

	        GUIEditor.label[6] = guiCreateLabel(10, 165, 74, 22, "Yazılımcı:", false, GUIEditor.tab.home)
	        ...
	        for i, rank in ipairs(staffTitles[4]) do
	       	...
	       	exports.mrp_global:guiComboBoxAdjustHeight(GUIEditor.combobox.scripter, count)
	        ...
	        for i, rank in ipairs(staffTitles[5]) do
	       	...
	       	exports.mrp_global:guiComboBoxAdjustHeight(GUIEditor.combobox.mapper, count)
	        ...
	        GUIEditor.label[13] = guiCreateLabel(10, 197, 74, 22, "Mapping:", false, GUIEditor.tab.home)
	        guiSetFont(GUIEditor.label[13], "default-bold-small")
	        guiLabelSetVerticalAlign(GUIEditor.label[13], "center")

	        GUIEditor.label[12] = guiCreateLabel(10, 197+yExtend, 74, 22, "Kayıt Geçmişi:", false, GUIEditor.tab.home)
	        ...
	        GUIEditor.gridcol.date = guiGridListAddColumn(GUIEditor.gridlist[1], "Tarih", 0.25)
	        GUIEditor.gridcol.type = guiGridListAddColumn(GUIEditor.gridlist[1], "Tip", 0.1)
	        GUIEditor.gridcol.from = guiGridListAddColumn(GUIEditor.gridlist[1], "Önceki", 0.2)
	        GUIEditor.gridcol.to = guiGridListAddColumn(GUIEditor.gridlist[1], "Sonraki", 0.2)
	        GUIEditor.gridcol.by = guiGridListAddColumn(GUIEditor.gridlist[1], "Tarafından", 0.1)
	        GUIEditor.gridcol.details = guiGridListAddColumn(GUIEditor.gridlist[1], "Açıklama", 0.1)

	        GUIEditor.button[1] = guiCreateButton(411, 16, 122, 28, "Onayla", false, GUIEditor.tab.home)
	        ...
	        for i, team in ipairs(staffTitles) do
		        ...
		        GUIEditor.gridcolStaff[i].rating = guiGridListAddColumn(GUIEditor.gridlistStaff[i], "Feedback Rating (out of 5)", 0.12)
		        GUIEditor.gridcolStaff[i].feedbacks = guiGridListAddColumn(GUIEditor.gridlistStaff[i], "Feedback Count", 0.12)
		        GUIEditor.gridcolStaff[i].rdstats = guiGridListAddColumn(GUIEditor.gridlistStaff[i], "Rol Dersi", 0.12)
		        ...
		        addEventHandler( "onClientGUIDoubleClick", GUIEditor.gridlistStaff[i], function(button, state)
		            ...
		            elseif button == "right" then
		               ...
		               local rdstats = (guiGridListGetItemText( GUIEditor.gridlistStaff[i], selectedRow, 6 ) or 0)
		               if setClipboard(rank.." "..username.." "..reports) then
		               		outputChatBox("Copied '"..rank.." "..username.." "..reports.." "..rating.." "..feedbacks.." "..rdstats.."' to clipboard. Double right click to edit this staff.")
		               end
	        ...
	        GUIEditor.button[2] = guiCreateButton(10, 425+yExtend, 700, 26, "Close", false, GUIEditor.window[1])  
...
function onTabSwitch( selectedTab )
	if GUIEditor.window[1] and isElement(GUIEditor.window[1]) then
		...
		if tab == "Admin Team" or tab == "Supporter Team" or tab == "VCT Team" or tab == "Scripting Team" or tab == "Mapping Team" then 
	...
	if GUIEditor.window[1] and isElement(GUIEditor.window[1]) and info and type(info) == "table" then
		...
		if user then
			if user.username then
				...
				guiSetText(GUIEditor.label[2], info.error or "Kullanıcı adı '"..user.username.."' veritabanında bulundu.")
				guiLabelSetColor ( GUIEditor.label[2], 0,255,0 )
				if info.error then
					clearStatusText()
				end
		...
		if changelogs then
			...
			for i=1, #changelogs do
				...
				guiGridListSetItemText(GUIEditor.gridlist[1], row, GUIEditor.gridcol.type, changelogs[i].promoted == "1" and "Terfi" or "Tenzil", false, false)
				...
				guiGridListSetItemText(GUIEditor.gridlist[1], row, GUIEditor.gridcol.details, changelogs[i].details or "Yok" , false, false)
			end
		end
	end
end
	...
	if GUIEditor.window[1] and isElement(GUIEditor.window[1]) and info and type(info) == "table" and #info > 0 then
		...
		for i, team in ipairs(staffTeams) do
			...
			for j, user in ipairs(team) do
				...
				guiGridListSetItemText(GUIEditor.gridlistStaff[i], row, GUIEditor.gridcolStaff[i].rdstats, user.rdstats, false, true)
			end
		end
		for i, rank in ipairs(staffTitles) do
			guiSetVisible(GUIEditor.gridlistStaff[i], true)
...
function formatRating(rating)
	if not rating or not tonumber(rating) then
		...
		return exports.mrp_global:round(rating, 2)
	end
end
	...
	if GUIEditor.window[1] and isElement(GUIEditor.window[1]) and info and type(info) == "table" and #info > 0 then
		...
		if changelogs then
			...
			for i=1, #changelogs do
				...
				guiGridListSetItemText(GUIEditor.gridlist.changelogs, row, GUIEditor.gridcolChangeLogs.type, changelogs[i].promoted == "1" and "Terfi" or "Tenzil", false, false)
				...
				guiGridListSetItemText(GUIEditor.gridlist.changelogs, row, GUIEditor.gridcolChangeLogs.details, changelogs[i].details or "Yok" , false, false)
			end
		end
		guiSetVisible(GUIEditor.gridlist.changelogs, true)
		guiSetVisible(GUIEditor.label.cloading, false)
...
function guiStaffChange()
	...
	if source == GUIEditor.combobox.admin then
		...
		if (not exports.mrp_integration:isPlayerSeniorAdmin(localPlayer) or myLevel < admin or myLevel < currentPos[1]) and not exports.mrp_integration:isPlayerLeadScripter(localPlayer) then
	...
	elseif source == GUIEditor.combobox.sup then
		...
		if not exports.mrp_integration:isPlayerSeniorAdmin(localPlayer) and not exports.mrp_integration:isPlayerSupportManager(localPlayer) then
	...
	elseif source == GUIEditor.combobox.vct then
		...
		if not exports.mrp_integration:isPlayerSeniorAdmin(localPlayer) and (myLevel <= vct or myLevel <= currentPos[3]) then
	...
	elseif source == GUIEditor.combobox.scripter then
		...
		if not exports.mrp_integration:isPlayerLeadScripter(localPlayer) then
	...
	elseif source == GUIEditor.combobox.mapper then
		...
		if not exports.mrp_integration:isPlayerLeadScripter(localPlayer) and (myLevel <= mapper or myLevel <= currentPos[5]) then
...
function showConfirmEditStaff()
    ...
    exports.mrp_global:centerWindow(GUIEditor.window[2])
