...
function getAdminTitle(thePlayer)
	return exports.mrp_global:getPlayerAdminTitle(thePlayer)
end
...
local function updateGUI()
	if show then
		...
		local adminLevel = exports.mrp_global:getPlayerAdminLevel(localPlayer) or 0
		if exports.mrp_integration:isPlayerSupporter(localPlayer) or exports.mrp_integration:isPlayerTrialAdmin(localPlayer) then
...
addCommandHandler("staffoverlay", function()
 if exports.mrp_integration:isPlayerSupporter(localPlayer) or exports.mrp_integration:isPlayerTrialAdmin(localPlayer) then
	if (show == true) then
		show = false
		exports["mrp_infobox"]:addBox("info", "Yetkili arayüzünü in-aktif hale getirdin. Raporları farklı bir şekilde okumak istiyorsan /rapordurum ac yazabilirsin.")
	else
		show = true
		exports["mrp_infobox"]:addBox("info", "Yetkili arayüzünü aktif hale getirdin.")
	end
 end
end)
...
function drawText ( )
	if show and exports.mrp_global:isStaffOnDuty(localPlayer) then
