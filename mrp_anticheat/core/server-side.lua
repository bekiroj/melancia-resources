function showSpeedToAdmins(velocity)
	...
	exports.mrp_global:sendMessageToAdmins("[Possible Speedhack/HandlingHack] " .. getPlayerName(client) .. ": " .. velocity .. "Mph/".. kph .." Kph")
end
...
function showDMToAdmins(kills)
	exports.mrp_global:sendMessageToAdmins("[Possible DeathMatching] " .. getPlayerName(client) .. ": " .. kills .. " kills in <=2 Minutes.")
end
...
function scanMoneyHacks()
	...
	for key, value in ipairs(players) do
		...
		if (logged==1) then
			if not (exports.mrp_integration:isPlayerTrialAdmin(value)) then
				
				local money = getPlayerMoney(value)
				local truemoney = exports.mrp_global:getMoney(value)
				if (money) then
