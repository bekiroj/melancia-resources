...
function drawTooltip(type, text, spec)
    ...
    if type == 1 then
        ...
        cx = cx - w/2
        cy = cy - h
        dxDrawRectangle(cx,cy,w,h,tocolor(55,55,55))
        --dxDrawRoundedRectangle(cx-4,cy + h/2,w+8,20,tocolor(55,55,55),tocolor(55,55,55))
        dxDrawText(text,cx,cy,cx+w,cy+h, tocolor(255,255,255,255),1,font,"center","center", false, false, false, true)
    elseif type == 2 then
        local font3 = exports['mrp_fonts']:getFont("Rubik-Regular", 10)
        ...
        local alpha = 255
        local size = 1
        local startX, startY = exports['mrp_core']:getCursorPosition()
        if spec then
        ...
        dxDrawOuterBorder(startX - length/2, startY - h, length, h, 2 * size, tocolor(30,30,30,math.max(alpha - 105, 0)))
        _dxDrawRectangle(startX - length/2, startY - h, length, h, tocolor(44, 44, 44, alpha * 0.85))
        dxDrawText(text,startX, startY - h, startX, startY, tocolor(255,255,255,alpha), size, font3, "center", "center", false, false, false, true)
    end
end
