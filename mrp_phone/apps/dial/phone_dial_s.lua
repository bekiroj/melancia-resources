...
function startDialing(to, from)
	...
	dbQuery(
		function(qh, source)
			local res, rows, err = dbPoll(qh, 0)
				...
				exports.mrp_anticheat:changeProtectedElementDataEx(source, "callingContact", contact, false)
					...
					if not foundInGame then
						...
						exports.mrp_anticheat:changeProtectedElementDataEx(source, "call.col", publicphone, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(source, "calling", tonumber(contact.entryNumber), false)
						exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, "calling", tonumber(from), false)
						exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, "callingwith", tonumber(contact.entryNumber), false)
						exports.mrp_anticheat:changeProtectedElementDataEx(source, "called", false, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, "called", true, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(source, "phonestate", 2, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, "phonestate", 3, false)
						...
						if t_ringtone > 1 and tone_volume > 0 then
							for _,nearbyPlayer in ipairs(exports.mrp_global:getNearbyElements(targetPlayer, "player"), 10) do
						...
						exports['mrp_logs']:dbLog(source, 29, { source, "ph"..tostring(from), targetPlayer, "ph"..tostring(contact.entryNumber) }, "**Starting call - " .. (contact.entryName or contact.entryNumber) .. "**")
						return true
	...
	{source}, mysql:getDBConnection(), "SELECT * FROM `phone_contacts` WHERE `entryName`='" .. (tostring(to)) .. "' OR `entryNumber`='" .. (tostring(to)) .. "' AND `phone`='" .. (tostring(from)) .."' LIMIT 1")
...
function makeCall(thePlayer)
	outputChatBox("#575757Melancia:#f9f9f9 Telefon görüşmesi gerçekleştirebilmek için telefonu açmalısın.",thePlayer, 255, 0, 0, true)
end
...
function acceptPhoneCall()
	...
	if caller then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(caller, "phonestate", 4, false)
		exports.mrp_anticheat:changeProtectedElementDataEx(source, "phonestate", 5, false)
...
function takeCallCost(cost, fromNumber, duration)
	if cost > 0 then
		if exports.mrp_bank:takeBankMoney(source, cost) then
			...
			if exports.mrp_global:giveMoney(foundFaction, cost) then
				return exports.mrp_bank:addBankTransactionLog(getElementData(source, "dbid"), -20, cost, 2, "Cellphone's phone call fee", "Call made from #"..fromNumber..", duration: "..duration, nil, nil)
			end
		end
	end
	return false
...
function loudSpeaker(thePlayer, commandName)
	...
	if (logged==1) then
		if (exports.mrp_global:hasItem(thePlayer, 2)) or getElementData(thePlayer, "call.col") then -- 2 = Cell phone item
			...
			if phonestate == 4 or phonestate == 5 then
				...
				if (not loudspeaker) then
					...
					exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "call.loudspeaker", true, false)
				else
					...
					exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "call.loudspeaker", false, false)
				end
			end
		end
	end
...
function talkPhone(thePlayer, commandName, ...)
	...
	if (logged==1) then
		if (exports.mrp_global:hasItem(thePlayer, 2)) or getElementData(thePlayer, "call.col") then
			if not (...) then
			...
			elseif getElementData(thePlayer, "injuriedanimation")  then
				...
				if (phoneState == 4 or phoneState == 5) then 
					...
					local languagename = call(getResourceFromName("mrp_languages"), "getLanguageName", language)
					...
					message = call( getResourceFromName( "mrp_chat" ), "trunklateText", thePlayer, message )
					...
					if (callprogress) then
						...
						exports.mrp_global:sendLocalText(thePlayer, username .. " [Telefon]: " .. message, nil, nil, nil, 10, {[thePlayer] = true})
						
						if isNumberAHotline(callingNumber) then
							writeCellphoneLog(thePlayer, nil, "Calls", message )
							exports['mrp_logs']:dbLog(thePlayer, 29, affected, "[" .. languagename .. "] " ..message) 
					...
					local message2 = message--call(getResourceFromName("mrp_languages"), "applyLanguage", thePlayer, target, call( getResourceFromName( "mrp_chat" ), "trunklateText", target, message ), language)
					local playerItems = exports["mrp_items"]:getItems(thePlayer)
					for i, val in ipairs(playerItems) do
						if val[1] == 2 then
							...
							--exports.mrp_global:sendLocalText(target, username .. "[Telefon "..taksiciTel.."]["..telefonCinsiyet.."] (Arka Ses): " .. message, 178, 178, 178, 10, {[thePlayer] = true})
					...
					if (loudspeaker) then -- Loudspeaker
						...
						for index, nearbyPlayer in ipairs(getElementsByType("player")) do
							if isElement(nearbyPlayer) and nearbyPlayer ~= target and getDistanceBetweenPoints3D(x, y, z, getElementPosition(nearbyPlayer)) < 40 and getElementDimension(nearbyPlayer) == getElementDimension(target) then
								local message2 = call(getResourceFromName("mrp_languages"), "applyLanguage", thePlayer, nearbyPlayer, call( getResourceFromName( "mrp_chat" ), "trunklateText", target, message ), language)
...
function resetPhoneState(thePlayer)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "callingwith", nil, false)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "calling", nil, false)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "called", nil, false)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "phonestate", nil, false)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "calltimer", nil, false)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "callingContact", nil, false)

	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "call.col", nil, false)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "callprogress", false)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "call.situation", false)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "call.location", false)
	exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "call.loudspeaker", false)
