...
function drawPhoneDial()
	if isPhoneGUICreated() then
		...
		guiSetFont ( ePhoneNumber, fonts[7] )
		...
		bCall = guiCreateButton(30,155,204/3,30,"Ara",false,wPhoneMenu)
		guiSetFont ( bCall, fonts[7] )
		bWhatsapp = guiCreateButton(30+(204/3),155,204/3,30,"Mesajla",false,wPhoneMenu)
		guiSetEnabled(bWhatsapp, false)
		guiSetFont ( bWhatsapp, fonts[7] )
		bKonumAt = guiCreateButton(30+(204)-(204/3),155,204/3,30,"Konum",false,wPhoneMenu)
		guiSetFont ( bKonumAt, fonts[7] )
...
function startDialing(from, to, popOutOnPhoneCall)
	if from and to and string.len(to) > 0 then
		...
		if not popOutOnPhoneCall then
			killDialingSounds()
			killDialingTimers()
			--local sound = playSound("components/sounds/touch_tone.mp3")
		...
		lCallingTo = guiCreateLabel(margin,posY-10,lineW,40, getContactNameFromContactNumber(callingTo[1], phone) or callingTo[1], false, wPhoneMenu)
		...
		if popOutOnPhoneCall then
			...
			playSound("components/sounds/ringtones/viberate.mp3")
...
function finishPhoneCall(reason)
	killDialingSounds()
	killDialingTimers()
	playSound("components/sounds/hangup.mp3")
		...
		if action then
			...
			elseif action == "start_dialing_tone" then
				if data then
					...
					local dialingTone = playSound("components/sounds/dialing_tone.mp3", true)
			...
			elseif action == "start_invalid_or_busy_tone" then
				...
				if data == "not_existed" then 
					local sound1 = playSound("components/sounds/invalid_tone.mp3")
					setSoundVolume(sound1, 0.2)
					table.insert(dialingSounds, sound1)
				elseif data == "out_of_service" then
					local sound1 = playSound("components/sounds/busy_tone.mp3", true)
			...
			elseif action == "connected" then
				if data then
					killDialingTimers()
					killDialingSounds()
					local sound1 = playSound("components/sounds/ringtones/viberate.mp3")
					table.insert(dialingSounds, sound1)
...
function drawRinging(phoneRinging)
	...
	local phoneRinging = getContactNameFromContactNumber(phoneRinging, phone) or phoneRinging
	if wRing and isElement(wRing) then
		guiSetEnabled(bRingingAnswer, true)
    	guiSetEnabled(bRingingDecline, true)
    	guiSetText(wRing, phoneRinging.." sizi arıyor.")
		return true
	end
	wRing = guiCreateWindow(ringOffSetOut,y,w,h, phoneRinging.." sizi arıyor.", false)
    guiWindowSetMovable(wRing, false)
    guiWindowSetSizable(wRing, false)

    bRingingAnswer = guiCreateButton(10, 28, 117, 35, "Cevapla", false, wRing)
    bRingingDecline = guiCreateButton(129, 28, 118, 35, "Meşgul", false, wRing) 
...
function startPressingDialSound(numbers)
	if numbers and tonumber(numbers) then
		...
		if soundToPlay then
			local sound = playSound("components/sounds/beeps/"..soundToPlay..".mp3")
			table.insert(dialingSounds, sound)
		else
			startPressingDialSoundIndex = 0
		end
