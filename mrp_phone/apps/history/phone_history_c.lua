...
function refreshHistory(newHistory, xoffset, yoffset, fromServer)
	...
	if #histories[tonumber(phone)] > 0 then
		for i = 1, #histories[tonumber(phone)] do 
		    ...
		    guiCreateStaticImage(10+xoffset, 29+yoffset, 16, 16, "components/images/"..flow..".png", false, wHistory)
			...
			if not history_unixtime or history_unixtime == nil then history_unixtime = "" end
		    ...
		    if private == 0 then
			    guiCreateStaticImage(163+xoffset, 15+yoffset, 48, 14, "components/images/call.png", false, wHistory)
			    ...
			    if tonumber(history_number) then
				    ...
				    guiCreateStaticImage(179+xoffset, 16+yoffset, 17, 12, "components/images/sms.png", false, wHistory)
				    history_btn_sms[i] = guiCreateButton(163+xoffset, 14+yoffset, 48, 16, "", false, wHistory)
				    guiSetAlpha(history_btn_sms[i], 0.3)
				    yoffset = yoffset - (31 - 14)
				else
					guiCreateStaticImage(167+xoffset, 33+yoffset, 15, 10, "components/images/sms.png", false, wHistory)
				    history_btn_sms[i] = guiCreateButton(163+xoffset, 31+yoffset, 24, 15, "", false, wHistory)
				    guiSetAlpha(history_btn_sms[i], 0.3)

				    guiCreateStaticImage(191+xoffset, 31+yoffset, 14, 14, "components/images/save.png", false, wHistory)  
				    history_btn_save[i] = guiCreateButton(187+xoffset, 31+yoffset, 24, 14, "", false, wHistory)    
				    guiSetAlpha(history_btn_save[i], 0.3)
				end
			end
			--guiSetAlpha(guiCreateStaticImage(10+xoffset, 50+yoffset, 200, 1, ":admin-system/components/images/whitedot.jpg", false, wHistory), 0.1)
		    yoffset = yoffset + 40 
		end
		...
		guiCreateLabel(0.5, 0.5, 1, 0.5, "Hiç çağrı kaydı yok.", true, wHistory)
	end
end
...
function analyzeHistory(his)
	...
	--outputChatBox("exports.mrp_mrp_datetime:formatTimeInterval( datesec ) - "..exports.mrp_mrp_datetime:formatTimeInterval( datesec ))
	return history_name, history_number, exports.mrp_datetime:formatTimeInterval( datesec ), flow, private
end
