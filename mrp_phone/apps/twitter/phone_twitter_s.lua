...
function hasAccess(player, hashKey)
    ...
    if exports.mrp_integration:isPlayerStaff(player) then
        return true
    end
    return false
end
    ...
    function(player, name, location, text)
        ...
        for k,v in ipairs(advertisementMessages) do
            ...
            if (found) or (found2) or (string.lower(text)==tostring(v)) then
                exports.mrp_global:sendMessageToAdmins("AdmWrn: "..getPlayerName(player).." adlı oyuncu yasaklı tweet attı, mesajı:" .. tostring(text))
                outputChatBox("#575757Melancia:#ffffff Tweet yazısında yasaklı kelimeler bulundu.", player, 255, 0, 0, true)
                return
            end
        end
        if not exports.mrp_integration:isPlayerDeveloper(player) then
