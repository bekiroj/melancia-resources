...
addEvent("phone:getPhoneSettings" , true)
addEventHandler("phone:getPhoneSettings" , root , getPhoneSettings)
...
function drawSettings(xoffset, yoffset)
    ...
    guiSetAlpha(guiCreateStaticImage(10+xoffset, 28+yoffset, 200, 1, ":mrp_carshop\img\whitedot.png", false, settingGuis.main), 0.1)
    local smallLine = 16
    yoffset = yoffset + 26
...
function guiSettingClick()
	...
	local playSound = function(tone, loop)
		if tone ~= "components/sounds/ringtones/quiet" then
	...
	elseif source == settingGuis[7] then --previous ringtone
		...
		if index and name then
			playSound("components/sounds/ringtones/"..name, true)
	...
	elseif source == settingGuis[12] then --previous sms_tone
		...
		if index and name then
			playSound("components/sounds/ringtones/"..name)
	...
	elseif source == settingGuis[26] then --previous tone_volume
		...
		if newVol then
			...
			playSound("components/sounds/beeps/8.mp3")
	...
	elseif source == settingGuis[32] then
		if guiGetText(settingGuis[32]) == "Aktif" then
			...
			--exports.mrp_mrp_OwlGamingLogs:drawInfoBox()
		end
