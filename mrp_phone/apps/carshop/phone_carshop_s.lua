...
function phoneBuyVeh(vehid, brandid, rgb, balance, vehname, plateState, plateEditText)
	...
	if tonumber(vehid) and tonumber(brandid) then
		if exports.mrp_global:takeMoney(player, balance) then
			...
			exports.mrp_global:sendMessageToAdmins("[PHONE CARSHOP] '" .. getPlayerName(player) .. "' isimli oyuncu "..vehname.." isimli aracı satın aldı.")
					...
					function(qh)
						...
						if rows > 0 then
							...
							if not insertid then
								...
								exports.mrp_global:sendMessageToAdmins("[PHONE CARSHOP] '" .. getPlayerName(player) .. "' isimli oyuncu "..vehname.." marka aldığı araç işlenemediği için parası iade edildi.")
			...
			outputChatBox("#575757Melancia:#f9f9f9 Bu aracı sipariş verebilmen için yeterli paran yok.", player, 255, 0, 0, true)
		end
	end
end
