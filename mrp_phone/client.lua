...
function createSetupGUIs()
	...
	addEventHandler("onClientGUIClick", setupBtn, function(state)
		if state == "left" then
			...
			if text:len() == 4 then
				outputChatBox("#575757Melancia:#ffffff Şifreniz kabul edildi, lütfen bekleyin.", 0, 255, 0, true)
	...
	addEventHandler("onClientGUIClick", setupBtn, function(state)
		if state == "left" then
			...
			if (tonumber(text) == tonumber(phonePassword)) then
				...
				outputChatBox("#575757Melancia:#ffffff Hatalı şifre girdiniz, tekrar deneyin.", 255, 0, 0, true)
			end
		end
	end)
end
...
function drawnPhone()
	...
	if phoneSetupState == 2 then
		...
		if appstoreActive then
			...
			--@draw AppStore
			c_apps = { ['twitter'] = false, ['safari'] = false, ['vergi'] = false, ['lsbank'] = false, ['spotify'] = false, ['emails'] = false, ['flappy-bird'] = false }
		...
		elseif isBankShowing and not isBankShowingLogged then
			dxDrawImage(x,y,w,h,"components/images/iphone_front_lsbank.png",0,0,0,tocolor(255,255,255))
		elseif isBankShowing and isBankShowingLogged then
			dxDrawImage(x,y,w,h,"components/images/iphone_front_lsbank_logged.png",0,0,0,tocolor(255,255,255))			
	...
	if drawApps then
		...
		c_apps = {"twitter", "safari", "vergi", "lsbank", "spotify", "emails", "flappy-bird"}
		-- First check it:
...
function drawHomeMidBlock()
	if isPhoneGUICreated() then
		...
		exports.mrp_global:guiComboBoxAdjustHeight(cHotlines, 12)
...
function hidePhoneGUI()
	...
	closeConfirmBox()
	killSettingSounds()
	--exports.mrp_mrp_OwlGamingLogs:closeInfoBox()
end
...
addCommandHandler("telefonfix", function()
...
function onGuiClick(button)
	...
	elseif button == "left" and phoneSetupState == 2 then
		...
		elseif source == bKonumAt then
			...
			--	outputChatBox("#575757Melancia:#ffffff Konum başarıyla atıldı!",0,255,0,true)
...
addEventHandler("konum:markerAc",root,function(p1,p2,x,y,z)
	...
	blip[p2] = createBlip(x,y,z,41)--waypoint blip
 	addEventHandler("onClientMarkerHit",marker[p2],function(hitElement)
		...
		outputChatBox("#575757Melancia:#ffffff Konuma ulaştınız!",0,255,0,true)
	end)
end)
...
addCommandHandler('bildirim',
	function(cmd,state)
		if (not state) then
			outputChatBox("#575757Melancia:#ffffff /bildirim [aç/kapat]",0,0,255,true)
			return
		end
		if (state == "aç") then
			outputChatBox("#575757Melancia:#ffffff Bildirimler başarıyla açıldı.", 0, 255, 0, true)
			localPlayer:setData('notifications->disabled', false)
		elseif (state == "kapat") then
			outputChatBox("#575757Melancia:#ffffff Bildirimler başarıyla kapatıldı.", 0, 255, 0, true)
