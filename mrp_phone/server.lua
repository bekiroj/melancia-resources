...
function initiatePhoneGUI(phone, popOutOnPhoneCall)
    ...
    {source}, mysql:getDBConnection(), "SELECT *, `charactername`, DATE_FORMAT(`bought_date`,'%b %d %Y %h:%i %p') AS `bought_date` FROM `phones` LEFT JOIN `characters` ON `phones`.`boughtby` = `characters`.`id` WHERE `phonenumber`='"..(tostring(phone)).."' LIMIT 1")
...
addEventHandler("installApp", root,
	function(data)
		...
		dbExec(mysql:getDBConnection(), "UPDATE phones SET custom_settings='"..state.."' WHERE phonenumber='"..phone.."'")
	end
)
...
addEventHandler("fetchApps", root,
	function(phone)
		local apps =  { ['twitter'] = true, ['safari'] = true, ['vergi'] = true, ['spotify'] = true, ['lsbank'] = false, ['emails'] = true, ['flappy-bird'] = true }
...
addEventHandler("completePhoneSetup", root,
	function(player, phoneNumber, phonePassword)
		exec = dbExec(mysql:getDBConnection(), "UPDATE phones SET phoneSetup=?, phonePassword=? where phonenumber=?",2,phonePassword, phoneNumber)
		if exec then
			outputChatBox("#575757Melancia:#ffffff Telefon kurulumu tamamlandı, şifreniz: "..phonePassword,player,0,255,0,true)
...
addEventHandler("logIn:lsbank", root,
	function(player,cardnumber,cardpassword)
		instance:query("SELECT * FROM atm_cards WHERE card_number='"..cardnumber.."'", {player, cardnumber, cardpassword},
			function(res, rows, err, player, cardnumber, cardpassword)
				if rows > 0 then
					if res["card_pin"] == cardpassword then
						triggerClientEvent(player, "logging:lsBank", player)
					else
						outputChatBox("#575757Melancia:#ffffff Yanlış kart şifresi girdiniz. ( Eğer ilk defa aldıysanız şifre 0000'dır. )",player,255,0,0,true)
					end
				else
					outputChatBox("#575757Melancia:#ffffff Kart numarası bulunamadı.",player,255,0,0,true)
				end
			end
		)
	end
...
function powerOn(phone, state)
	...
	return triggerClientEvent(source, "phone:powerOn:response", source, dbExec(mysql:getDBConnection(), "UPDATE `phones` SET `turnedon`='"..state.."' WHERE `phonenumber`='"..(tostring(phone)).."'"), state)
end
...
function applyPhone(string, popOutOnPhoneCall)
	...
	if string == "phone_in" then
		...
		if getElementData(source, "phone_anim") ~= "0" then
			...
			exports.mrp_bone_attach:attachElementToBone(phoneO[source], source, 12, -0.05, 0.02, 0.02, 20, -90, -10)
		else
		...
		if getElementData(source, "cellphoneGUIStateSynced") ~= 1 then
			exports.mrp_anticheat:changeProtectedElementDataEx(source, "cellphoneGUIStateSynced", 1 , true)
		end
		exports.mrp_anticheat:changeProtectedElementDataEx(source, "cellphoneGUIStateSynced", 1 , true)
	elseif string == "phone_talk" then
		if getElementData(source, "phone_anim") ~= "0" then
			...
			setElementDimension(phoneO[source], getElementDimension(source))
			setElementInterior(phoneO[source], getElementInterior(source))
			exports.mrp_bone_attach:attachElementToBone(phoneO[element], source, 12, -0.03, 0.02, 0.02, 20, -90, -10)
	...
	elseif string == "phone_out" then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(source, "cellphoneGUIStateSynced", nil , true)
		if isElement(phoneO[source]) then
...
function destroyPhone(element)
	if canPlayerCall(element) then
		exports.mrp_global:removeAnimation(element) 
	end
	if isElement(phoneO[element]) then
		exports.mrp_bone_attach:detachElementFromBone(phoneO[element])
	...
	for _, phoneNumber in ipairs(phoneNumbers) do
		...
		if found and foundElement and isElement(foundElement) then
			...
			if (phoneState==0) then
				exports.mrp_anticheat:changeProtectedElementDataEx(foundElement, "calling", nil, false)
				exports.mrp_anticheat:changeProtectedElementDataEx(foundElement, "called", nil, false)
				exports.mrp_anticheat:changeProtectedElementDataEx(foundElement, "call.col", nil, false)
			end
		end
	end
end
...
function answerPhone(thePlayer, commandName)
	...
	if (logged==1) then
		if (exports.mrp_global:hasItem(thePlayer, 2)) then
			...
			elseif (calling) then
				...
				if (phoneState==0) then
					...
					if not found then
						...
						exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, "phonestate", 1, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(foundElement, "phonestate", 1, false)
						exports.mrp_anticheat:changeProtectedElementDataEx(foundElement, "called", nil, false)
						...
						exports['mrp_logs']:dbLog(thePlayer, 29, { thePlayer, "ph"..tostring(ownPhoneNo), foundElement, "ph"..tostring(calling) }, "**Picked up phone**") 
					end
...
function searchForPhone(phoneNumber)
	...
	if phoneNumber then
		for key, value in ipairs(exports.mrp_pool:getPoolElementsByType("player")) do
			...
			if (logged==1) then
				local foundPhone,_,foundPhoneNumber = exports.mrp_global:hasItem(value, 2, tonumber(phoneNumber))
				if foundPhone then
					return true, value
				end
			end
...
function fetchFirstPhoneNumber(target)
	local foundPhone,_,foundPhoneNumber = exports.mrp_global:hasItem(target, 2, tonumber(phoneNumber))
	return foundPhoneNumber
end

function setEDX(thePlayer, index, newvalue, sync, nosyncatall)
	return exports.mrp_anticheat:changeProtectedElementDataEx(thePlayer, index, newvalue, sync, nosyncatall)
end
...
function cleanUpOnePlayer(player)
	...
	exports.mrp_anticheat:changeProtectedElementDataEx(player, "cellphoneGUIStateSynced", nil, true)
end
...
function findToNumber(player,number)
	...
	for k,v in ipairs(getElementsByType("player")) do
		if getElementData(v,"loggedin") == 1 then
			if exports["mrp_global"]:hasItem(v,2,number) then
				found = found + 1
				if found == 1 then
					outputChatBox("#575757Melancia:#ffffff "..getPlayerName(player):gsub("_", " ").." adlı kullanıcı sana konum yolladı!",v,0,255,0,true)
					outputChatBox("#575757Melancia:#ffffff SMS ile konum gönderildi, gitmek istediğin alan blip ile işaretlenecek.",v,0,255,0,true)
					return v
				else
					outputChatBox("#575757Melancia:#ffffff Konum atmak istediğiniz oyuncu oyunda olmadığı için yalnızca SMS gönderildi!",player,255,0,0,true)
					return false
				end
			end
		end
...
function SAN_newAdvert(message, sourceName, sourceElement, theCost, outboundPhoneNumber)
	...
	exports.mrp_global:sendMessageToSupporters("[ADVERT] ID#"..advertID .. " ("..advertMsg..") created by "..sourceName..".")
	...
	for adIndex, adTable in ipairs (ads) do
		...
		elseif adTable[ AD_FROZEN ] then
			...
			if (currentTime - adTable [ AD_EDITTIME ]) > 5 then
				table.insert(removeIDs, adIndex)
				exports.mrp_global:sendMessageToSupporters("[ADS] Removing ad "..adIndex.." due being 10 minutes frozen. (`"..adTable[AD_MESSAGE].."`, `"..adTable[AD_SENDERNAME].."`)")
			end
		elseif ( not didAirSomething ) and not (adTable[AD_EDITTIME] == adTable[AD_CREATIONTIME] ) then
			...
			exports.mrp_logs:logMessage("ADVERT: " .. adTable[AD_MESSAGE] .. " ((".. adTable[AD_SENDERNAME].." ))", 2)
			for key, value in ipairs(exports.mrp_pool:getPoolElementsByType("player")) do
				if getElementData(value, "loggedin")==1 then
					local hasTogAd, togAdState = exports.mrp_donators:hasPlayerPerk(value, 2)
					if hasTogAd and tonumber(togAdState) == 1 then
						--donothing
					else
						if exports.mrp_integration:isPlayerSupporter(value) or exports.mrp_integration:isPlayerTrialAdmin(value) then
...
function SAN_freezeAdvert(thePlayer, commandHandler, advertID)
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer) then
		...
		exports.mrp_global:sendMessageToSupporters("[AD] "..getPlayerName(thePlayer).." froze advert "..advertID .. ".")
	end
end
...
function SAN_unfreezeAdvert(thePlayer, commandHandler, advertID)
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer) then
		...
		ads[advertID][AD_EDITTIME] = getRealTime().timestamp
		exports.mrp_global:sendMessageToSupporters("[AD] "..getPlayerName(thePlayer).." unfroze advert "..advertID .. ".")
...
function SAN_deleteAdvert(thePlayer, commandHandler, advertID)
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) or exports.mrp_integration:isPlayerSupporter(thePlayer) then
		...
		exports.mrp_global:sendMessageToSupporters("[AD] "..getPlayerName(thePlayer).." marked advert "..advertID .. " as aired.")
	end
end
...
function requestContacts(fromNumber)
    ...
    {source}, mysql:getDBConnection(), "SELECT * from `phone_contacts` WHERE `phone`='".. ( fromNumber ) .."' ORDER BY `entryName`")
end
...
function forceUpdateContactList(player, fromPhone)
    ...
    {source}, mysql:getDBConnection(), "SELECT * from `phone_contacts` WHERE `phone`='".. ( fromPhone ) .."' ORDER BY `entryName`")
end
...
function deletePhoneContact(name, number, phoneBookPhone)
	if (client) then
		...
		if not exports.mrp_global:hasItem(client,2, tonumber(phoneBookPhone)) then
			return
		end
		if name and number then
			if tonumber(number) then
				local result = dbExec(mysql:getDBConnection(), "DELETE FROM `phone_contacts` WHERE `phone`='" ..  (phoneBookPhone).."' AND `entryName`='".. (name) .."' AND `entryNumber`='".. (number) .."'")
				if result then
...
function saveCurrentRingtone(itemValue, phoneBookPhone)
	if client and itemValue then
		...
		local result = dbExec(mysql:getDBConnection(), "UPDATE `phones` SET `ringtone`='" ..  (itemValue).."' WHERE `phonenumber`='"..(phoneBookPhone).."'")
		if not result then
			outputChatBox("Error, please try it again.", client, 255,0,0)
			return
		end
            ...
            function(qh)
                ...
                if rows > 0 then
                    for index, row in ipairs(res) do
                        ...
                        exports.mrp_pool:allocateElement(shape)
                        setElementDimension(shape, dimension)
                        exports.mrp_anticheat:changeProtectedElementDataEx(shape, "dbid", id, false)
                    end
                end
            end,
        mysql:getDBConnection(), "SELECT id, x, y, z, dimension FROM publicphones")
	end
)
...
function addPhone(thePlayer, commandName)
	if (exports.mrp_integration:isPlayerTrialAdmin(thePlayer)) then
		...
		local query = dbExec(mysql:getDBConnection(), "INSERT INTO publicphones SET x="  .. (x) .. ", y=" .. (y) .. ", z=" .. (z) .. ", dimension=" .. (dimension))
		...
		if (query) then
            ...
            mysql:getDBConnection(), "SELECT id FROM publicphones WHERE id=LAST_INSERT_ID()")
	...
	if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) then
		...
		if not id then
			...
			if (colShape) then
				...
				local result = dbExec(mysql:getDBConnection(), "DELETE FROM publicphones WHERE id=" .. (id))
...
function writeCellphoneLog(thePlayer, theOtherPlayer, type, message, starting )
	if starting or (message and string.len(message) > 0) and type then
		...
		if tonumber(charId1) and tonumber(number1) and tonumber(number2Test) then
			...
			local message1 = ""
			if starting then
				message1 = "\nOUTGOING "..(type == "Calls" and "CALL" or "SMS").." FROM #"..number1.." ("..string.upper(exports.mrp_global:getPlayerName(thePlayer))..") TO #"..number2Test.." ("..(theOtherPlayer and string.upper(exports.mrp_global:getPlayerName(theOtherPlayer)) or string.upper(isNumberAHotline(number2Test)))..") AT ".. getCurrentDateTimeText()..":"
			else
				message1 = "-> "..message
			end
			...
			if tonumber(charId2) and tonumber(number2) and tonumber(number1Test) and tonumber(number1Test) == tonumber(number1) and tonumber(number2Test) == tonumber(number2) then
				...
				local message2 = ""
				if starting then
					message2 = "\nINCOMING "..(type == "Calls" and "CALL" or "SMS").." FROM #"..number1.." ("..string.upper(exports.mrp_global:getPlayerName(thePlayer))..") TO #"..number2Test.." ("..string.upper(exports.mrp_global:getPlayerName(theOtherPlayer))..") AT ".. getCurrentDateTimeText()..":"
				else
					message2 = "<- "..message
				end
