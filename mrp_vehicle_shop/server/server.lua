...
local global = exports.mrp_global
	...
	thread:foreach(shops, function(v)
		...
		if vehicleData then
			...
			if not (vehicle ) then
				...
				setVehicleVariant(vehicle, exports['mrp_vehicle']:getRandomVariant(getElementModel(vehicle)))
				v["vehicle"] = vehicle
...
function carshop_blockEnterVehicle(thePlayer)
	...
	if (isCarShop) then
		...
		if not exports.mrp_global:hasMoney(thePlayer, costCar) or costCar == 0 then
			payByCash = false
		end
...
function carshop_buyVehicle(paymentMethod)
	...
	if not exports.mrp_global:canPlayerBuyVehicle(client) then
		if isOverlayDisabled then 
			outputChatBox("Azami araç sayısına zaten ulaştınız.", client, 0, 255, 0)
		else
			exports.mrp_hud:sendBottomNotification(client, "Maksimum araç limiti", "Azami araç sayısına zaten ulaştınız. Ayrıntılar için /stats")
		end
		return false
	end
	...
	if (paymentMethod == "cash") then
		if not exports.mrp_global:hasMoney(client, costCar) or costCar == 0 then
			if isOverlayDisabled then
				outputChatBox("Bu arkadaş için elinizde yeterli para yok..", client, 0, 255, 0)
			else
				exports.mrp_hud:sendBottomNotification(client, "Para her zaman bir problemdir..", "Bu arkadaş için elinizde yeterli para yok..")
			end
			return false
		else
			exports.mrp_global:takeMoney(client, costCar)
		end
	elseif (paymentMethod == "bank") then
		...
		if money < 0 or costCar == 0 then
			if isOverlayDisabled then 
				...
				exports.mrp_hud:sendBottomNotification(client, "Para her zaman bir problemdir..", "Banka hesabınızda bu arkadaş için yeterli para yok...")
			end
		else
	...
	elseif (paymentMethod == "token") then
		if exports.mrp_global:hasItem(client, 263, 1) then
			exports.mrp_global:takeItem(client, 263, 1)
		else
			return false
		end
	else
	...
	call( getResourceFromName( "mrp_items" ), "deleteAll", 3, smallestID )
	exports.mrp_global:giveItem( client, 3, smallestID )		

	destroyElement(source)					
	exports['mrp_vehicle']:reloadVehicle(smallestID)
...
addEvent('carshop:infoCar', true)
addEventHandler('carshop:infoCar', root, function(vehicle)
	if vehicle then
		...
		outputChatBox('[Melancia]#D0D0D0 '..getElementData(vehicle, "brand")..' '..getElementData(vehicle, "model")..' '..getElementData(vehicle, "year")..'', source, 195,184,116,true)
		outputChatBox('[Melancia]#D0D0D0 Araç Fiyatı: '..exports.mrp_global:formatMoney(getElementData(vehicle,"carshop:cost"))..'₺', source, 195,184,116,true)
		outputChatBox('[Melancia]#D0D0D0 Araç Vergisi: '..vergiMiktari..'₺', source, 195,184,116,true)
	else
		outputChatBox('[Melancia]#D0D0D0 Bir hata oluştu lütfen tekrar deneyiniz.', source, 195,184,116,true)
	end
end)