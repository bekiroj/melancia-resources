...
--- GTA:MTA Lua thread thread scheduler.
-- @author Inlife
-- @license MIT
-- @url https://github.com/Inlife/mta-lua-thread
...
class "_thread" {
    ...
    -- @access public
    -- @usage local threadmanager = thread();
    ...
    -- @access private
    -- @param int id Thread id (in table thread.threads)
    ...
    -- @usage thread:setPriority("normal");
    -- @usage thread:setPriority(50, 200);
    ...
    -- Usage:
        -- @usage thread:iterate(1, 10000, function(i)
        --     print(i);
        -- end);
    iterate = function(self, from, to, func, callback)
        self:add(function()
            ...
            if (self.debug) then
                outputDebugString("! thread iterate: " .. (getTickCount() - a) .. "ms", 0, 87,87,87);
            end
            if (callback) then
                callback();
            end
    ...
    -- @param function [callback] Callback function, called when execution finished
    -- Usage:
        -- @usage thread:query("SELECT * FROM vehicles", function(res, rows, err)
        --     print(table[1].id);
        -- end);
    query = function(self, array, func)
        self:add(function()
            local a = getTickCount();
            local lastresume = getTickCount();
            
            local mysql = exports.mrp_mysql:getConnection()
            ...
            array)
            if (self.debug) then
                outputDebugString("! thread query: " .. (getTickCount() - a) .. "ms", 0, 87,87,87);
            end
            if (callback) then
                callback();
            end
    ...
    -- Iterate over array (prepare cycle)
    -- @access server
    -- @param table array Input array
    -- @param function func Iterate using func
        -- Function func params:
    ...
    -- @param function [callback] Callback function, called when execution finished
    -- Usage:
        -- @usage thread:prepare("UPDATE vehicles SET model_id = 430 WHERE id = 1", "vehicles", function(insert_id)
        --     print(table[1].id);
        -- end);
    prepare = function(self, array, table_name, func)
        self:add(function()
            local a = getTickCount();
            local lastresume = getTickCount();
            ...
            if (mysql:exec(array)) then
                mysql:query(
                    function(qh)
                        local res, rows, err = qh:poll(0);
                        if rows > 0 then
                            func(res[1]["id"], rows, err);
                        else
                            func(false);
                        end
                    end,
                "SELECT id FROM "..table_name.." WHERE id = LAST_INSERT_ID()");
            end
            if (self.debug) then
                outputDebugString("! thread query prepare: " .. (getTickCount() - a) .. "ms", 0, 87,87,87);
            end
            if (callback) then
                callback();
            end
    ...
    -- @param function [callback] Callback function, called when execution finished
    -- Usage:
        -- @usage thread:foreach(vehicles, function(vehicle, id)
        --     print(vehicle.title);
        -- end);
    foreach = function(self, array, func, callback)
        self:add(function()
            ...
            if (self.debug) then
                outputDebugString("! thread foreach: " .. (getTickCount() - a) .. "ms", 0, 87,87,87);
            end
            if (callback) then
                callback();
            end
...
function thread:setDebug(...)
    getInstance():setDebug(...);
end

function thread:setPriority(...)
    getInstance():setPriority(...);
end

function thread:iterate(...)
    getInstance():iterate(...);
end

function thread:foreach(...)
    getInstance():foreach(...);
end
