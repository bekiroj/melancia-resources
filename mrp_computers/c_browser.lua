...
local function getUrlInfo(url)
    ...
    local segments = exports.mrp_global:split(url:gsub("http://", ""):gsub("https://", ""), "/")
...
function openBrowser(home_url)
    ...
    local bHomeButton = guiCreateButton(5, 25, 45, 25, "Ana Sayfa", false, wBrowser)
    ...
    addEventHandler("onClientGUIClick", bCloseButton,
        function(button, state)
            if button == "left" and state == "up" then
                 closeBrowser()
                 if exports.mrp_phone:isPhoneGUICreated() then
                    setElementData(localPlayer, "exclusiveGUI", true)
                    exports.mrp_phone:drawPhoneHome()
                 end 
            end
        end, false)
