...
function openEmailWindow()
    ...
    if not (wEmail) then
        ...
        -- Username
        username_label = guiCreateLabel(13,54,63,16,"Kullanıcı İsmi:",false,wEmail)
        username_input = guiCreateEdit(78,52,200,22,"J.Smith",false,wEmail)

        address_label = guiCreateLabel(13,88,63,16,"Domainler:",false,wEmail)
        ...
        local teamID = exports.mrp_factions:getCurrentFactionDuty(getLocalPlayer())
        if (teamID==1) then
        ...
        password_label = guiCreateLabel(13,154,63,16,"Şifre:",false,wEmail)
        ...
        -- Login
        email_login_button = guiCreateButton(67,184,70,26,"Giriş",false,wEmail)
        addEventHandler("onClientGUIClick",email_login_button,function()
            ...
            guiSetText(email_error_label,"Bir kullanıcı ismi seçin!")
            ...
            guiSetText(email_error_label,"Bir Domain seçmeniz gerekiyor!")
            elseif (tostring(guiGetText(password_input))=="") then
                guiSetText(email_error_label,"Şifre girin!")
            else
...
function name_error()
    guiSetText(email_error_label, "Bu kullanıcı ismi kullanılıyor!")
end
addEvent("name_in_use",true)
addEventHandler("name_in_use", getLocalPlayer(),name_error)

function login_error()
    guiSetText(email_error_label, "Yanlış kullanıcı ismi yada şifre!")
end
addEvent("loginError",true)
addEventHandler("loginError", getLocalPlayer(),login_error)

function not_leader()
    guiSetText(email_error_label, "Bu domain ile kayıt olamazsın!")
end
...
function show_inbox(inbox_table, accountName)
    ...
    inbox_grid_date = guiGridListAddColumn(inbox_grid,"Tarih",0.16)
    inbox_grid_sender = guiGridListAddColumn(inbox_grid,"Kimden",0.3)
    inbox_grid_subject = guiGridListAddColumn(inbox_grid,"Konu",0.5)
    ...
    if not(inbox_message_datetitle_label) then
        ...
        inbox_message_datetitle_label = guiCreateLabel(0.02,0.4,0.96,0.1,"Alınan Tarih:",true,inbox_tab)
        ...
        inbox_message_fromtitle_label = guiCreateLabel(0.5,0.4,0.96,0.1,"Kimden:",true,inbox_tab)
        ...
        inbox_message_subjecttitle_label = guiCreateLabel(0.02,0.45,0.96,0.1,"Konu:",true,inbox_tab)
    ...
    if not (check_mail_button) then
        check_mail_button = guiCreateButton(0.2,0.9,0.2,0.08,"Maili kontrol et",true,inbox_tab)
        ...
        inbox_delete_mail_button = guiCreateButton(0.4,0.9,0.2,0.08,"Sil",true,inbox_tab)
        ...
        --REPLY BUTTON
        inbox_reply_button = guiCreateButton(0.6,0.9,0.2,0.08,"Yanıtla",true,inbox_tab)
        addEventHandler("onClientGUIClick",inbox_reply_button,function()
            local msg = inbox_table[activeMail]
            if msg and msg[2] ~= "" then
                guiSetText(new_message_to_input, msg[3])

                if msg[4]:find( "Cevap: " ) ~= 1 then
                    guiSetText(new_message_subject_input, "Cevap: " .. msg[4])
                else
...
function show_outbox(outbox_table,accountName)
    ...
    outbox_grid_date = guiGridListAddColumn(outbox_grid,"Tarih",0.16)
    outbox_grid_sender = guiGridListAddColumn(outbox_grid,"Kimden",0.3)
    outbox_grid_subject = guiGridListAddColumn(outbox_grid,"Konu",0.5)
    ...
    if not(outbox_message_datetitle_label) then
        -- Static labels showing date and sender
        outbox_message_datetitle_label = guiCreateLabel(0.02,0.4,0.96,0.1,"Tarih alındı:",true,outbox_tab)
        ...
        outbox_message_totitle_label = guiCreateLabel(0.5,0.4,0.96,0.1,"Kime:",true,outbox_tab)
        ...
        outbox_message_subjecttitle_label = guiCreateLabel(0.02,0.45,0.96,0.1,"Konu:",true,outbox_tab)
    ...
    if not (outbox_delete_mail_button) then
        ...
        outbox_delete_mail_button = guiCreateButton(0.4,0.9,0.2,0.08,"Sil",true,outbox_tab)
...
function invalid_address()
    guiSetText(new_message_to_input, "Adres bulunamadı!")
end
