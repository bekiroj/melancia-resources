	...
	if (hasLanguage) then
		...
		if currSkill < 100 then
			...
			if chance == 1 then
				...
				exports.mrp_anticheat:changeProtectedElementDataEx(player, "languages.lang" .. slot .. "skill", currSkill+1, false)
				dbExec(mysql:getConnection(),"UPDATE characters SET lang" .. (slot) .. "skill = " .. (currSkill + 1) .. " WHERE id = " .. (getElementData( player, "dbid" ))) 
			end
		end
	end
end
...
function removeLanguage(player, language)
	...
	if (hasLanguage) then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(player, "languages.lang" .. slot, 0, false)
		exports.mrp_anticheat:changeProtectedElementDataEx(player, "languages.lang" .. slot .. "skill", 0, false)
		dbExec(mysql:getConnection(),"UPDATE characters SET lang" .. (slot) .. " = 0, lang" .. (slot) .. "skill = 0 WHERE id = " .. (getElementData( player, "dbid" )) )
		return true
	else
		return false
	end
...
function learnLanguage(player, lang, showmessages, skill)
	...
	if (hasLanguage) then
		...
		if skill then
			exports.mrp_anticheat:changeProtectedElementDataEx(player, "languages.lang" .. slot .. "skill", skill, false)
			dbExec(mysql:getConnection(),"UPDATE characters SET lang" .. (slot) .. " = " .. (lang) .. ", lang" .. (slot) .. "skill = '" .. (( skill or 0 )) .. "' WHERE id = " .. (getElementData( player, "dbid" )) )
			return true
		end
		...
		if (freeslot==0) then
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(player, "languages.lang" .. freeslot, lang, false)
			exports.mrp_anticheat:changeProtectedElementDataEx(player, "languages.lang" .. freeslot .. "skill", skill or 0, false)
			dbExec(mysql:getConnection(),"UPDATE characters SET lang" .. (freeslot) .. " = " .. (lang) .. ", lang" .. (freeslot) .. "skill = '" .. (( skill or 0 )) .. "' WHERE id = " .. (getElementData( player, "dbid" )) )
...
function useLanguage(lang)
	...
	if (hasLanguage) then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(source, "languages.current", slot, false)
		dbExec(mysql:getConnection(),"UPDATE characters SET currLang = " .. (slot) .. " WHERE id = " .. (getElementData( source, "dbid" )) )
...
function unlearnLanguage(lang)
	...
	if (hasLanguage) then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(source, "languages.current", nextSlot, false)
		dbExec(mysql:getConnection(), "UPDATE characters SET currLang = " .. (nextSlot) .. " WHERE id = " .. (getElementData( source, "dbid" )) )
		showLanguages(source)
	end
end
...
for key, value in pairs( languages ) do
	function speakLang( thePlayer, commandName, ... )
		if doesPlayerHaveLanguage( thePlayer, key ) then
			call( getResourceFromName( "mrp_chat" ), "localIC", thePlayer, table.concat({...}, " "), key )
		else
