...
function reloadPed(id)
    ...
    local thePed = exports.mrp_pool:getElement("ped", tonumber(id))
        ...
        function(qh)
            ...
            if rows > 0 then
                for index, row in ipairs(res) do
                    ...
                    if ped then
                        exports.mrp_anticheat:changeProtectedElementDataEx(ped, "dbid", row.id)
                        exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.dbid", row.id)
                        exports.mrp_pool:allocateElement(ped, row.id, true)
                        ...
                        if row.type then
                            exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.type", row.type)
                        end
                        exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.name", pedname)
                        exports.mrp_anticheat:changeProtectedElementDataEx(ped, "ped:name", pedname) -- For chat system
                        exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.gender", gender)
                        exports.mrp_anticheat:changeProtectedElementDataEx(ped, "name", pedname) --for owl
                        if row.nametag then
                            exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.nametag", true)
                            exports.mrp_anticheat:changeProtectedElementDataEx(ped, "nametag", true) --for owl
                        end
                        exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.behav", row.behaviour)
                        if row.money then --if ped have money, set the amount
                            exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.money", row.money)
                            ...
                            if(row.owner_type ~= 0 and row.owner ~= 0) then
                                exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.owner_type", row.owner_type)
                                exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.owner", row.owner)
                            end
                        end
                        ...
                        if row.animation then
                            exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.animation", row.animation)
                            --outputDebugString(tostring(row.animation))
                            local animTable = exports.mrp_global:split(row.animation, ";")
                        ...
                        local badges = {}
                        badges = exports['mrp_items']:getBadges()
                        --outputDebugString(tostring(#badges).." badges")
                        for k,v in pairs(badges) do
                            local hasItem = exports['mrp_items']:hasItem(ped, k)
                            if hasItem then
                                local itemresult = exports['mrp_items']:npcUseItem(ped, k)
                        ...
                        exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.createdBy", row.created_by)
                        exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.createdByUsername", exports.mrp_global:getUserNameFromID(row.created_by))
                        exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.createdAt", row.created_at)
                        exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.comment", row.comment)
                        exports.mrp_anticheat:changeProtectedElementDataEx(ped, "rpp.npc.synced", tonumber(row.synced) == 1 or false)
...
function healByHospitalPed(ped)
    ...
    if(needed < 5) then
        exports.mrp_RPPchat:chatPed(ped, "say", "You seem to be fine, sir. No treatment needed.")
    else
        ...
        if(getPlayerMoney(source) > cost or getPlayerMoney(source) == cost) then
            ...
            exports.mrp_RPPchat:chatMe(source, false, "takes out a wallet and hands a few dollar notes to "..pedname)
            exports.mrp_RPPchat:chatPed(ped, "me", "takes the money")
            takePlayerMoney(source, tonumber(cost))
            exports.mrp_RPPchat:chatPed(ped, "say", "Thank you sir. I'll get you fixed up.")
            exports.mrp_RPPchat:chatPed(ped, "me", "heals "..playername)
            setElementHealth(source, newHealth)
        else
            exports.mrp_RPPchat:chatMe(source, false, "takes out a wallet and hands a few dollar notes to "..pedname)
            exports.mrp_RPPchat:chatPed(ped, "say", "Sorry sir. I don't think that is enough.")
        end
    end
end
...
function makeTempPed(thePlayer, command, behav, gender, skin, name) --create temporary ped (no dbid)
    if exports.mrp_integration:isPlayerTrialAdmin(thePlayer) then
        ...
        if name then
            local splitname = exports.mrp_global:split(name, "_")
            if(#splitname >= 2) then
...
function pedMoveToTram(ped, tram)
    ...
    --else
        ...
        pedMoveTimer[ped] = nil
        setPedAnimation(ped)
        exports.mrp_trams:npcEnterTramPassenger(ped, tram)
    --end
end
        ...
        if(getElementType(hitElement) == "vehicle") then
            if(getElementModel(hitElement) == 449) then
                ...
                if(getVehicleController(hitElement)) then
                    if(getResourceState(getResourceFromName("mrp_trams")) == "running") then
...
function respawnPed(player, ped)
    if(getElementType(ped) == "ped") then
        ...
        elseif getElementData(ped, "rpp.npc.spawnpos2") then
            ...
            local pos = exports.mrp_global:split(posstring, ",")
        ...
        for k, v in next, getAllElementData(ped) do
            exports.mrp_anticheat:changeProtectedElementDataEx(new, k, v)
        end
        exports.mrp_anticheat:changeProtectedElementDataEx(new, "activeConvo", 0)
...
function healPed(player, ped)
    if(getElementType(ped) == "ped") then
        ...
        if not (factionType==4) then
            ...
            local foundkit, slot, itemValue = exports.mrp_global:hasItem(player, 70)
            if foundkit then
                if isPedDead(ped) then
                    ...
                    if itemValue > 1 then
                        exports['mrp_items']:updateItemValue(player, slot, itemValue - 1)
                    else
                        exports.mrp_global:takeItem(thePlayer, 70, itemValue)
                        if not exports.mrp_global:hasItem(thePlayer, 70) then
                    ...
                    local price = 100                                   
                    exports.mrp_global:giveMoney( getTeamFromName("San Fierro Emergency Services"), math.ceil((1-tax)*price) )
                    exports.mrp_global:giveMoney( getTeamFromName("Government of San Fierro"), math.ceil(tax*price) )
                    if pedName then
                        outputChatBox("You healed " ..pedName.. " (NPC).", player, 0, 255, 0)
                    else
                        outputChatBox("You healed a NPC.", player, 0, 255, 0)
                    end
                    exports.mrp_logs:dbLog(player, 35, ped, "HEAL NPC FOR $" .. price)
                end
            else
...
function deletePed(player, ped)
    if(getElementType(ped) == "ped") then
        if exports.mrp_integration:isPlayerAdmin(player) then
            ...
            if dbid > 0 then
                ...
                destroyElement(ped)
                exports.mrp_global:sendMessageToAdmins("[Peds] Temp NPC '"..tostring(pedName).."' was deleted by " .. getPlayerName(player) .. ".")
            end
        end
    end
end
...
function deleteDbPed(player, pedID, ped)
    ...
    if exports.mrp_integration:isPlayerTrialAdmin(player) then
        if not ped then
            ped = exports.mrp_global:getPoolElement("ped", pedID)
        end
        if not ped then
        ...
        if pedType then
            if(not allowedFunctionPeds[pedFunction]) then
                if not exports.mrp_integration:isPlayerAdmin(player) then
...
function hideMyID(thePlayer, command)
    if(exports.mrp_integration:isPlayerScripter(thePlayer)) then
