...
function showsize(thePlayer)
	if not exports.mrp_integration:isPlayerScripter(thePlayer) then
		return false
	end
...
addEventHandler("onResourceStop", resourceRoot, function () --Maxime
	exports.mrp_data:save(poolTable, "poolTable")
	exports.mrp_data:save(indexedPools, "indexedPools")
end)

addEventHandler("onResourceStart", resourceRoot, function() --Maxime
	local loaded = exports.mrp_data:load("poolTable")
	if loaded then
		poolTable = loaded
	end
	loaded = exports.mrp_data:load("indexedPools")
	if loaded then
		indexedPools = loaded
	end
