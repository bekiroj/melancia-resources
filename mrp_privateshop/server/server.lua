...
function isimDegistirOnayla(isim, fiyat)
	...
	exports.mrp_global:sendMessageToAdmins("[MARKET] '" .. getPlayerName(source) .. "' isimli oyuncu ismini '" .. isim .. "' olarak değiştirildi.")
...
function changeName(targetPlayer,newName)
	exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, "legitnamechange", 1, false)
	...
	if (name) then
		...
		exports.mrp_anticheat:changeProtectedElementDataEx(targetPlayer, "legitnamechange", 0, false)
		exports.mrp_logs:dbLog(thePlayer, 4, targetPlayer, "MARKET ISIM DEGISIKLIGI "..targetPlayerName.." -> "..tostring(newName))
	else
...
function kisimDegistirOnayla(isim, fiyat)
	...
	outputChatBox("(( " ..getPlayerName(source):gsub("_", " ").. " sunucudan yasaklandı. Sure: Sınırsız Gerekce: Kullanıcı Adı Değişikliği))", arrarPlayer, 255, 0, 0)
	exports.mrp_global:sendMessageToAdmins("[MARKET] '" .. getElementData(source, "account:username") .. "' isimli oyuncu ismini '" .. isim .. "' olarak değiştirildi.")
...
function vipVerdirtme(vipSeviye, vipGun, vipFiyat)
	exports["mrp_vipsystem"]:addVIP(source, vipSeviye, vipGun)
	...
	exports.mrp_global:sendMessageToAdmins("[MARKET] '" .. getPlayerName(source) .. "' isimli oyuncu "..vipGun.." günlük VIP [" .. vipSeviye .. "] aldı.")
end
...
function karakterSlotArttirmaLOG(fiyat)
	exports.mrp_global:sendMessageToAdmins("[MARKET] '" .. getPlayerName(source) .. "' isimli oyuncu +1 karakter slotu arttırma aldı.")
...
function aracSlotArttirmaLOG(player, fiyat)
	...
	exports.mrp_global:sendMessageToAdmins("[MARKET] '" .. getPlayerName(player) .. "' isimli oyuncu +1 araç slotu arttırma aldı.")
end
...
function evSlotArttirmaLOG(player, fiyat)
	...
	exports.mrp_global:sendMessageToAdmins("[MARKET] '" .. getPlayerName(player) .. "' isimli oyuncu +1 ev slotu arttırma aldı.")
end
...
function privateCarCreate(vehid, brandid, rgb, balance, vehname, plateState, plateEditText)
	...
	if tonumber(vehid) and tonumber(brandid) then
		...
		exports.mrp_global:sendMessageToAdmins("[MARKET] '" .. getPlayerName(player) .. "' isimli oyuncu "..vehname.." marka araç satın aldı.")
		...
		local var1, var2 = exports['mrp_vehicle']:getRandomVariant(vehid)
		local letter1 = string.char(math.random(65,90))
		local letter2 = string.char(math.random(65,90))
		if plateEditText == "" then
			plate = tonumber(34).. " " .. letter1 .. letter2 .. " " .. math.random(1000, 9999)
		else
			plate = plateEditText
		end
				...
				function(qh)
					...
					if rows > 0 then
						...
						if not insertid then
							setElementData(player, "bakiye", playerBalance + balance)
							exports.mrp_global:sendMessageToAdmins("[MARKET] '" .. getPlayerName(player) .. "' isimli oyuncu "..vehname.." marka aldığı araç işlenemediği için parası iade edildi.")
							outputChatBox("Aldığın araç veritabanına işlenemediği için paran iade edildi!", player, 255,0,0)
							return false
						end
						call( getResourceFromName( "mrp_items" ), "deleteAll", 3, insertid )
						exports.mrp_global:giveItem( player, 3, insertid )
						--setElementData(veh, "dbid", insertid)
						destroyElement(veh)
						exports['mrp_vehicle']:reloadVehicle(insertid)
					end
				end,
...
addEventHandler("market->silahHakEkle", root,
	function(player, itemIndex, price)
		...
		local itemSlot = exports['mrp_items']:getItems(player)
		for i, v in ipairs(itemSlot) do
			if v[3] == itemIndex then
				...
				exports.mrp_global:takeItem(player, 115, v[2])
				eren = exports.mrp_global:explode(":",v[2])[1]..":"..exports.mrp_global:explode(":",v[2])[2]..":"..exports.mrp_global:explode(":",v[2])[3].."::"..yeniHak
				exports.mrp_global:giveItem(player, 115, eren)
			
				outputChatBox("[!] #ffffff"..exports.mrp_global:explode(":", v[2])[3].." silahının hakkı arttırıldı. Eski Hak: "..eskiHak.." - Yeni Hak: "..yeniHak, player, 0, 55, 255, true)
...
local mySerial = exports.mrp_global:createWeaponSerial( 1, getElementData(source, "dbid"), getElementData(source, "dbid"))
	if  exports.mrp_global:giveItem(source, 115, weaponid..":"..mySerial..":"..getWeaponNameFromID(weaponid).."::") then
...
function vehTintPrivate(vehID, price)
	local vehicle = exports.mrp_pool:getElement("vehicle", vehID)
	if vehicle and isElement(vehicle) then
		if getElementData(vehicle, "owner") == getElementData(source, "dbid") then
			...
			exports.mrp_anticheat:changeProtectedElementDataEx(vehicle, "tinted", true, true)
			...
			dbQuery(
				function(qh)
					...
					if rows > 0 then
						...
						setTimer(function() exports["mrp_vehicle_manager"]:loadCustomVehProperties(tonumber(vehID), vehicle) end, 5000, 1)
...
function plakaDegistirSERVER(oyuncu, plateText, aracID, fiyat)
	...
	local theVehicle = exports.mrp_pool:getElement("vehicle", aracID)
	if theVehicle then
		...
		dbQuery(
			function(qh)
				...
				if rows > 0 then
					if res[1].no == 0 then
						if (exports.mrp_vehicle_plate:checkPlate(plateText)) and getVehiclePlateText(theVehicle) ~= plateText then
							...
							exports.mrp_anticheat:changeProtectedElementDataEx(theVehicle, "plate", plateText)
...
addEventHandler("market->addVehicleNeon", root,
	function(player, vehicleID, neonIndex)
		...
		local vehicle = exports.mrp_pool:getElement("vehicle", vehicleID)
		if vehicle and isElement(vehicle) then
			if getElementData(vehicle, "owner") == getElementData(source, "dbid") then
				...
				if not vehicleNeon then
					...
					if not exports.mrp_global:hasMoney(player, 1000) then
					...
					exports.mrp_global:takeMoney(player, 1000)
				end
...
addEventHandler("market->setVehicleTexture", root,
	function(player, vehicle, link, texture_name, price)
		local vehicle = exports.mrp_pool:getElement("vehicle", vehicle)
...
addEventHandler("market->changeFactionSettings", root,
	function(player, factionName, factionType)
		...
		if factionElement then
			exports.mrp_global:sendMessageToAdmins("[MARKET] '" .. getPlayerName(player) .. "' isimli oyuncu '" .. getElementData(factionElement, "name") .. "' adlı birliğin adını "..factionName.." olarak değiştirdi.")
