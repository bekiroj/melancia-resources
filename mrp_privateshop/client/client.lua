...
aracListesi = {
	...
	{"Audi A7", 125, 529, 1076, 260, 35},
	{"Range Rover", 75, 490, 1078, 250, 25},
	{"Mercedes Amg", 50, 602, 1079, 230, 25},
	{"BMW M4", 30, 412, 1080, 225, 20},
	{"Masserati", 30, 402, 1081, 215, 15},
	{"Passat", 60, 405, 1034, 230, 25},
	{"Tofaş", 50 , 561, 1077, 220 , 15},
	{"Maverick", 175 , 487, 413 , 200 , 50},
}
...
function marketSistemiAC()
	if not exports.mrp_integration:isPlayerDeveloper(localPlayer) and maintance then outputChatBox("[!] #ffffffBurayı güncelliyoruz, sürprizleri bekle ;)", 255, 0, 0, true) return end
	...
	imageWindow = false
	normalWindow = guiCreateWindow(sx/2-600/2,sy/2-400/2,540,400, "Melancia - Market Sistemi", false)
	...
	for index, value in pairs(exports['mrp_animations']:getCustomAnimations()) do
	...
	oPrevElement = exports["mrp_object_preview"]:createObjectPreview(previewObject, 0, 0, 180, 0.46, 0.40, 0.26, 0.20, true, true)
	--rotation config
	...
	addEventHandler('onClientGUIScroll', scrollbar,
		function()
			...
			exports["mrp_object_preview"]:setRotation(oPrevElement,0, 0, 155 + rotation * 360)
		end, false)
	--color config
	--[[
	...
	for index, value in ipairs(exports.mrp_pet:getDogs()) do
	...
	--guiGridListAddColumn(gridlist42, "Hak Fiyatı", 0.20)
	for index, value in ipairs(exports['mrp_items']:getItems(localPlayer)) do
		 if value[1] == 115 and hakListesi[exports['mrp_items']:getItemModel(value[1], value[2])] then
			local row = guiGridListAddRow(gridlist42)
			guiGridListSetItemText(gridlist42, row, 1, exports['mrp_items']:getItemName(value[1], value[2]), false, false)
		--	guiGridListSetItemText(gridlist42, row, 2, value[3].." ₺", false, false)
			guiGridListSetItemData(gridlist42, row, 1, value[3], false, false)
			guiGridListSetItemText(gridlist42, row, 2, (hakListesi[exports['mrp_items']:getItemModel(value[1], value[2])] or 9999).." ₺", false, false)
			guiGridListSetItemData(gridlist42, row, 2, (hakListesi[exports['mrp_items']:getItemModel(value[1], value[2])] or 0), false, false)
			local silahHak = (#tostring(exports.mrp_global:explode(":", value[2])[5])>0 and exports.mrp_global:explode(":", value[2])[5]) or 3
			silahHak = not restrictedWeapons[tonumber(exports.mrp_global:explode(":", value[2])[1])] and silahHak or "-"
	...
	function()
		...
		if getElementData(localPlayer, "marketSistemiAcik") == 1 then
			...
			if isElement(bakiyeEdit) then
				...
				if tonumber(t) then
					guiSetText(bakiyeDonusecek,"Dönüştürülecek para: ₺"..exports.mrp_global:formatMoney(tonumber(t)*1000))
				else
...
function historySilmeAc()
	...
	exports.mrp_global:centerWindow(historyGUI)
...
function camFilmiAC()
	camFilmiGUI = guiCreateWindow(0, 0, 299, 116, "Melancia * Cam Filmi ( Araç ID Girin )", false)
	guiWindowSetSizable(camFilmiGUI, false)
	exports.mrp_global:centerWindow(camFilmiGUI)
...
function kelebekKapiAc()
	kelebekFilmiGUI = guiCreateWindow(0, 0, 299, 116, "Melancia * Kelebek Kapı ( Araç ID Girin )", false)
	guiWindowSetSizable(kelebekFilmiGUI, false)
	exports.mrp_global:centerWindow(kelebekFilmiGUI)
...
function plakaDegisikligiAC()
    plakaWINDOW = guiCreateWindow(0.38, 0.37, 0.24, 0.28, "Melancia * Araç Plaka Değişikliği", true)
...
function isimDegistirmePaneli()
	showCursor(true)
	guiSetInputEnabled(true)
    isimDegistirmeWINDOW = guiCreateWindow(0.35, 0.39, 0.30, 0.24, "Melancia * İsim Değiştirme Paneli", true)
...
function kisimDegistirmePaneli()
	showCursor(true)
	guiSetInputEnabled(true)
    isimDegistirmeWINDOW = guiCreateWindow(0.35, 0.39, 0.30, 0.24, "Melancia * Kullanıcı Adı Değiştirme Paneli", true)
    guiWindowSetSizable(isimDegistirmeWINDOW, false)
...
VIPNUMARA = vipNumara
	if getElementData(localPlayer, "vip") == vipNumara or getElementData(localPlayer, "vip") == 0 or not getElementData(localPlayer, "vip") then
		...
		vipSatinAlmaWINDOW = guiCreateWindow(0.36, 0.37, 0.29, 0.19, "Melancia * VIP ["..vipNumara.."] Satın Alma Paneli", true)
...
function vergiOnayiAc()
	...
	vergiWINDOW = guiCreateWindow(0.35, 0.39, 0.30, 0.24, "Melancia * Vergi Azaltıcı Onay Paneli", true)
...
addEventHandler("onClientGUIClick", guiRoot, function()
	...
	if source == onaylaButonu6 then
		...
		local anims = exports['mrp_animations']:getCustomAnimations()
		if anims[selectedID] then
			...
			if isElement(previewObject) then
				...
				if oPrevElement then
					exports["mrp_object_preview"]:destroyObjectPreview(oPrevElement)
				end
...
addEventHandler("vehicletexture->showList", root,
    function(vehID)
        ...
        vehicleTextureGUI = guiCreateWindow(0, 0, 571, 246, "Araç Kaplama Sistemi * Melancia", false)
        guiWindowSetSizable(vehicleTextureGUI, false)
		exports.mrp_global:centerWindow(vehicleTextureGUI)
		guiSetInputEnabled(true)
                        ...
                        if verifyImageURL(link) then
                            if find:vehicle(vehID) then
                                ...
                                if (tonumber(vehicle:getData("owner")) == tonumber(localPlayer:getData("dbid"))) then
									...
									exports['mrp_item_texture']:addTexture(tempVehicle, foundedTexture, link)
...
function birlikAdiDuzenle()
	...
	exports.mrp_global:centerWindow(birlikDuzenleme.window[1])
...
function vipBilgiPanel()
	...
	vipWindow = guiCreateWindow(0, 0, 450, 400, "Melancia * VIP Ozellikleri", false)
	guiWindowSetSizable(vipWindow, false)
	exports.mrp_global:centerWindow(vipWindow)
...
function ckActirma()
	if not isElement(CKClass.window[1]) then
		...
		exports.mrp_global:centerWindow(CKClass.window[1])
		guiSetInputEnabled(true)
...
function createLogGUI()
	...
	window = guiCreateWindow(0, 0, 860, 481, "Melancia - Marketlog Sistemi", false)
