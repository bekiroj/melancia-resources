...
local region = ColShape.Sphere(822.01171875, 1.876953125, 1004.1796875, 3)
setElementInterior(region, 3)
setElementDimension(region, 136)
local pickup = Pickup(822.01171875, 1.876953125, 1004.1796875, 3, 1239)
pickup:setData('informationicon:information', '#3568CD/atyarisi#ffffff\nTürkiye Binicilik Ligi')
setElementInterior(pickup, 3)
setElementDimension(pickup, 136)

Racing.constructor = function()
	if not localPlayer:isWithinColShape(region) then return end
	show = true
	active = false
	finish = false
	maxX = 600
	...
	Racing.render = Timer(
		function()
			if show and localPlayer:getData('money') > 5000 then
				...
				dxDrawImage(screen.x/2-625/2, screen.y/2-435/2, 150, 150, 'components/logo.png')
				...
				dxDrawText('At Yarışı @melancia',screen.x/2-75/2, screen.y/2+410/2, 650, 150, tocolor(200,200,200,250), 1, font)
...
addCommandHandler('atyarisi', Racing.constructor)