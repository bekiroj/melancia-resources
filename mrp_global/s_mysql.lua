...
function getSmallestIdFromDbTable(tableName) -- finds the smallest ID in the SQL instead of auto increment
	...
	local result = dbPoll(dbQuery(exports.mrp_mysql:getConnection(), "SELECT MIN(e1.id+1) AS nextID FROM "..tableName.." AS e1 LEFT JOIN "..tableName.." AS e2 ON e1.id +1 = e2.id WHERE e2.id IS NULL"), -1)
	if result then
