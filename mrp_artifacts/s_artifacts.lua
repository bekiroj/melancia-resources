...
function addArtifact(player, artifact, noOutput, customItemTexture) --Start to wear an artifact the player is not already wearing
	...
	if player and artifact then	
		if artifacts[player] and artifacts[player][artifact] then
			...
			exports.mrp_bone_attach:attachElementToBone(object,player,data[ART_BONE],data[ART_X],data[ART_Y],data[ART_Z],data[ART_RX],data[ART_RY],data[ART_RZ])
			...
			if exports.mrp_global:isResourceRunning( 'item-texture' ) then
				if data[ART_TEXTURE] then
					...
					for k,v in ipairs(data[ART_TEXTURE]) do
						exports["mrp_item_texture"]:addTexture(object, v[2], v[1])
					end
			...
			if not noOutput and g_artifacts_mes[artifact] and g_artifacts_mes[artifact][1] then
				exports.mrp_global:sendLocalMeAction(player, tostring(g_artifacts_mes[artifact][1]))
			end
		end
	end
end
	...
	if player and artifact then	
		if not artifacts[player] or not artifacts[player][artifact] then
			...
			if not noOutput and g_artifacts_mes[artifact] and g_artifacts_mes[artifact][2] then
				exports.mrp_global:sendLocalMeAction(player, tostring(g_artifacts_mes[artifact][2]))
			end
		end
	end
end
...
function setPlayerArtifactProperty(player, artifact, property, value) --exported function to let other scripts change things about a worn artifact (adjust position, change model, change bone, etc.)
	if artifacts[player] and artifacts[player][artifact] and isElement(artifacts[player][artifact]) then
		...
		elseif(property == "bone") then
			local ped, bone, x, y, z, rx, ry, rz = exports.mrp_bone_attach:getElementBoneAttachmentDetails(object)
			exports.mrp_bone_attach:detachElementFromBone(object)
			local result = exports.mrp_bone_attach:attachElementToBone(object,ped,value,x,y,z,rx,ry,rz)
			if not result then
				exports.mrp_bone_attach:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
		elseif(property == "x") then
			local ped, bone, x, y, z, rx, ry, rz = exports.mrp_bone_attach:getElementBoneAttachmentDetails(object)
			exports.mrp_bone_attach:detachElementFromBone(object)
			local result = exports.mrp_bone_attach:attachElementToBone(object,ped,bone,(x+value),y,z,rx,ry,rz)
			if not result then
				exports.mrp_bone_attach:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
		elseif(property == "y") then
			local ped, bone, x, y, z, rx, ry, rz = exports.mrp_bone_attach:getElementBoneAttachmentDetails(object)
			exports.mrp_bone_attach:detachElementFromBone(object)
			local result = exports.mrp_bone_attach:attachElementToBone(object,ped,bone,x,(y+value),z,rx,ry,rz)
			if not result then
				exports.mrp_bone_attach:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
		elseif(property == "z") then
			local ped, bone, x, y, z, rx, ry, rz = exports.mrp_bone_attach:getElementBoneAttachmentDetails(object)
			exports.mrp_bone_attach:detachElementFromBone(object)
			local result = exports.mrp_bone_attach:attachElementToBone(object,ped,bone,x,y,(z+value),rx,ry,rz)
			if not result then
				exports.mrp_bone_attach:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
		elseif(property == "rx") then
			local ped, bone, x, y, z, rx, ry, rz = exports.mrp_bone_attach:getElementBoneAttachmentDetails(object)
			exports.mrp_bone_attach:detachElementFromBone(object)
			local result = exports.mrp_bone_attach:attachElementToBone(object,ped,bone,x,y,z,(rx+value),ry,rz)
			if not result then
				exports.mrp_bone_attach:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
		elseif(property == "ry") then
			local ped, bone, x, y, z, rx, ry, rz = exports.mrp_bone_attach:getElementBoneAttachmentDetails(object)
			exports.mrp_bone_attach:detachElementFromBone(object)
			local result = exports.mrp_bone_attach:attachElementToBone(object,ped,bone,x,y,z,rx,(ry+value),rz)
			if not result then
				exports.mrp_bone_attach:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
		elseif(property == "rz") then
			local ped, bone, x, y, z, rx, ry, rz = exports.mrp_bone_attach:getElementBoneAttachmentDetails(object)
			exports.mrp_bone_attach:detachElementFromBone(object)
			local result = exports.mrp_bone_attach:attachElementToBone(object,ped,bone,x,y,z,rx,ry,(rz+value))
			if not result then
				exports.mrp_bone_attach:attachElementToBone(object,ped,bone,x,y,z,rx,ry,rz)
			end
			return result
