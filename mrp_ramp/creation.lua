...
function ramp_add ( p, cmd )
    ...
    local ty = y + radius * math.cos ( math.rad ( rz ) )
		...
		if query then
			...
			exports.mrp_global:giveItem ( p, 151, id )
			exports.mrp_global:sendMessageToAdmins ( "AdmWarn: " .. creator .. " created a ramp with ID " .. id )
		else
...
function ramp_delete ( p, cmd, id )
    ...
    local query = dbExec(mysql:getConnection(), "DELETE FROM ramps WHERE id = " .. id )
		...
		if query then
			...
			exports.mrp_global:sendMessageToAdmins ( "AdmWarn: " .. deleter .. " deleted a ramp with ID " .. id )
		else
