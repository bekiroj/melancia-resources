			...
			function(qh)
				...
				if rows > 0 then
					for index, row in ipairs(res) do
						...
						exports.mrp_pool:allocateElement(object)
...
function updateBankMoney(thePlayer, charID, money, transfer)
	...
	if charID < 0 then -- faction id
		...
		if not transfer then
			return exports.mrp_global:setMoney(foundFaction, money)
		else
			if transfer == "minus" then
				return exports.mrp_global:takeMoney(foundFaction, money)
			elseif transfer == "plus" then
				return exports.mrp_global:giveMoney(foundFaction, money)
			else
				return false
			end
		end
...
function getATMName(theAtm)
	return "ATM Machine ID#"..getElementData(theAtm, "dbid").." at "..exports.mrp_global:getElementZoneName(theAtm)
end

addEvent('BankBrowser-DepositMoney', true)
addEventHandler('BankBrowser-DepositMoney', root,
	function(amount)
		if exports.mrp_global:hasMoney(source, amount) and amount > 0 then
			exports.mrp_global:takeMoney(source, amount)
...
addEventHandler('BankBrowser-WidthrawMoney', root,
	function(amount)
		if source:getData('bankmoney') >= amount and amount > 0 then
			exports.mrp_global:giveMoney(source, amount)
			source:setData('bankmoney', source:getData('bankmoney') - amount)
			dbExec(mysql:getConnection(), "UPDATE characters SET bankmoney='"..source:getData('bankmoney').."' WHERE id='"..source:getData('dbid').."'");
...
function transferMoneyToPersonal(business, name, amount, reason)
	...
	if not dbid and not reciever then
		...
		if business then
			...
			if exports.mrp_global:takeMoney(theTeam, amount) then
		...
		if reciever then
			if dbid < 0 then
				exports.mrp_global:giveMoney(reciever, amount)
			else
		...
		outputChatBox("You transfered $" .. exports.mrp_global:formatMoney(amount) .. " from your "..(business and "business" or "personal").." account to "..name..(string.sub(name,-1) == "s" and "'" or "'s").." account.", client, 255, 194, 14)
		
		if business then
			exports.mrp_logs:dbLog(client, 25, { getPlayerTeam(client), "ch" .. dbid }, "TRANSFER FROM BUSINESS " .. amount .. " TO " .. name)
		else
			exports.mrp_logs:dbLog(client, 25, { client, "ch" .. dbid }, "TRANSFER " .. amount .. " TO " .. name)
		end
