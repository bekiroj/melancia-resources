...
local bones = {
"Baş", "Boyun", "Omurga", "Alt karın", "Sol köprücük", "Sağ köprücük",
"Sol omuz", "Sağ omuz", "Sol dirsek", "Sağ dirsek",
"Sol el", "Sağ el", "Sol kalça", "Sağ kalça", "Sol diz", "Sağ diz",
"Sol ayak bileği", "Sağ ayak bileği", "Sol ayak", "Sağ ayak",
}
...
function saveWeaponTable()
	local xml = fileExists(xmlPath) and xmlLoadFile(xmlPath) or xmlCreateFile(xmlPath, "attachments")
	if xml then
...
function createSelectionWindow()
	...
	sWin = guiCreateWindow(sx - 225, sy - 250, 215, 202, "Silah Konumlandırma", false)

	weaponsGridList = guiCreateGridList(0, 24, 215, 130, false, sWin)
	...
	local weaponName = guiGridListAddColumn(weaponsGridList, "Silah", 0.5)
	local weaponAttached = guiGridListAddColumn(weaponsGridList, "Bağlı", 0.2)
	addEventHandler("onClientGUIDoubleClick", weaponsGridList, doubleClickedWeapon, false)
	populateWeaponGridList()

	local help = guiCreateLabel(12, 155, 200, 12, "Açıp kapatmak için silaha çift tıklayın.", false, sWin)
	guiSetFont(help, "default-small")
	guiLabelSetHorizontalAlign(help, "center")

	btnSelectionEdit = guiCreateButton(0, 172, 97, 20, "Düzenle", false, sWin)
	addEventHandler("onClientGUIClick", btnSelectionEdit, createEditWindow, false)

	btnSelectionClose = guiCreateButton(107, 172, 97, 20, "Kapat", false, sWin)
...
addCommandHandler("silahgoster", createSelectionWindow)
	...
	for i = 1, 12 do
		...
		if weapon ~= 0 then
			...
			guiGridListSetItemText(weaponsGridList, row, 3, getElementData(localPlayer,
				"attachingWeapon" .. weapon) and "Evet" or "Hayır", false, false)
		end
	end
end
...
function createEditWindow()
	...
	local sx, sy = guiGetScreenSize()
	eWin = guiCreateWindow(sx - 250, sy - 375, 225, 325, "Silah Koordinatlarını Düzenle", false)
	guiWindowSetSizable(eWin, false)

	guiCreateLabel(10, 26, 210, 15,
		"Silah: " .. name .. " (" .. id .. ") Skin: #" .. skin, false, eWin)

	guiCreateLabel(10, 52, 50, 15, "Kemik:", false, eWin)
	bonesComboBox = guiCreateComboBox(65, 50, 150, 150, "Seç", false, eWin)
		...
		addEventHandler("onClientGUIChanged", posEditBox[i],
			function ()
				...
				if math.abs(x) > 0.5 or math.abs(y) > 0.5 or math.abs(z) > 0.5 then
					...
					if isElement(editingObject) then
						exports.mrp_bone_attach:attachElementToBone(editingObject,
	...
	btnEditSave = guiCreateButton(0, 295, 67, 20, "Kaydet", false, eWin)
	addEventHandler("onClientGUIClick", btnEditSave, saveWeaponAttachment, false)

	btnEditReset = guiCreateButton(78, 295, 67, 20, "Varsayılan", false, eWin)
	addEventHandler("onClientGUIClick", btnEditReset, resetWeaponAttachment, false)

	btnEditClose = guiCreateButton(146, 295, 67, 20, "Kapat", false, eWin)
	addEventHandler("onClientGUIClick", btnEditClose, closeEditWindow, false)

	local help = guiCreateLabel(10, 255, 210, 35, "X, Y ve Z uzaklıkları, -0.5 ile 0.5 arasında olmalıdır..\n" ..
		"Çevrenize bakmak için 'm' tuşuna basın.", false, eWin)
	guiSetFont(help, "default-small")
	guiLabelSetHorizontalAlign(help, "center")
...
function createEditingObject(weapon, bone, x, y, z, rx, ry, rz)
	...
	exports.mrp_bone_attach:attachElementToBone(object, localPlayer, bone, x, y, z, rx, ry, rz)
...
addEventHandler("onClientPlayerWeaponSwitch", root,
	function (prev, curr)
		...
		if getPedWeapon(localPlayer, prev) == 0 then
			...
			triggerServerEvent("destroyWeaponFromSlot", localPlayer, prev)
		end
...
addEventHandler("accounts:character:select",root,onChar)
addEventHandler("finishItemMove",root,onChar)

function updateWeapons()
	print("updated : "..getTickCount(  ))
	loadWeaponTable()
	for i,v in pairs(defaultWeaponPositions) do
		if not ZORUNLU_OLMAYAN[i] then
			setElementData(localPlayer, "attachingWeapon" .. i, true, true)
		end
	end
	for i, model in pairs(models) do
		if getElementData(localPlayer, "attachingWeapon" .. i) then
...
addEvent("st.updateWeapons",true)
addEventHandler("st.updateWeapons",root,updateWeapons)
addEventHandler("onClientResourceStart", resourceRoot, updateWeapons)
