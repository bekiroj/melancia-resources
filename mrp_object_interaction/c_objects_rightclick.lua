...
itemworld = exports['mrp_item_world']
item = exports['mrp_items']
		...
		if (getDistanceBetweenPoints3D(x, y, z, wx, wy, wz)<=maxDistance) then
			...
			if getElementParent(getElementParent(element)) == getResourceRootElement(getResourceFromName("mrp_item_world")) then
				...
				if itemworld:can(localPlayer, "use", element) then
					...
					elseif(exports.mrp_sittablechairs:isSittableChair(element)) then
						if(exports.mrp_sittablechairs:canSitOnChair(element)) then
							row.a = rightclick:addrow("Otur")
							addEventHandler("onClientGUIClick", row.a,  function (button, state)
								exports.mrp_sittablechairs:attemptToSitOnChair(element)
							end, false)
						end
					end
				end

				if (getElementDimension(element) ~= 0 or exports.mrp_integration:isPlayerTrialAdmin(localPlayer) or getElementData(localPlayer, "admin:tempmove")) and not getElementData(localPlayer, "jailed") then
					if not rcMenu then rcMenu = rightclick:create(itemName) end
				...
				if not noPickupFor[itemID] and itemworld:can(localPlayer, "pickup", element) then
					...
					addEventHandler("onClientGUIClick", row.pickup,  function (button, state)
						if itemID ~= 223 and itemID ~= 103 then
							if exports.mrp_global:hasSpaceForItem(localPlayer, itemID, itemValue, metadata) then
				...
				if(exports.mrp_sittablechairs:isSittableChair(element)) then
					if(exports.mrp_sittablechairs:canSitOnChair(element)) then
			...
			if(model == 2517) then --SHOWERS
				if not rcMenu then  rcMenu = exports.mrp_rightclick:create("Duş Kabini") end
				if showering[1] then
					row.a = exports.mrp_rightclick:addrow("Duştan Çık")
					addEventHandler("onClientGUIClick", row.a,  function (button, state)
						takeShower(element)
					end, false)
				else
					row.a = exports.mrp_rightclick:addrow("Duş Al")
					addEventHandler("onClientGUIClick", row.a,  function (button, state)
						takeShower(element)
					end, false)
				end
			elseif(model == 2146) then --Stretcher (ES)
				if not rcMenu then  rcMenu = exports.mrp_rightclick:create("Stretcher") end
				row.a = exports.mrp_rightclick:addrow("Take Stretcher")
				addEventHandler("onClientGUIClick", row.a,  function (button, state)
			...
			elseif(model == 962) then --Airport gate control box
				...
				if airGateID then
					if not rcMenu then  rcMenu = exports.mrp_rightclick:create("Control Box") end
					row.a = exports.mrp_rightclick:addrow("Control Gate")
					addEventHandler("onClientGUIClick", row.a,  function (button, state)
