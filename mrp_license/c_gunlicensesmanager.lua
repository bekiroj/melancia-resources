...
function mainGUI()
	...
	GUIEditor.window[1] = guiCreateWindow(532, 299, 542, 266, "Firearms Licensing Unit - Powered by Tree Technology", false)
...
function issuelicenseGUI()
	...
	addEventHandler("onClientGUIClick", issue.button[1], function ()
		...
		if licensetype == "Tier 2" then
			if exports.mrp_factions:isPlayerFactionLeader(getLocalPlayer(), 1) or exports.mrp_factions:isPlayerFactionLeader(getLocalPlayer(), 50) or exports.mrp_integration:isPlayerSeniorAdmin(getLocalPlayer()) then
			else
		...
		timer3 = setTimer( function ()
				...
				refreshMainGUI()
				for k,v in ipairs(licenses_table) do
					if v["charactername"] == string.gsub(guiGetText(issue.edit[1]), " ", "_") then
						guiGridListSetVerticalScrollPosition(GUIEditor.gridlist[1], k / #licenses_table * 100)
						guiGridListSetSelectedItem(GUIEditor.gridlist[1], k - 1, 1)
						guiSetInputMode("allow_binds")
						if isElement(issue.window[1]) then destroyElement(issue.window[1]) end
