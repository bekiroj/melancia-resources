...
function recoveryLicense(licensetext, cost, itemID, npcName)
	if not exports.mrp_global:takeMoney(source, cost) then
		exports.mrp_hud:sendBottomNotification(source, npcName, "Can I have $"..exports.global:formatMoney(cost).." fee for recovering the "..licensetext.." please?")
		return false
	end

	if exports.mrp_global:giveItem(source, itemID, getPlayerName(source):gsub("_", " ")) then
		exports.mrp_hud:sendBottomNotification(source, npcName, "You have paid $"..exports.global:formatMoney(cost).." fee for recovering the "..licensetext..".")
	end
end
...
function onLicenseServer()
	...
	if (gender == 0) then
		exports.mrp_global:sendLocalText(source, "Carla Cooper diyor ki: Merhaba beyefendi, ehliyet için mi başvuracaktınız?", 255, 255, 255, 10)
	else
		exports.mrp_global:sendLocalText(source, "Carla Cooper diyor ki: Merhaba hanfendi, ehliyet için mi başvuracaktınız?", 255, 255, 255, 10)
	end
end
...
function payFee(amount, reason)
	if exports.mrp_global:takeMoney(source, amount) then
		if not reason then
			reason = "a license"
		end
		exports.mrp_hud:sendBottomNotification(source, "Motorlu Taşıtlar Departmanı", reason.." için ₺"..exports.mrp_global:formatMoney(amount).." ödediniz.")
	end
end
