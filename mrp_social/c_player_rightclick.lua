...
function showPlayerSelfMenu()
	...
	if getElementData(getLocalPlayer(), "realism:stretcher:hasStretcher") then
		if not rcMenu then
			rcMenu = exports['mrp_rightclick']:create("Me ("..tostring(playerid)..")")
		end
		row.stretcher = exports['mrp_rightclick']:addRow("Sedyeyi bırak")
...
function showPlayerMenu(targetPlayer, friend)
	...
	rcMenu = exports.mrp_rightclick:create(string.gsub(exports.mrp_global:getPlayerName(targetPlayer), "_", " ").." ("..tostring(playerid)..")")
	
	if not friend then
		bAddAsFriend = exports['mrp_rightclick']:addRow("Arkadaş Ekle")
		addEventHandler("onClientGUIClick", bAddAsFriend, caddFriend, false)
	else
		bAddAsFriend = exports['mrp_rightclick']:addRow("Arkadaşlıktan çıkar")
		addEventHandler("onClientGUIClick", bAddAsFriend, cremoveFriend, false)
	end

	-- FRISK
	bFrisk = exports['mrp_rightclick']:addRow("Üstünü Ara")
	addEventHandler("onClientGUIClick", bFrisk, cfriskPlayer, false)
	
	-- RESTRAIN
	local cuffed = getElementData(player, "Kelepçele")
	if cuffed == 0 then
		bRestrain = exports['mrp_rightclick']:addRow("Restrain")
		addEventHandler("onClientGUIClick", bRestrain, crestrainPlayer, false)
	else
		bRestrain = exports['mrp_rightclick']:addRow("Kelepçeyi çıkar")
	...
	if (blindfold) and (blindfold == 1) then
		bBlindfold = exports['mrp_rightclick']:addRow("Göz Bandını Çıkar")
		addEventHandler("onClientGUIClick", bBlindfold, cremoveBlindfold, false)
	else
		bBlindfold = exports['mrp_rightclick']:addRow("Göz Bandı")
		addEventHandler("onClientGUIClick", bBlindfold, cBlindfold, false)
	end
	
	-- STABILIZE
	if exports.mrp_global:hasItem(getLocalPlayer(), 70) and getElementData(player, "injuriedanimation") then
		bStabilize = exports['mrp_rightclick']:addRow("Stabilize")
...
function cStabilize(button, state)
	if button == "left" and state == "up" then
		if (exports.mrp_global:hasItem(getLocalPlayer(), 70)) then -- Has First Aid Kit?
...
function cBlindfold(button, state, x, y)
	if (button=="left") then
		if (exports.mrp_global:hasItem(getLocalPlayer(), 66)) then -- Has blindfold?
...
function crestrainPlayer(button, state, x, y)
	if (button=="left") then
		if (exports.mrp_global:hasItem(getLocalPlayer(), 45) or exports.mrp_global:hasItem(getLocalPlayer(), 46)) then
			local restrained = getElementData(player, "restrain")
			...
			if (restrained==1) then
				...
				if (exports.mrp_global:hasItem(getLocalPlayer(), 45)) then
					restrainedObj = 45
				elseif (exports.mrp_global:hasItem(getLocalPlayer(), 46)) then
					restrainedObj = 46
				end
...
function cunrestrainPlayer(button, state, x, y)
	if (button=="left") then
		...
		if (restrained==0) then
			...
			if (exports.mrp_global:hasItem(getLocalPlayer(), 47, dbid)) or (restrainedObj==46) then -- has the keys, or its a rope
...
function friskShowItems(items)
	...
	for k, v in ipairs(items) do
		local itemName = v[1] ~= 80 and exports.mrp_global:getItemName(v[1]) or v[2]
