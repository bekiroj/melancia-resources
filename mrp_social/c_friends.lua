...
-- account logged in
--
addEvent( "social:mrp_auth", true )
addEventHandler( "social:mrp_auth", root,
	function( accountID )
...
function drawFriends( )
	...
	local line = scroll_y
	for _, data in ipairs( { { title = "Çevimiçi", items = online }, { title = "Çevrimdışı", items = offline } }) do
		-- render title
		...
		for key, value in ipairs( data.items ) do
			...
			-- draw name
			if exports.mrp_integration:isPlayerTrialAdmin(value.player) then
				dxDrawText( value.name, x, y, w, 15, tocolor ( 255, 194, 14, 255 ), 1, font, "left", "top", false, false, true )
			elseif exports.mrp_integration:isPlayerSupporter(value.player) then
				dxDrawText( value.name, x, y, w, 15, tocolor ( 70, 200, 30, 255 ), 1, font, "left", "top", false, false, true )
			else
			...
			if inBox and justClicked then
				...
				elseif not value.editable then
					...
					friends_message_window = guiCreateWindow( screen_x / 2 - 200, screen_y / 2 - 30, 400, 60, "Durum Mesajını Güncelle", false )
					local e = guiCreateEdit( 5, 25, 290, 30, value.message, false, friends_message_window )
					local c = guiCreateButton( 300, 25, 45, 30, "Kaydet", false, friends_message_window )
					local d = guiCreateButton( 350, 25, 45, 30, "Kapat", false, friends_message_window )
			...
			if value.player then
				text = "En son oynama " .. getPlayerName( value.player ):gsub( "_", " " ) .. " (" .. getElementData( value.player, "playerid" ) .. ")."
			else
				text = ( formatTimeInterval( value.lastOnline ) .. " Saat önce." ):gsub( "  ", " " )
			end
