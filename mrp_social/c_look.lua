...
function createLookWindow( name, description, attributes, picture, editFunction )
	...
	local column = guiGridListAddColumn( grid, "Rozet, Kask veya Maske", 0.95)
	...
	if editFunction then
		...
		local edit = guiCreateButton( x, 360, 130, 25, "Düzenle", false, advlook_window )
		addEventHandler( "onClientGUIClick", edit, editFunction, false )
		
		x = x + 140
	end
	local close = guiCreateButton( x, 360, 130, 25, "Kapat", false, advlook_window )
	...
	if usePicture then
		...
		requestBrowserDomains({ "i.imgur.com", "resimag.com" })
		addEventHandler("onClientBrowserWhitelistChange", root,
			 function(newDomains)
				 if newDomains[1] == "i.imgur.com" or newDomains[1] == "resimag.com" then
...
addEventHandler( "social:look", getRootElement( ),
	function( age, race, gender, weight, height, description )
		...
		if source == localPlayer or exports.mrp_global:isAdminOnDuty(localPlayer) then
		...
		local editButton = createLookWindow( currentLookData["name"],
			currentLookData["name"] .. " " .. ( race == 0 and "siyahi" or race == 1 and "beyaz" or "asyalı" ) .. " " .. ( gender == 1 and "kadın" or "erkek" ) ..
			", kilosu " .. math.floor(weight * 2.2 + 0.5) .. " lb (" .. weight .. "kg)" ..
			" ve boyu " .. height .. "cm.\n\n" ..
			"Saç Rengi: " .. description[1] .. "\n" ..
			"Saç Stili:: " .. description[2] .. "\n" ..
			"Yüz Hatları: " .. description[3] .. "\n" ..
			"Fiziksel Özellikleri: " .. description[4] .. "\n" ..
			"Kıyafetler: " .. description[5] .. "\n" ..
			"Aksesuarlar: " .. description[6],
			description[8] or {},
			description[7],
			editFunction
		)
...
function createEditablesWindow( )
	...
	advlook_edit_window = guiCreateWindow( screen_x - 191, 172, 181, 297, "Özelliklerini Düzenle!", false )
	...
	local column = guiGridListAddColumn( grid, "Ayarlar", 0.9 )
	...
	addEventHandler( "onClientGUIClick", save,
		function( )
			...
			if localPlayer ~= currentLookData["player"] then
				if exports.mrp_global:isAdminOnDuty(localPlayer) then
